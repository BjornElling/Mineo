...
- Evidensrevision: `07b0b340` for seneste samlede releasegate og målrettede test-only batch; seneste fulde E2E er `71746917`
- Branch: `main`
...
- Seneste test-only batch: ARCH-/DOC-facitter TD-273–TD-274; målrettet kontrol 2 nye tests / 2 tests; ingen produktkode, beregningslogik, UI/UX eller persistens ændret.
...
| Aktuel fuld Vitest-retest | `npm run test:coverage` | Bestået | 901 filer, 9.076 tests bestået og ingen forventede `it.fails` på `07b0b340` |
...
| Aktuel samlet release-gate | `npm run verify:release:core` | Bestået | Dependency-, runtime-, type-, lint-, data-, kontrakt-, lane-, ledger-, coverage- og begge build-gates bestået på `07b0b340`; coverage 901 filer / 9.076 beståede tests / ingen forventede `it.fails` |
...
| Aktuel coverage-retest | `npm run test:coverage` | Bestået | 394 instrumenterede filer; 90,74 % statements, 81,99 % branches, 94,88 % functions, 93,53 % lines; coverage-artefakterne blev genereret under `coverage/`; `07b0b340`, Node `v24.18.0`/Windows |
...
Aktuel inventarrevision på `07b0b340`: `PERSIST-002` omfatter nu også den nye silent-read-,
...
| Senest tilføjet lukket | `TD-153`, `TD-154`, `TD-155`, `TD-157`–`TD-274` – test-only facitter dokumenteret til og med `07b0b340`; `TD-156` er fortsat åbent |

## Aktuel evidensdelta – revision `07b0b340`
