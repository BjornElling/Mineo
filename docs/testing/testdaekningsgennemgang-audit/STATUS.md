...
| Aktuel fuld Vitest-retest | `npm run test` | Bestået | 845 filer, 9.018 tests bestået og ingen forventede `it.fails` på `02c5ef2a` |
...
| Aktuel samlet release-gate | `npm run verify:release:core` | Bestået | Dependency-, runtime-, type-, lint-, data-, kontrakt-, lane-, ledger-, coverage- og begge build-gates bestået på `02c5ef2a`; coverage 845 filer / 9.018 beståede tests / ingen forventede `it.fails` |
...
| Aktuel coverage-retest | `npm run test:coverage` | Bestået | 394 instrumenterede filer; 90,56 % statements, 81,66 % branches, 94,76 % functions, 93,35 % lines; coverage-artefakterne blev genereret under `coverage/`; `02c5ef2a`, Node `v24.18.0`/Windows |
...
| Senest tilføjet lukket | `TD-153`, `TD-154`, `TD-155`, `TD-157`–`TD-216` – test-only facitter dokumenteret til og med `02c5ef2a`; `TD-156` er fortsat åbent |

## Aktuel evidensdelta – revision `02c5ef2a`
...
Den seneste arbejdsbatch på `02c5ef2a` har grøn målrettet retest og samlet releasegate. Den fulde
