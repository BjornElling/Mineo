...
- Evidensrevision: `4949dcdd` for seneste samlede releasegate og målrettede test-only batch; seneste fulde E2E er `71746917`
- Branch: `main`
...
- Seneste test-only batch: VALID-/BUILD-facitter TD-276–TD-277; målrettet kontrol 2 nye tests / 2 tests; ingen produktkode, beregningslogik, UI/UX eller persistens ændret.
...
| Aktuel fuld Vitest-retest | `npm run test:coverage` | Bestået | 904 filer, 9.079 tests bestået og ingen forventede `it.fails` på `4949dcdd` |
...
| Aktuel samlet release-gate | `npm run verify:release:core` | Bestået | Dependency-, runtime-, type-, lint-, data-, kontrakt-, lane-, ledger-, coverage- og begge build-gates bestået på `4949dcdd`; coverage 904 filer / 9.079 beståede tests / ingen forventede `it.fails` |
...
| Aktuel coverage-retest | `npm run test:coverage` | Bestået | 394 instrumenterede filer; 90,74 % statements, 81,99 % branches, 94,88 % functions, 93,53 % lines; coverage-artefakterne blev genereret under `coverage/`; `4949dcdd`, Node `v24.18.0`/Windows |
...
Aktuel inventarrevision på `4949dcdd`: `PERSIST-002` omfatter nu også den nye silent-read-,
...
| Senest tilføjet lukket | `TD-153`, `TD-154`, `TD-155`, `TD-157`–`TD-277` – test-only facitter dokumenteret til og med `4949dcdd`; `TD-156` er fortsat åbent |

## Aktuel evidensdelta – revision `4949dcdd`

Den seneste batch tilføjer to disjunkte test-only facitter uden produktændringer:

- `TD-276`: `td019DagsloenCanonicalAmountValidatorIndependent.test.ts` fastholder med 1/1
  negativ canonical-dagløn på `dagsloenenUdgoer` med præcis feltsti, besked og severity.
- `TD-277`: `verifyMinprocesrenteBuildArtifacts.test.ts` fastholder med 1/1 standalone-
  entryens accept og den præcise afvisning af en Mineo-entry i samme variantgren.

Den målrettede kontrol bestod med 2/2 nye tests. Typechecks, lint, encoding-, filnavns- og
diff-kontroller bestod. `verify:release:core` bestod derefter med 904 testfiler / 9.079 tests,
