...
- Evidensrevision: `b1a0b3e6` for seneste samlede releasegate; seneste målrettede test-only batch er `881d285b`, og seneste fulde E2E er `72a6d44f`
- Branch: `main`
...
| Aktuel fuld Vitest-retest | `npm run test:coverage` | Bestået | 767 filer, 8.857 tests bestået og ingen forventede `it.fails`; coverage 90,26 / 81,39 / 94,03 / 93,06 på `b1a0b3e6` |
...
| Aktuel samlet release-gate | `npm run verify:release:core` | Bestået | Dependency-, runtime-, type-, lint-, data-, kontrakt-, lane-, ledger-, coverage- og begge build-gates bestået på `b1a0b3e6`; coverage 767 filer / 8.857 beståede tests / ingen forventede `it.fails` |
...
| Aktuel coverage-retest | `npm run test:coverage` | Bestået | 393 instrumenterede filer; 90,26 % statements, 81,39 % branches, 94,03 % functions, 93,06 % lines; `coverage/index.html`, `coverage/clover.xml` og `coverage/coverage-final.json`; `b1a0b3e6`, Node `v24.18.0`/Windows |
...
Den seneste målrettede batch på `881d285b` og den samlede releasegate på `b1a0b3e6` er retestet
grønt. Næste arbejdsenhed er en af de åbne, test-only dækningshuller, indtil et punkt kræver udviklerens
