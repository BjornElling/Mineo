...
- Evidensrevision: `add748af` for seneste samlede releasegate og målrettede test-only batch; seneste fulde E2E er `71746917`
- Branch: `main`
...
- Seneste test-only batch: INPUT-/BUILD-facitter TD-278–TD-279; målrettet kontrol 2 nye tests / 2 tests; ingen produktkode, beregningslogik, UI/UX eller persistens ændret.
...
| Aktuel fuld Vitest-retest | `npm run test:coverage` | Bestået | 906 filer, 9.081 tests bestået og ingen forventede `it.fails` på `add748af` |
...
| Aktuel samlet release-gate | `npm run verify:release:core` | Bestået | Dependency-, runtime-, type-, lint-, data-, kontrakt-, lane-, ledger-, coverage- og begge build-gates bestået på `add748af`; coverage 906 filer / 9.081 beståede tests / ingen forventede `it.fails` |
...
| Aktuel coverage-retest | `npm run test:coverage` | Bestået | 394 instrumenterede filer; 90,74 % statements, 81,99 % branches, 94,88 % functions, 93,54 % lines; coverage-artefakterne blev genereret under `coverage/`; `add748af`, Node `v24.18.0`/Windows |
...
Aktuel inventarrevision på `add748af`: `PERSIST-002` omfatter nu også den nye silent-read-,
...
| Senest tilføjet lukket | `TD-153`, `TD-154`, `TD-155`, `TD-157`–`TD-279` – test-only facitter dokumenteret til og med `add748af`; `TD-156` er fortsat åbent |

## Aktuel evidensdelta – revision `add748af`

Den seneste batch tilføjer to disjunkte test-only facitter uden produktændringer:

- `TD-278`: `settleNoOpAfterUndoReference.test.ts` fastholder med 1/1 gentaget canonical
  no-op-settle efter undo med bevaret redo-fremtid, revision, state-identitet og session-state.
- `TD-279`: `deployAssetDirectoryLiveness.test.ts` fastholder med 1/1, at begge Wrangler-
  asset-mapper matcher de tilsvarende Vite-builds.

Den målrettede kontrol bestod med 2/2 nye tests. Typechecks, lint, encoding-, filnavns- og
diff-kontroller bestod. `verify:release:core` bestod derefter med 906 testfiler / 9.081 tests,
