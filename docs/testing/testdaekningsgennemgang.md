...
| CALC-001 | Satser og fælles beregningsdata – `src/domain/satser/**` | Forkert satsår eller kildedata giver forkert visning og downstream-resultat. | `satser-contract.md`, `calculation-data-contract.md` | unit/integration/E2E | `domain/satser/**`, `domain/calculations/satserCalculations.test.ts`, `components/pages/Satser.downloadGate.integration.test.tsx`, `e2e/satser-tab-settle.spec.ts` | Den målrettede domænesuite bestod med 4 filer / 47 tests og dækker satsårsprojektion, seeded new-case, ASL-maksimum samt begge reguleringsmetoder med manglende-år- og heltalsgrænser. Integration, E2E og komplet data-/consumer-paritet mangler. | I gang |
...
| CALC-006 | Erstatningsopgørelse, rækkeevaluering og EO-inspektion – `src/domain/erstatningsopgoerelse/**`, `src/domain/eoRowEvaluation/**`, `src/domain/eoInspektion/**` | Forkert rækkeprioritet, TAF-/svie-/lønperiode, issue eller downstream-projektion ændrer opgørelse/dokument. | `eo-snapshot-contract.md`, `periodisering-contract.md`, `domain-boundary-contract.md`, relevante EO-contracts | unit/integration/E2E | `domain/erstatningsopgoerelse/**`, `domain/eoRowEvaluation/**`, `domain/eoInspektion/**`, `components/pages/erstatningsopgoerelse/**`, EO-E2E-specs | Et nyt uafhængigt EO-orakel bestod med 3 tests og kontrollerer totals, dokumentprojektion, inspektionsdage og dependency-/clampadfærd. Weekendydelsesscenariet viser samtidig en eksisterende `control:sammentaelling_mismatch`, som blokerer dokumentet; TD-016 kræver domæne-/produktbeslutning. Den øvrige delrækkevise audit er åben. | I gang |
...
| Mutation/modprøve og resultat | Der er ikke kørt mutation på CALC-006. Oraklet er en uafhængig modprøve af tre konkrete end-to-end-sammenhænge; den må ikke læses som bevis for alle rækkegrene. |
| Kørte kommandoer, miljø og artefaktlink | `npx vitest run src/__tests__/domain/erstatningsopgoerelse/eoSnapshotIndependentOracle.test.ts --reporter=dot` – 3/3 tests bestået på Node `v24.18.0`/Windows. Vite udsendte den kendte `configLoader: 'native'`-advarsel. |
...
| Felt | Udfyldes |
| --- | --- |
| Inventar-ID / gennemgået revision | `DOC-001` / `d0bc0672` |
...
| TD-016 | 2026-09-10 | `CALC-006` | Høj | En uafhængig EO-stikprøve med en ren weekendydelse i en arbejdsdagsbaseret TAF-sag giver `control:sammentaelling_mismatch`. Snapshot har canonical TAF-værdier, men dokumentgaten blokerer, og side-/inspection-beviset kræver fortsat særskilt afklaring. Det kan være tilsigtet fail-closed, men den forventede fallback og brugeroplevelse er uafklaret. | `eoSnapshotIndependentOracle.test.ts` bestod med 3/3 tests. I den konkrete sag blev TAF-arbejdsdagsgrundlag og canonical totals kontrolleret uafhængigt, hvorefter snapshot blev `status: 'error'` med `control:sammentaelling_mismatch`, og dokumentet var blokeret. | Åbent fund. Ingen produktkode ændret. Forelæg udvikleren, om den observerede fail-closed-adfærd er korrekt, eller om beregning, issue, fallback eller visning skal ændres. | Ja – enhver ønsket ændring af beregning eller synlig adfærd kræver udviklerens godkendelse. | Retest bestået: 3/3 uafhængige EO-tests. Åben – kræver udviklerens beslutning. |
