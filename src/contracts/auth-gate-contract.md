...
**Senest verificeret mod kode:** 2026-08-19 (normaliseringen i §2.3 er udvidet til at trimme
blanktegn i enderne, og §2.7 er ny: de to tekniske fejlårsager skal have hver sin besked, og
`LoginPage` skelner dem på fejlklassen. Baggrund: brugerfundene BB-055 og BB-056. Verificeret mod
`auth.ts`/`LoginPage.tsx` og dækket af de to auth-testfiler.)
...
3. **100 % client-side (jf. `AGENTS.md`).** Auth-flowet må aldrig indføre serverkald, eksterne API'er, telemetri eller ekstern logging. Adgangskode-verifikation sker lokalt via Web Crypto (`crypto.subtle.digest('SHA-256', …)`) efter normalisering; kun hex-hashes sammenlignes, og kun mod de hardcodede `SHARED_PASSWORD_HASHES`.

   **Normaliseringen har ÉT sted, og det er `hashPassword`.** Den er (a) blanktegns-neutral i enderne (`trim()`) og (b) case-neutral (`toLocaleLowerCase('da-DK')`), i den rækkefølge; config-hashes er beregnet på en plaintext, der opfylder begge. Trimmet gælder kun enderne – blanktegn INDE i koden er betydende. Et andet led i login-flowet må ikke normalisere teksten på sin egen måde: login-siden trimmede tidligere i sit «har du skrevet noget»-tjek, men ikke i verifikationen, og de to led var derfor uenige om, hvad adgangskoden ER. Konsekvensen var, at den rigtige adgangskode kopieret fra den mail, den blev sendt i – hvor markeringen næsten altid tager det afsluttende mellemrum eller linjeskift med – blev afvist som «Forkert adgangskode», mens feltet viste prikker, så der var intet at se. En adgangskode kan ikke meningsfuldt begynde eller slutte med blanktegn, så der gives intet væk. Klartekst-adgangskoder må aldrig persisteres eller logges af auth-laget. Det dedikerede browser-testpassword må stå i `AGENTS.md`, fordi det skal være tilgængeligt i nye agent-sessioner og ikke er en brugerhemmelighed.
...
7. **De to tekniske fejlårsager skal have hver sin besked, og `LoginPage` skelner dem på TYPEN.** Login er det ene sted, hvor en fejl er en total blindgyde, så beskeden skal kunne handles på, hvor det er muligt. Blokeret lagring (`AuthStorageUnavailableError`) kan brugeren selv rette på et minut og skal derfor sige hvordan – tillad websitedata for webstedet – mens en manglende kryptografi-funktion er uafhjælpelig og beholder den generiske tekst. `LoginPage` må ikke kassere en mere præcis besked til fordel for sin egen generiske: de to årsager blev tidligere lagt sammen til én sætning, der hverken sagde hvad der gik galt, eller hvad brugeren kunne gøre. Forgreningen skal ske på fejlklassen, ikke på tekstindhold.
...
- `src/__tests__/auth/auth.test.ts` (verifikation inkl. case- og ende-blanktegns-neutralitet og at INTERNE blanktegn er betydende, persistens af flag, `AuthStorageUnavailableError` ved manglende storage og ved begge skrivefejl, deterministisk fejl ved manglende `crypto.subtle`).
- `src/__tests__/auth/LoginPage.test.tsx` (gaten forbliver lukket ved tomt input, forkert adgangskode og ved begge tekniske fejlårsager; de to årsager giver hver sin besked – prøven hævder eksplicit, at den generiske tekst IKKE vises ved blokeret lagring; åbnes først når både verifikation og persistering er lykkedes).
