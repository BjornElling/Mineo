...
import { GreenfieldGridDateCell, GreenfieldGridPercentCell } from '../../inputCore/react/fields/greenfieldGridCells';
...
export type GreenfieldLoenudviklingManuelProcentsatsTableProps = Readonly<{
  ...
  return <StandardGridTable tableWidth="1130px" useSmallFont={useSmallFont}>
    ...
    <tbody>{renderRows.map((renderRow, index) => {
      ...
      return <tr key={renderRow.rowId} data-mineo-row-id={renderRow.rowId} style={getStandardGridBodyRowStyle(index)}>
        <td style={getStandardGridCellStyle({ align: 'center' })}>{isBase ? <GridReadOnlyLockedCell gridCell={gc(0)} displayValue={baseDateDisplay} align="center" errorMessage={baseDateErrorMessage} infoTooltipText={baseDateInfoTooltipText} /> : <GreenfieldGridDateCell gridCell={gc(0)} cell={table.buildCellSpec(renderRow, bindings.manualPercentFields.dato, 0)} />}</td>
        <td style={getStandardGridCellStyle({ align: 'right' })}>{isBase ? <GridReadOnlyLockedCell gridCell={gc(1)} displayValue={appendInputUnitSuffix(formatPercentDisplay(0, true), INPUT_UNIT_SUFFIX.percent)} align="right" placeholder={LOCKED_PERCENT_PLACEHOLDER} /> : <GreenfieldGridPercentCell gridCell={gc(1)} cell={table.buildCellSpec(renderRow, bindings.manualPercentFields.procent, 1)} />}</td>
