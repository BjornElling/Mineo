...
- Evidensrevision: `f46b1164` for seneste samlede releasegate og målrettede test-only batch; seneste fulde E2E er `71746917`
- Branch: `main`
...
- Seneste test-only batch: PERSIST-/CALC-facitter TD-231–TD-232; målrettet kontrol 2 nye tests / 2 tests; ingen produktkode, beregningslogik, UI/UX eller persistens ændret.
...
| `ARCH-002` | Registry-completeness, rule-factory-liveness og udvalgte negative modcases tilføjet til arkitekturharnessets dedikerede tests | `architectureRules.test.ts`: 1 fil / 191 tests bestået; separat forventningsliste med 88 regel-ID'er, AST-opslag af eksporterede definitions/aggregater samt re-export-, liveness- og namespace-import-modcases. `focusTraversalNamespaceImportBlindspot.test.ts` kræver 1/1 finding for namespace-import, og `TD-015` er lukket for de konkrete huller | De enkelte reglers negative modcases, importgrænser og øvrige livenessværn mangler | `I gang` |
...
| Senest tilføjet lukket | `TD-153`, `TD-154`, `TD-155`, `TD-157`–`TD-232` – test-only facitter dokumenteret til og med `f46b1164`; `TD-156` er fortsat åbent |

## Aktuel evidensdelta – revision `f46b1164`
