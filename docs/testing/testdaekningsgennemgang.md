...
| Baseline: antal kildefiler, testfiler, tests, E2E-specs og mutationsscore | Ved auditstart: 923 produktionsfiler (`.ts/.tsx`), 638 testfiler i alt, 8.421 Vitest-tests, 36 E2E-specs og 174 E2E-tests. Slutnær revision `87b4df72`: 920 produktionsfiler, 660 Vitest-testfiler, 8.586 beståede tests samt 6 forventede `it.fails`, 40 E2E-specs og 179 beståede E2E-tests. Mutationsrunneren er kvalificeret modulvist: money 56/58 dræbt og `dateCommit.ts` 4/6 dræbt med 2 ækvivalente overlevere. |
...
| --- | --- | --- | --- | --- | --- |
| `CALC-002` | Årslønsengine og branch-tests | Separat håndberegnet oracle uden engine-helperen | Metode A/B/C, hele januar + februar 2024, nul og feriegrænse | `aarsloenIndependentOracle.test.ts`: 5/5; den samlede målrettede årslønsflade: 34/34 | Delvist – øvrige branches og downstream-paritet åbne |
...
| `CALC-004` | Varige mén-engine, projection og gate | Håndberegnet totalsag med særskilt aldersreduktion og afrunding | `10.530 × 37`, 22 % reduktion og ceil-afrunding | Engine → projection → gate giver `303.896` kr.; målrettet flade: 82/82 | Delvist – mutation, browserrejse og dokumentparitet åbne |
