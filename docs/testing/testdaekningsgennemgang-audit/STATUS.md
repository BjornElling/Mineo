...
- Evidensrevision: `e7b1c96c` for seneste samlede releasegate og målrettede test-only batch; seneste fulde E2E er `71746917`
- Branch: `main`
...
- Seneste test-only batch: PERSIST-/CALC-facitter TD-227–TD-228; målrettet kontrol 6 filer / 67 tests inklusive relaterede facitter; ingen produktkode, beregningslogik, UI/UX eller persistens ændret.
...
| `PERSIST-002` | Storage/settings, filhåndtag, PWA-filflows og reel session-reload gennemgået | 17 filer / 182 målrettede unit-/hook-tests; PWA-/filkørsel 4/4; reel input + aktiv fane + reload 1/1; supplerende 8-filers platformssuite med 89/89 beståede tests efter callable picker-detektion; `safeLocalStorage.test.ts` bestod med 11/11 direkte Node-fallback-isolationsassertions; `fileHandleVerification.test.ts` bestod med 22/22; `fileHandleKvStore.test.ts` bestod med 2/2; `fileOperationClientSession.persistence.test.ts` tilføjer 1/1 sessionStorage-klientidentitetsfacit efter simuleret reload; `fileSaveTarget.test.ts` tilføjer 1/1 orphaned-handle-facit før picker-flowet; `fileSaveTargetPermissionApiFailure.test.ts` tilføjer 1/1 permission-API-facit med handle-oprydning og picker-fallback; `fileHandleStorage.pendingRequestValidation.test.ts` tilføjer 1/1 fail-closed-facit for negativt ignoredFileCount; ugyldige `Date`-inputs har 8/8 grønne negative modcases | Fuld browser-/platformssammenhæng, herunder installeret PWA-filaflevering; `TD-022` | `I gang` |
...
Aktuel inventarrevision på `e7b1c96c`: `PERSIST-002` omfatter nu også `fileHandleStorage.pendingRequestValidation.test.ts`
med 1/1 fail-closed-facit, og `CALC-006` omfatter `eoInspektionRegulationKlLoenaftalerIndependent.test.ts`
med 1/1 facit for rå KL-lønaftaleentries i særskilt reguleringstabel. De tilsvarende målrettede inventartal er
dermed 17 filer / 182 tests for PERSIST-002 og 169 filer / 2.183 tests for CALC-006; række-/periodefacitterne
er 15/15.
...
| Senest tilføjet lukket | `TD-153`, `TD-154`, `TD-155`, `TD-157`–`TD-228` – test-only facitter dokumenteret til og med `e7b1c96c`; `TD-156` er fortsat åbent |

## Aktuel evidensdelta – revision `e7b1c96c`

Den seneste batch tilføjer to disjunkte test-only facitter uden produktændringer:

- `TD-227`: `fileHandleStorage.pendingRequestValidation.test.ts` fastholder med 1/1 fail-closed-
  resultat og intet IndexedDB-write for negativt `ignoredFileCount`.
- `TD-228`: `eoInspektionRegulationKlLoenaftalerIndependent.test.ts` fastholder med 1/1 den
  særskilte KL-lønaftale-reguleringstabel med to rå entries, præcise datoer og procentvisning.

Den målrettede kontrol bestod med 6 filer / 67 tests inklusive relaterede eksisterende facitter.
Typechecks, lint, encoding-, filnavns- og diff-kontroller bestod. `verify:release:core` bestod
derefter med 855 testfiler / 9.030 tests, coverage 90,71 / 81,89 / 94,79 / 93,50 og begge
