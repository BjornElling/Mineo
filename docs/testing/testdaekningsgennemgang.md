...
| Baseline: antal kildefiler, testfiler, tests, E2E-specs og mutationsscore | Ved auditstart: 923 produktionsfiler (`.ts/.tsx`), 638 testfiler i alt, 8.421 Vitest-tests, 36 E2E-specs og 174 E2E-tests. Aktuel auditrevision `83abfdad`: 920 produktionsfiler, 661 Vitest-testfiler, 8.603 beståede tests samt 6 forventede `it.fails`, 40 E2E-specs og 180 beståede E2E-tests. Mutationsrunneren er kvalificeret modulvist: money 56/58 dræbt, `dateCommit.ts` 4/6 dræbt, årsløn 108/113 dræbt, varige mén 94/114 dræbt, forsørgertab 21/21 dræbt og procesrente 104/136 dræbt med 23 triagerede survivors og 9 dokumenterede timeouts; alle ikke-dræbte mutationer er triageret. |
| Baseline: `test:coverage`-rapport og de dækkede/udeladte mapper | `coverage/` fra auditrevisionen: 396 instrumenterede filer i `src/domain`, `src/utils`, `src/hooks` og `src/contexts`; den samlede releasekørsel på `83abfdad` gav 89,23 % statements, 80,48 % branches, 92,35 % functions og 92,02 % lines – 18.369 / 20.584, 12.950 / 16.090, 2.972 / 3.218 og 16.776 / 18.230 målte enheder. Coverage-konfigurationen omfatter ikke de øvrige produktionsfiler; de skal klassificeres i inventaret. |
...
| Kørte kommandoer, miljø og artefaktlink | `npx vitest run src/__tests__/data/uafhaengigSatsFacitmatrix.test.ts src/__tests__/domain/satser/satserDownstreamIndependentOracle.test.ts --reporter=dot` – de uafhængige facitspor bestod. `npx vitest run src/__tests__/data/uafhaengigeDataEndepunkter.test.ts --reporter=dot` – 25/25. `npx vitest run src/__tests__/domain/data/td020FolkepensionDownstream.test.ts --reporter=dot` – 2/2. Den tidligere samlede data-/satssuite bestod med 23 filer / 477 tests på Node `v24.18.0`/Windows. |
...
| Kørte kommandoer, miljø og artefaktlink | `npx vitest run src/__tests__/data src/__tests__/domain/satser src/__tests__/quality/consumerInventory.test.ts --reporter=dot` – 23 filer / 477 tests bestået på Node `v24.18.0`/Windows. Vite udsendte den kendte `configLoader: 'native'`-advarsel. |
...
| Kørte kommandoer, miljø og artefaktlink | Målrettet CALC-006-kørsel: `npx vitest run src/__tests__/domain/erstatningsopgoerelse src/__tests__/domain/eoRowEvaluation src/__tests__/domain/eoInspektion src/__tests__/components/pages/erstatningsopgoerelse --reporter=dot` – 22 filer / 233 tests bestået på Node `v24.18.0`/Windows. De nye orakler bestod med 7/7. Vite udsendte den kendte `configLoader: 'native'`-advarsel. |
...
| Felt | Udfyldes |
| --- | --- |
| Inventar-ID / gennemgået revision | `DOC-001` / `e867ab99` |
...
| Felt | Udfyldes |
| --- | --- |
| Inventar-ID / gennemgået revision | `DOC-002`, `DOC-003` / arbejdsrevision efter `83abfdad` |
...
| Felt | Udfyldes |
| --- | --- |
| Inventar-ID / gennemgået revision | `SHELL-001`, `SHELL-002` / arbejdsrevision efter `e867ab99` |
...
| Manglende test, svage assertioner eller dubletter | Bootstrap-testen observerer nu, at stylesheet, PWA-filåbning, install-capture, `beforeDesktopRender`, `afterDesktopRenderSetup` og `renderApp` ikke kaldes på en unsupported device, samtidig med at preload-recovery, install-prompt-suppression og hard-stop-rendering sker. `unsupported-device-hard-stop.spec.ts` beviser desuden den synlige mobilside, manglende appfelter og tavs runtime. Lazy/PWA-recovery er fortsat jsdom-/mockbaseret, ikke en faktisk manglende chunk → synlig recovery → kritisk reload-rejse. Native `launchQueue` kan ikke fuldt fremkaldes i Playwright. `pwa-file-open.spec.ts` bruger nu den fælles `runtimeSignals`-/`runtimeErrors`-fixture. |
...
| Kørte kommandoer, miljø og artefaktlink | `bootstrapUnsupportedDeviceSideEffects.test.tsx` samt de eksisterende shared-tests bestod med 3 filer / 9 tests. `npm run test:e2e -- e2e/pwa-file-open.spec.ts e2e/unsupported-device-hard-stop.spec.ts` bestod med 2/2; de øvrige isolerede E2E-kørsler for shell/404, smoke, minimumsviewporter, PWA, Firefox-fallback og MinProcesrente bestod som angivet ovenfor på Node `v24.18.0`/Windows. `npm run typecheck:e2e` og `e2eSuiteConventions.test.ts` bestod efter ændringen. |
| Fund-ID'er, beslutninger og opfølgning | `TD-021` er lukket for både bootstrap-sideeffekter og synligt browser-hard-stop. `TD-022` dækker fortsat manglende end-to-end lazy/PWA-recovery; native `launchQueue` kan ikke fuldt reproduceres. Test-only styrkelse kan tilføjes; ændringer af gate-, recovery- eller PWA-adfærd forelægges udvikleren. |
| Reviewer / dato / slutstatus | Codex / 2026-09-10 / `I gang` |
...
| `SHELL-001` / `SHELL-002` | Auth, bootstrap, browsergate, PWA og recovery | Gate-sideeffekter og mobil-hard-stop er nu dækket, men manglende lazy-chunk recovery mangler | Styrk kun observerbare browser-/recoveryforløb | `bootstrapUnsupportedDeviceSideEffects.test.tsx` og `unsupported-device-hard-stop.spec.ts` dækker sideeffekter og synligt hard-stop; shell-/PWA-E2E er grønne; `TD-022` står åbent | Delvist – `TD-022` |
...
- [x] `npm run test:coverage` er kørt som del af `npm run verify:release:core` på `83abfdad` og rapporteret med instrumenteret filmængde.
...
| `CALC-006` | EO-snapshot, rækkeevaluering og inspektion | Uafhængige række-/periodefacitter og separat inspection-oracle | Rækkeprioritet, svie/smerte, TAF, ferie, overlap og skudår | EO-/inspection-orakler: 8/8; række-/periodefacitter er grønne. Weekend-TAF reproducerer `control:sammentaelling_mismatch` | Delvist – `TD-016` kræver udviklerens beslutning |
...
| TD-018 | 2026-09-11 | `CALC-007` | Høj | EET har bred unit-/snapshot-/gate-dækning, men ingen generel kontrol af engine → snapshot → UI → dokumentindhold. Differencekravsfanen har fortsat ikke en asymmetrisk dokumentrejse, og PDF-paritet mangler. | `eetIndependentOracle.test.ts` bevarer den samlede håndfacitprøve. `eetPageAudit.spec.ts` downloader nu Word fra hovedappen og kræver løbende ydelsers titel, afgørelsesdata, beløb og faktisk `.docx`-indhold samt kapitaliseringens synlige afgørelse og samme beregnede kapitalbeløb i `word/document.xml`. Den samlede spec bestod med 6/6 på basisbanen efter commit `83abfdad`; den fulde valgte E2E-suite bestod med 180/180. | Delvist lukket for EET-Word-download og observerbart dokumentindhold i to outputvarianter. Fuld engine → snapshot → UI → dokumentkæde for alle relevante faner og PDF↔Word-paritet er åben. | Ikke påkrævet for testændringen; ingen produktkode er ændret. | Delvist retestet – `eetPageAudit.spec.ts` 6/6 på `83abfdad`, samlet fund åbent |
...
| TD-020 | 2026-09-11 | `DATA-001` / `CALC-001` | Mellem | Data-/sats-suiterne har stærke katalog-, fingerprint- og integritetsguards, men flere facitassertions læser produktionsdata som forventning. Validatorpartitioner og komplet downstream-paritet er derfor ikke uafhængigt bevist. | `uafhaengigeDataEndepunkter.test.ts` tilføjer 25 statiske endpoint-/partitionfacitter med historiske og nyere grænser. `td020FolkepensionDownstream.test.ts` tilføjer 2/2 statiske folkepensionsfacitter for fem perioder og manglende dækning. Den uafhængige satsmatrix og downstream-kæde består fortsat med den samlede data-/satssuite på 23 filer / 477 tests. Sygedagpenge-2005-afvigelsen er fortsat uafklaret. | Delvist lukket for de registrerede statiske endpoints og den konkrete folkepensionsforbruger. Åbent for validatorpartitioner og komplet konsumentparitet; runtime-data/validatorer ændres ikke som del af auditten. | Ikke påkrævet for test-only styrkelse. | Delvist lukket – endpoint- og folkepensionsfacitter tilføjet, øvrige downstream-huller åbne |
...
| TD-022 | 2026-09-11 | `SHELL-002` | Mellem | Lazy/PWA-recovery er kun bevist med jsdom/mocks, ikke som faktisk manglende chunk → synlig recovery → kritisk reload. Native `launchQueue` kan ikke fuldt fremkaldes i Playwright. | `pwa-file-open.spec.ts` bruger nu fælles `runtimeSignals`-/`runtimeErrors`-fixture. PWA-filåbning, service worker og installation bestod fortsat isoleret. Den faktiske manglende lazy chunk → recovery-rejse er ikke reproduceret, fordi den kontrollerede E2E-harness ikke kan fremkalde en releaseidentisk chunkfejl stabilt. | Åbent. Dokumentér eller tilføj en reproducerbar recovery-E2E, hvis en releaseidentisk manglende asset kan fremkaldes uden at ændre produktet. Native `launchQueue` forbliver en platformbegrænsning. | Ikke påkrævet for test-only styrkelse. | Delvist retestet – fixturebrug korrigeret, lazy recovery åben |
| TD-023 | 2026-09-10 | `MIN-001` | Mellem | Standalone MinProcesrente havde isolation-, reset-, fokus- og afvist-dato-bevis, men ikke en samlet valid input → beregning → gate → PDF-download-rejse eller browserbaseret namespace-/runtime-isolation. | `minprocesrente-valid-download.spec.ts` bestod med 1/1 og kontrollerer gyldig beregning, aktive gates, faktisk PDF-download og afvist navigation via `beforeunload`; `minprocesrente-recovery-and-focus.spec.ts` bestod med 1/1. `minprocesrente-namespace-isolation.spec.ts` navigerer med almindeligt input mellem standalone og hovedapp og bestod med 1/1; den fulde valgte E2E-suite bestod med 180/180. | Lukket for den registrerede observerbare brugerrejse og browserbaseret namespace-/runtime-isolation. Fuld outputparitet og mutation er fortsat almindelige auditrestpunkter. Eventuelle ændringer af procesrente, gate eller dokumentadfærd forelægges udvikleren. | Ikke påkrævet for test-only styrkelse. | Lukket for TD-023's registrerede isolation- og brugerrejse efter 1/1 målrettet samt 180/180 fuld E2E |
