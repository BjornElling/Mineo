...
- Evidensrevision: `b908c540` for seneste samlede releasegate og målrettede test-only batch; seneste fulde E2E er `71746917`
- Branch: `main`
...
- Seneste test-only batch: DATA-/DOC-facit TD-284; målrettet kontrol 1 ny test / 1 test; ingen produktkode, beregningslogik, UI/UX eller persistens ændret. TD-025 blev retestet serielt 10 gange med 90/90 grønne tests uden reproduktion, men den parallelle årsag er fortsat uforklaret.
...
| Aktuel fuld Vitest-retest | `npm run test:coverage` | Bestået | 911 filer, 9.086 tests bestået og ingen forventede `it.fails` på `b908c540` |
...
| Aktuel samlet release-gate | `npm run verify:release:core` | Bestået | Dependency-, runtime-, type-, lint-, data-, kontrakt-, lane-, ledger-, coverage- og begge build-gates bestået på `b908c540`; coverage 911 filer / 9.086 beståede tests / ingen forventede `it.fails` |
...
Aktuel inventarrevision på `b908c540`: `PERSIST-002` omfatter nu også den nye silent-read-,
...
| Senest tilføjet lukket | `TD-153`, `TD-154`, `TD-155`, `TD-157`–`TD-284` – test-only facitter dokumenteret til og med `b908c540`; `TD-156` er fortsat åbent |

## Aktuel evidensdelta – revision `b908c540`

Den seneste batch tilføjer ét disjunkt test-only facit uden produktændringer:

- `TD-284`: `td020Kapitalisering2025TabelADocumentIndependent.test.ts` fastholder med 1/1
  2025-tabel A, pensionsalder, alder i måneder, faktor og de tilsvarende Word-labels.

Den målrettede kontrol bestod med 1/1 ny test. Typechecks, lint, encoding-, filnavns- og
diff-kontroller bestod. `verify:release:core` bestod derefter med 911 testfiler / 9.086 tests,
...
fulde E2E-suite kræver ikke ny kørsel, fordi ændringen kun vedrører testkode. Carson retestede
desuden den kendte TD-025-risiko serielt 10 gange med 90/90 grønne tests uden reproduktion;
den parallelle coverage-/Tooltip-årsag er fortsat ikke forklaret, så TD-025 forbliver delvist
