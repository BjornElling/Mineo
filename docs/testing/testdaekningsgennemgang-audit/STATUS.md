...
- Evidensrevision: `2b4baf1a` for seneste samlede releasegate og quality-batch; E2E-testrevisionen er `7b8f01ef`
- Branch: `main`
...
- Seneste test-only batch: ARCH-002-facittet TD-304; tre nye bracket-facitter og 4/4 grønne cases i den målrettede fil, 195/195 architecture-tests samlet; ingen produktkode, workflow, beregningslogik, UI/UX eller persistens ændret. Den efterfølgende samlede releasegate på `2b4baf1a` bestod med 920 testfiler / 9.102 tests og begge builds.
- Seneste read-only restpunktstria: TD-004 er genkontrolleret med grøn advisory-gate, men står fortsat som vedligeholdelsespost, indtil parentens `qs`-range åbnes; de 11 faktiske historiske `.eo`-filer under TD-001 er indlæst read-only, hvoraf 10 gik direkte igennem og 1 gik gennem den eksisterende preflight. Den ene fil stammer fra en intern udviklingsudgave, og udvikleren har besluttet, at der ikke indføres yderligere kompatibilitet for netop denne historiske struktur. TD-292 er nu lukket for det besluttede standalone-artifactværn: alle manifestets `file`-, `imports`- og `dynamicImports`-referencer kontrolleres. B-002 er lukket for den konkrete CI-/artefaktkobling efter ekstern workflowkørsel #284 på `ed6bd4e`. De øvrige punkter kræver henholdsvis installeret PWA/OS, fysisk PDF-/Word-viewerinspektion eller autoritativt tabelkildefacit.
...
- Seneste samlede retest: den eksisterende releasegate på revision `2b4baf1a` er fortsat grøn med 920 testfiler og 9.102 beståede tests uden tilbageværende forventede `it.fails`; coverage er 90,74 / 82,00 / 94,88 / 93,54, og begge builds bestod. Den efterfølgende implementeringsbatch er målrettet retestet med 4/4 Vitest-filer og 13/13 tests, 2/2 validatorfiler og 91/91 tests, begge produktionsbuilds samt TD-088-browserrejsen med 1/1. TD-001, TD-003, TD-016, TD-017, TD-043, TD-088, TD-156 og TD-292 er nu lukket for de besluttede adfærdspunkter; åbne produkt-/proveniens-/outputparitetsfund, TD-014/TD-018, TD-022's resterende installerede-PWA-bevis og TD-025-stabilitetsrisikoen forhindrer fortsat afslutning.

## Seneste implementeringsbatch efter udviklerens svar (2026-09-15)

- TD-016 bruger nu EO-motorens kanoniske fald-tilbage-dage i kontroltabellen. En offentlig ydelse, der kun dækker en weekend eller helligdage i en arbejdsdagsbaseret sag, vises derfor i ydelseskolonnen, summerer korrekt og udløser ikke længere et falsk kontrolbrud. Beregningen ændres ikke – kontroltabellen følger den allerede besluttede beregningsfordeling.
- TD-156 samler fejlbeskeder, når de i det væsentlige beskriver den samme manglende reguleringssats. En fejl af en anden type, fx en dato før den tidligste tilladte satsperiode, forbliver særskilt.
- TD-292 kontrollerer nu alle standalone-manifestentries, deres `file`-filer samt alle `imports`- og `dynamicImports`-referencer. Kvalitetstests dækker både grønne referencer, manglende filer, ukendte entries og fortsat afvisning af Mineo-entryen.
- TD-088's E2E-test kræver nu, at **Luk** skjuler både fejlrapport-previewet og den underliggende preflight-dialog.
- TD-043's afrundingsbeslutning er fastholdt: viste afrundede beløb bruges i efterfølgende totaler på skærm, PDF og Word. Den eksisterende KL-lønaftale-undtagelse er bevaret, hvor den trinvist afrundede, regulerede enhedsløn sikrer, at den brugerviste løn ikke forskydes af en afrundet mellemprocent.
- TD-001's 11 brugerleverede filer blev kun læst gennem den almindelige Hent-rejse. Ingen fil blev skrevet, flyttet eller slettet. Den ene eksisterende preflight blev godkendt eksplicit; der blev ikke registreret page-fejl eller console-fejl.
- TD-001-opfølgningen krævede ingen produktændring: filen stammer fra en intern udviklingsudgave, som ikke skal være kompatibilitetsmål for offentliggjorte brugere.
- Den fulde Vitest-kørsel nåede 919/920 grønne testfiler og 9.133/9.134 tests; den ene fejl var en eksisterende forventningsassertion for TD-156's tidligere to fejlbeskeder. Efter opdateringen bestod de berørte validatorer med 2/2 filer og 91/91 tests. Den målrettede ændringssuite bestod med 4/4 filer og 13/13 tests, begge builds bestod, og TD-088-browserrejsen bestod med 1/1.
...
| Aktuel fuld Vitest-retest | `npm run test:coverage` | Bestået | 920 filer, 9.102 tests bestået og ingen forventede `it.fails` på `2b4baf1a` |
...
| Aktuel samlet release-gate | `npm run verify:release:core` | Bestået | Dependency-, runtime-, type-, lint-, data-, kontrakt-, lane-, ledger-, coverage- og begge build-gates bestået på `2b4baf1a`; coverage 920 filer / 9.102 beståede tests / ingen forventede `it.fails` |
...
- `verify-build-artifacts.mjs` kontrollerer nu, at hver PWA-assetsti faktisk findes i `outDir`; den
  ...
  jobtrin; E2E-matrixen er nu også koblet til det uploadede artefakt. GitHub Actions-kørsel #284 bestod med
  releasegate, alle 10 E2E-jobs, artefakt og deploy på `ed6bd4e`; B-002 er lukket for den konkrete kobling.
...
| --- | --- | --- | --- | --- |
| `INPUT-001` | Kontrakt- og testinventar gennemgået; målrettet suite kørt separat; kontrolleret no-op-modprøve og uafhængig referencekontrol udført | Baseline 5 filer / 176 tests bestået; svækket no-op-gate gav 51 fejl / 146 tests; gendannet kontrol 2 filer / 103 tests bestået; separat referencekontrol 1/1 test bestået; `immediateClearHistoryReference.test.ts` tilføjer et uafhængigt rejected → immediate clear → undo → redo-facit med atomisk sletning, ét history-trin, ordret råtekst, nabofelt og field-origin; se hoveddokumentets detaljerække | Kvalificeret mutationsrunner, fuld testkvalitetsrevision og browser-/adapterparitet | `I gang` |
...
| `PERSIST-002` | Storage/settings, filhåndtag, PWA-filflows og reel session-reload gennemgået | 44 filer / 209 målrettede unit-/hook-tests; PWA-/filkørsel 4/4; reel input + aktiv fane + reload 1/1; supplerende 8-filers platformssuite med 89/89 beståede tests efter callable picker-detektion; `safeLocalStorage.test.ts` bestod med 11/11 direkte Node-fallback-isolationsassertions; `fileHandleVerification.test.ts` bestod med 22/22; `fileHandleKvStore.test.ts` bestod med 2/2; den efterfølgende test-only batch tilføjer 9/9 uafhængige silent-read-, standardmappe-, permission- og File System Access-facitter; ugyldige `Date`-inputs har 8/8 grønne negative modcases | Fuld browser-/platformssammenhæng, herunder installeret PWA-filaflevering; `TD-022` | `I gang` |
| `INPUT-002` / `VALID-001` | Form/grid-overflader, produktions-ChoiceField og schema-/validatorhuller gennemgået | 69 filer / 1.529 tests; produktionsbundet form/grid-paritet, 12 nested row-schemas, faktiske `ZodError.issues`, 11 uafhængige validator-literaltests, 13 håndskrevne typed runtime-cases inklusive negativ fritvalg-procent, et obligatorisk top-level schema-facit, direkte Beregningsperiode-lønreguleringsfacit samt selvstændige KRL-, offentlig-løn-, overenskomst-, manuel-løn-, manuel-procentsats- og SFGG-facitter er dækket. `td019TafLoseFeriedageValidatorIndependent.test.ts` tilføjer et uafhængigt TAF-facit for for mange løse feriedage, `td019ForligAnsvarsgradBroekValidatorIndependent.test.ts` dækker nu både en forligsbrøk over 1, nul som tæller og samtidig udfyldt procent/brøk med præcise stier, beskeder og severity, `td019TafBeregningsperiodeDatoordenValidatorIndependent.test.ts` dækker omvendt TAF-beregningsperiode med præcis feltsti, besked og severity, `td019SfggReferenceperiodeOrderValidatorIndependent.test.ts` dækker en weekend-only SFGG-referenceperiode med præcis issue-evidens, og `td019TafDatoordenValidatorIndependent.test.ts` dækker omvendt TAF-datoorden med præcis issue-evidens. `td019SvieSmerteMenafoerelseCutoffValidatorIndependent.test.ts` dækker desuden cutoff ved ménafgørelsesdatoen med præcis issue-evidens, og `td019TafDagsloenValidatorIndependent.test.ts` kræver med 1/1 præcis afvisning af manglende dagsløn ved Angivet dagsløn. `fieldShells.test.tsx` fastholder desuden med 1/1, at direkte grid-input med tredje decimal afvises uden ændring af canonical værdi, revision eller rejected-input-map; de nye `td019TidligereModtagetTafCanonicalValidatorIndependent.test.ts`- og `rowId.test.ts`-facitter tilføjer henholdsvis 1/1 negativ TAF-beløbsgrænse og 3/3 crypto-/fallbackfacitter for rækkeidentitet | Yderligere validator-fixture-uafhængighed og downstream-partitioner; `TD-019` | `I gang` |
| `DATA-001` | Data-/satskatalog, integritet og udvalgte uafhængige endpointfacit gennemgået | Den målrettede data-/sats-/endpoint-suite havde baseline på 34 filer / 532 tests og er suppleret med uafhængige downstream-facitter for RLTN-komponent, Læreroverenskomst, Læreroverenskomst-TAF, ledighedsydelse, uddannelseshjælp, revalideringsydelse, ressourceforløbsydelse og flextilskud gennem kalenderdagsconsumeren samt ILON12 gennem det faktiske reguleringsdokument. Dertil kommer literal-facit for udvalgte perioder, 25 statiske endpoint-/partitionfacitter, 2/2 statiske folkepensions-downstreamfacitter, 2/2 statiske pensionsalder-downstreamfacitter, 1/1 statisk forsørgertabs-downstreamfacit, 1/1 KRL-downstreamfacit, 1/1 Statistik/ILON12-downstreamfacit og 16/16 offentlig KL-løn-tests med konkret komponent-/dagstotalfacit samt sats → beregning → dokument-kæde. `loenudviklingBeregning.test.ts` har desuden et statisk SBLON2-downstream-facit med 48/48 beståede tests, procesrente-downstreamfacittet bestod 1/1, og sygedagpengebanen har statiske facitter for 2025 samt 2019/2020-satsgrænsen; samme fil har desuden et RLTN-facit for 2024-04-01–2024-10-01 og et svie/smerte-facit for 2024 med 230 kr. fuld og 115 kr. delvis sats. `td020UddannelseshjaelpDownstreamIndependent.test.ts` fastholder et håndskrevet 2024-facit på 1.500 kr. fordelt over fire kalenderdage med 375 kr. pr. dag, `td020RevalideringsydelseDownstreamIndependent.test.ts` fastholder et håndskrevet skudårs-/weekendfacit på 2.000 kr. fordelt over fire kalenderdage med 500 kr. pr. dag, `td020RessourceforloebsydelseDownstreamIndependent.test.ts` fastholder et håndskrevet månedsgrænse-/weekendfacit på 2.800 kr. fordelt over fire kalenderdage med 700 kr. pr. dag, og `td020FlextilskudDownstreamIndependent.test.ts` fastholder et håndskrevet weekendfacit på 900 kr. fordelt over to kalenderdage med 450 kr. pr. dag | Resterende kilder, validatorpartitioner og komplet downstream-paritet; `TD-020` | `I gang` |
...
| `ARCH-002` | Registry-completeness, rule-factory-liveness og udvalgte negative modcases tilføjet til arkitekturharnessets dedikerede tests | `architectureRules.test.ts`: 1 fil / 192 tests bestået; separat forventningsliste med 88 regel-ID'er, AST-opslag af eksporterede definitions/aggregater samt re-export-, liveness- og namespace-import-modcases. `focusTraversalNamespaceImportBlindspot.test.ts` kræver 1/1 finding for namespace-import, og `storageBoundaryGlobalThisNegative.test.ts` kræver 1/1 finding for rå `globalThis.localStorage`; `TD-015` er lukket for de konkrete huller | De enkelte reglers negative modcases, importgrænser og øvrige livenessværn mangler | `I gang` |
| `DATE-001` | Pengefladen er mutationstestet modulvist med den kvalificerede command-runner; datoassertions er styrket | 58 money-mutationer: 56 dræbt, 2 triageret som ækvivalente; `fromKroner`-mutationen ved `> 1e-4` → `>= 1e-4` er konkret triageret som matematisk ækvivalent, fordi grænsen ikke kan rammes eksakt af et gyldigt repræsenterbart ikke-nul heltalsbeløb med den krævede decimalopløsning. 13 money-tests grønne. Dato-/SH-suiten: 6 filer / 109 tests grønne efter præcise grænseassertions, eksakt 2024-facit, håndberegnet skudårsinterval og uafhængigt ISO-ugefacit for årsskifte og uge 53; ugyldig `Date` afvises nu i `utcDayMath`; `periodeBeregningKalendermaanederIndependentOracle.test.ts` fastholder fire unikke hele måneder ved årsskifte og overlap, `periodeBeregningKalendermaanederAfvisningIndependentOracle.test.ts` afviser 28. februar 2024 som falsk månedsslutning, og `periodeBeregningDagLeapYearIndependentOracle.test.ts` fastholder fire unikke dage ved overlappende 28. februar–2. marts 2024; `coverage/mutation/mutation.json` | Uafhængig håndregning og resten af dato-/periodiseringsfladen mangler. `TD-003` er lukket for den interne ugyldig-dato-invariant. | `I gang` |
...
| `CALC-006` | EO-snapshot, canonical totals, dokumentprojektion, inspektionsdage og sidevisning stikprøvet med fem uafhængige orakler samt række-/periodefacitter | Den målrettede `CALC-006`-kørsel bestod med 171 filer / 2.185 tests, heraf 16 nye uafhængige række-/periodecases. `eoInspektionOffentligeYdelserInvalidRowsIndependent.test.ts` fastholder blank, ukendt og omvendt offentlig ydelsesrække med 1/1; `eoInspektionRegulationKlLoenaftalerIndependent.test.ts` fastholder rå KL-lønaftaleentries med 1/1; `eoInspektionRegulationKlCanonicalIndependent.test.ts` fastholder canonical KL-segmenter med 1/1; `eoInspektionSammentaellingMissingAbsenceIndependent.test.ts` fastholder manglende øvrige fraværsdage med 1/1; de fem EO-/inspection-orakler består samlet af 13 tests. De to nye EO-løntidsfacitter kræver henholdsvis ingen svie/smerte-dag ved manglende valgt satsår og ingen offentlig løndag før første gældende regulering. Det nye ugeperiode-facit fordeler 900 kr. i Beløb-tilstand over fem hverdage med 180 kr. pr. dag og ingen integrity issues. Midlertidig EET-konsistens er nu prøvet i begge retninger med konkrete advarselsrækker, og `eoRowTafEndeligEetIndependent.test.ts` fastholder et uafhængigt facit for endelig EET som TAF-ophørsårsag. `td020RevalideringsydelseDownstreamIndependent.test.ts` fører et håndskrevet revalideringsydelsesfacit over skuddag og weekend gennem den faktiske kalenderdagsconsumer med 1/1, `td020RessourceforloebsydelseDownstreamIndependent.test.ts` fører et håndskrevet ressourceforløbsydelsesfacit over månedsgrænsen og weekend med 1/1, `td020FlextilskudDownstreamIndependent.test.ts` fører et håndskrevet flextilskudsfacit over en ren weekend med 1/1, og `td020FoertidspensionDownstreamIndependent.test.ts` fører et håndskrevet førtidspensionsfacit over årsskifte og nytårsdag med 1/1. En weekendydelse i arbejdsdagsbaseret TAF gav observeret `control:sammentaelling_mismatch`; `TD-016` er åbent | Øvrige rækkegrene, dokumentparitet, mutation og fuld E2E; udviklerens beslutning om TD-016 | `I gang` |
| `DOC-001` | Katalog, definitioner, gate/lifecycle og renderer-wiring gennemgået som evidensbaseline for alle registrerede outputs | Fokuseret fixture-/lifecyclekontrol: fixture-registret 67/67 og 33 dokumentfiler / 285 tests efter faktisk PDF-/Word-artefaktkontrol. `standaloneRenteAlleDocumentDefinition.test.ts` tilføjer 1/1 semantisk renderer-facit, `satserDocumentRendererWiringIndependent.test.ts` tilføjer 1/1 typed Satser-rendererfacit, `aarsloenDocumentIndependent.test.ts` tilføjer 1/1 uafhængigt årslønfacit gennem dokumentgeneratoren, `aarsloenDocumentPdfWordArtifactIndependent.test.ts` og `aarsloenDocumentMethodAIndependent.test.ts` tilføjer to faktiske Årsløn-PDF/Word-facitter, og `eetDocumentDefinitionIndependent.test.ts` består med 2/2 EET-definition-facitter, hvor efter-EAL-sagen kontrolleres i faktisk PDF og Word. Kataloget dækker 18 Mineo- og 3 standalone-outputs; TD-012 er lukket og TD-013 retestet med udvidet fasebevis | TD-014: manglende generel fysisk rendering og uafhængig PDF/Word-paritet | `I gang` |
| `SHELL-001` / `SHELL-002` | Auth, routes, desktop-/unsupported-device-gate, 404, PWA, service worker, preload og browsermotorer gennemgået | 26 fokuserede filer / 125 tests; bootstrap-sideeffekter 1/1, synlig mobil-hard-stop 1/1, shell/404 4/4, minimumsviewporter 12/12, PWA-installation 8/8, lazy-chunk recovery 4/4, native LaunchQueue-form 2/2 i Chrome/Edge, manifestets `.eo`-filhandler til `/open` 8/8 i Chrome, øvrige målrettede browserflows grønne; seneste fulde E2E 215/217 på `38df665f` efter fælles helper-konvergens | Fuld OS-/installeret-PWA-filaflevering og øvrig PWA-/platformsparitet; `TD-022` | `I gang` |
| `MIN-001` | Standalone isolation, reset/fokus, error boundary, valid beregning, PDF, exit-guard og browserbaseret namespace-isolation gennemgået | `minprocesrente-valid-download.spec.ts`: 1/1, `minprocesrente-namespace-isolation.spec.ts`: 1/1 samt 215/217 i seneste fulde E2E på `38df665f` | Fuld outputparitet og mutation | `I gang` |
| `BUILD-001` | Asset-eksistenskontrol, release-graf og PWA-manifestets faktiske filudvalg gennemgået | Syntetisk `verifyBuildArtifacts.test.ts`: 2/2; `releaseGateBuildLiveness.test.ts`: 2/2 med positiv/negativ scriptgraf; begge produktionsbuilds bestået; E2E-matrixen downloader det uploadede artefakt; lokal E2E på 10 projektbaner bestod med 215/217; statisk artefakt-flow-værn, lokal preview-smoke og ekstern workflowkørsel #284 bestod | Chunk-/Vite-advarsler og øvrige builddetaljer står fortsat åbne; B-002 er lukket for den konkrete CI-/artefaktkobling | `I gang` |
| `ARCH-003` | Lane-tag-vagten parser nu syntaksbevidst tags i E2E-specs | `e2eSuiteConventions.test.ts`: 20/20 bestået; `check:e2e-lanes`: 2 gyldige tags; `githubActionsRuntimeCheck.test.ts`: 8/8 med statisk CI-artefaktkobling, deployets push-til-main-gate og de fire dedikerede browserprojekter; `githubActionsReleaseStepLiveness.test.ts`: 2/2 med aktiv `verify:release:core`-step og negativt kommentarfacit; TD-006 lukket; ekstern workflowkørsel #284 bestod med 10/10 E2E-jobs | Øvrige release-/CI-værn og semantisk arkitekturgennemgang mangler | `I gang` |
...
Aktuel inventarrevision på `f7c4edd5`: `PERSIST-002` omfatter nu også den nye silent-read-,
standardmappe-, permission- og File System Access-batch med 9/9 målrettede facitter, og
`PERSIST-001` omfatter `fileHelpersSelectFileSuccessIndependent.test.ts` med 1/1 input-
fallbackfacit. `VALID-001` omfatter desuden `td019EoAngivetLoenFeriePctRangeValidatorIndependent.test.ts`
med 1/1 typed range-facit samt `td019EoAngivetLoenFeriePctUpperRangeValidatorIndependent.test.ts`
med 1/1 øvre typed range-facit. `CALC-006` omfatter
`eoInspektionSammentaellingMissingAbsenceIndependent.test.ts` med 1/1 facit for manglende øvrige fraværsdage.
De tilsvarende målrettede inventartal er dermed 44 filer / 209 tests for PERSIST-002 og 171 filer /
2.185 tests for CALC-006; række-/periodefacitterne er 16/16.
`ARCH-002` er samtidig udvidet med `storageBoundaryGlobalThisNegative.test.ts` til 192 quality-tests,
og `DOC-001` med `satserDocumentRendererWiringIndependent.test.ts` til 33 dokumentfiler / 285 tests.
`VALID-001` er samtidig udvidet med `td019StoreBededagPctRangeValidatorIndependent.test.ts` med 1/1
negativ `storeBededagPct`-facit, og `DATA-001`/`TD-020` med
`td020Kapitalisering2026DocumentIndependent.test.ts` med 1/1 2026-tabel-A- og Word-facit.
`ARCH-002` er desuden udvidet med `reguleringDefaultImportBlindspot.test.ts` med 1/1
default-import-facit samt `reguleringSideEffectImportBlindspot.test.ts` med 1/1
side-effect-import-facit, og `DOC-001` med `documentDynamicImportFailureIndependent.test.ts`
med 1/1 renderer-load/dynamic-import-facit. `DOC-001`/`DOC-003` omfatter desuden
`documentResolvedRendererForwardingIndependent.test.ts` med 1/1 typed renderer-forwarding-
og artifact-facit. `VALID-001` omfatter nu også
`td019SvieSmerteAktuelPeriodeCanonicalValidatorIndependent.test.ts` med 1/1 independent
canonical-beløbsfacit for den aktuelle svie/smerte-periode.
`VALID-001` omfatter desuden `td019DagsloenCanonicalAmountValidatorIndependent.test.ts`
med 1/1 independent canonical-beløbsfacit for `dagsloenenUdgoer`, og `BUILD-001`/`SHELL-001`
omfatter `verifyMinprocesrenteBuildArtifacts.test.ts` med 1/1 standalone-entry-facit.
`INPUT-001` omfatter desuden `settleNoOpAfterUndoReference.test.ts` med 1/1 typed referencefacit
for gentaget canonical no-op-settle efter undo, og `ARCH-003`/`BUILD-001`/`SHELL-001` omfatter
`deployAssetDirectoryLiveness.test.ts` med 1/1 statisk Vite-til-Wrangler asset-directory-facit.
`VALID-001` omfatter desuden `td019MaanedsloenCanonicalAmountValidatorIndependent.test.ts`
med 1/1 independent canonical-beløbsfacit for `maanedsloenenUdgoer`, og `ARCH-003`/`SHELL-001`
omfatter `e2eServerIdentityLiveness.test.ts` med 1/1 fælles E2E-serveridentitets- og
kørselsrækkefølgefacit.
`PERSIST-002` omfatter desuden `fileSystemAccessOpenGetFileFailureIndependent.test.ts` med 1/1
facit for picker-success efterfulgt af `getFile()`-fejl, og `DOC-001`/`DOC-003` omfatter
`documentSourceContextUndefinedMemoIndependent.test.ts` med 1/1 `undefined`-memo-facit.
`DATA-001`/`TD-020`/`DOC-001`/`DOC-003` omfatter desuden
`td020Kapitalisering2025TabelADocumentIndependent.test.ts` med 1/1 uafhængigt tabel-A- og
Word-facit for kapitaliseringsdato 31.12.2025.
`DATA-001`/`TD-020`/`DOC-001`/`DOC-003` omfatter desuden
`td020Kapitalisering2025TabelBDocumentIndependent.test.ts` med 1/1 uafhængigt tabel-B- og
Word-facit for kapitaliseringsdato 01.01.2025.
`DOC-001`/`DOC-003`/`TD-014`/`TD-018` omfatter desuden
`shDageDocumentPdfWordArtifactIndependent.test.ts` med 1/1 uafhængigt PDF-/Word-artefaktfacit
for SH-dage-perioden 28.03–01.04.2024, og `VALID-001` omfatter
`td019TafLoseFeriedageLowerRangeValidatorIndependent.test.ts` med 1/1 typed facit for negativt
antal løse feriedage.
`DATA-001`/`TD-020`/`DOC-001`/`DOC-003` omfatter desuden
`td020Kapitalisering2026TabelBDocumentIndependent.test.ts` med 1/1 uafhængigt tabel-B- og
Word-facit for kapitaliseringsdato 01.01.2026, og `VALID-001` omfatter
`td019OevrigeFravaersdageLowerRangeValidatorIndependent.test.ts` med 1/1 typed facit for
negativt antal øvrige fraværsdage.
`INPUT-001` er desuden udvidet med `resetSectionHistoryIndependent.test.ts` med 1/1 facit for
...
`DATA-001`/`TD-020` er desuden udvidet med `td020SatserDiverseReguleringssatsDocumentIndependent.test.ts`
med 1/1 håndskrevet Word-facit, og `PERSIST-002` med
`fileOperationClientSessionInvalidStorageIndependent.test.ts` med 1/1 invalid-session/reload-facit.
...
| Åbent – kræver udviklerbeslutning eller ekstern evidens | `TD-001`, `TD-016`, `TD-088`, `TD-156`, `TD-292` |
| Senest tilføjet lukket | `TD-153`, `TD-154`, `TD-155`, `TD-157`–`TD-304` – test-only facitter dokumenteret; `TD-156` er fortsat åbent |

## Seneste test-only batch – TD-295

UI-006/CALC-002-facittet tilføjer `ui-006-aarsloen-periodetype-roundtrip.spec.ts`.
Chrome-basisbanen gennemfører med 1/1 login, åbning af Årslønsberegning, indtastning af
måned 1, år 2025 og 30.000 kr., skift til Dato med 01-01-2025–31-01-2025 og tilbage til
Måned. Måned, år, beløb og rækkevisning bevares, og `runtimeErrors` samt
`externalRequests` er tomme. Målrettet E2E, E2E-typecheck, lint, encoding-, filnavns- og
diff-kontroller bestod. Der blev ikke ændret produktkode, beregningslogik, UI/UX eller
persistens, og den samlede releasegate blev ikke gentaget, fordi ændringen er test-only.
TD-295 er lukket for den konkrete periodeskift-partition; øvrig UI-006/CALC-002-dækning
er fortsat i gang.

## Seneste test-only batch – TD-296

UI-008-facittet udvider `ui-008-satser-browserrejse.spec.ts` med et browserforløb for
det laveste understøttede satsår 2005. Testen fastholder `Arbejdsskadesatser 2005`,
minimum årsløn `145.000 kr.` og reguleringsprocent `5,5 %` samt tomme `runtimeErrors`
og `externalRequests`. Den målrettede UI-008-E2E-kørsel bestod med 3/3, herunder de
eksisterende 2020- og 2024-forløb; E2E-typecheck og målrettet lint bestod også.
Der blev ikke ændret produktkode, beregningslogik, UI/UX eller persistens, og den
samlede releasegate blev ikke gentaget, fordi ændringen er test-only. TD-296 er lukket
for den konkrete nedre satsårs-partition; øvrig UI-008-dækning er fortsat i gang.

## Seneste test-only batch – TD-297

CALC-006/DOC-001-facittet tilføjer `erstatningsopgoerelseDocumentDefinitionIndependent.test.ts`.
En håndskrevet typed sag med én øvrig kravrække på 1.200 kr. føres med 1/1 gennem den
faktiske `erstatningsopgoerelseDocumentDefinition.project()` og definitionens
`loadRenderer()` til Word-OOXML. Testen fastholder samlet total, dato, tekst, beløb,
bilagsvalg og filnavn. Den målrettede test, test-typecheck og lint bestod efter korrektion
af den faktiske dokumentformatterings `1.200,00 kr.`. Der blev ikke ændret produktkode,
...
## Seneste test-only batch – TD-299

MIN-001/BUILD-001-facittet styrker `minprocesrente-valid-download.spec.ts` med en
assertion på `externalRequests`. Den eksisterende gyldige standalone-rejse med
beregning, PDF-download og exit-guard bestod med 1/1, og request-listen var tom.
E2E-typecheck og målrettet lint bestod. Der blev ikke ændret produktkode,
beregningslogik, UI/UX eller persistens, og den samlede releasegate blev ikke gentaget,
fordi ændringen er test-only. TD-299 er lukket for den konkrete standalone-PDF → ingen
eksterne requests-partition; øvrig MIN-001/BUILD-001-dækning er fortsat i gang.

## Seneste test-only batch – TD-300–TD-302

UI-005-facittet udvider `ui-005-forsoergertab-word-browserrejse.spec.ts` med en monteret
fejl → blokeret dokumentgate → rettelse → aktiv gate-overgang. `Tilkendt for periode` sættes
til 11 år, rød feltfejl og disabled download kræves, hvorefter 10 år genindsættes og aktiv
download samt bevaret `82.741 kr.` kræves. Filens samlede Chrome-basisbane bestod med 2/2,
uden runtimefejl eller eksterne requests.

DOC-003-facittet udvider `kapitaliseringWordContent.test.ts` med særfaktorgrenen ved højst
to år til folkepension. Faktisk Word-tekst fastholder `Kapitaliseret pga. ≤ 2 år til
folkepension? Ja` og `Særfaktor (≤ 2 år til folkepension) 1,245` samt fravær af de
modsatrettede faktorlabels med 1/1. VALID-001-facittet tilføjer
`td019AngivetDagsloenKrlDaekningValidatorIndependent.test.ts` med 1/1 præcis KRL-
dækningsissue for Angivet dagsløn efter 30-09-2026. Målrettede unit-/validator-tests,
typechecks, lint, encoding-, filnavns- og diff-kontroller bestod. Der blev ikke ændret
produktkode, beregningslogik, UI/UX eller persistens; den samlede releasegate er endnu ikke
gentaget efter denne batch.

## Seneste test-only batch – TD-303

BUILD-001-facittet udvider `verifyBuildArtifacts.test.ts` med en negativ syntetisk
`index.html`-case, der peger på `/src/main.tsx` i stedet for et bygget asset. Testen kræver
exitstatus 1 og den præcise fejl `mineo-buildets index.html peger ikke entydigt på et bygget
asset.`; filens samlede quality-suite bestod med 4/4. Typecheck, lint, encoding-, filnavns- og
diff-kontroller samt pre-commit bestod. Der blev ikke ændret produktkode, workflow,
beregningslogik, UI/UX eller persistens; den samlede releasegate er endnu ikke gentaget efter
denne batch.

## Seneste test-only batch – TD-304
...
facitter og den eksisterende globalThis-case bestod med 4/4 i den målrettede fil, og den
...
runtimefejl, runtime-signaler eller eksterne requests. Kørselen er fortsat seneste fulde E2E,
fordi TD-304 kun ændrede quality-harnesset.

## Seneste samlede releasegate – revision `2b4baf1a`
...
## Seneste samlede releasegate – revision `27cc0751`
...
## Seneste samlede releasegate – revision `78752b56`
...
linjer. Dependency-, runtime-, type-, lint-, data-, kontrakt-, lane-, ledger-, coverage- og
build-gates bestod. Den fulde valgte E2E-suite er fortsat senest grøn på `71746917` med 221
tests og 2 forventede skips ud af 223 på 10 projektbaner; den seneste batch ændrede kun testkode
og dokumentation efter denne E2E-kørsel. Vite- og chunk-advarslerne er kendte ikke-blokerende
buildobservationer. TD-001, TD-016, TD-014, TD-018, TD-022, TD-088, TD-025 og TD-156 forbliver
blokerende restpunkter for auditkonklusionen; B-002 er lukket for den konkrete CI-/artefaktkobling.

## Aktuel evidensdelta – revision `f7c4edd5`

Den seneste batch tilføjer to disjunkte test-only facitter uden produktændringer:

- `TD-288`: `td020Kapitalisering2026TabelBDocumentIndependent.test.ts` fastholder med 1/1
  2026-tabel B, pensionsalder, alder i måneder, faktor og de tilsvarende Word-labels.
- `TD-289`: `td019OevrigeFravaersdageLowerRangeValidatorIndependent.test.ts` fastholder med 1/1
  negativ typed `oevrigeFravaersdage` med præcis feltsti, besked og severity.

Den målrettede kontrol bestod med 2/2 nye tests. Typechecks, lint, encoding-, filnavns- og
diff-kontroller bestod. `verify:release:core` bestod derefter med 916 testfiler / 9.091 tests,
coverage 90,74 / 81,99 / 94,88 / 93,54 og begge builds. Coverage målte 18.705/20.613
statements, 13.204/16.104 branches, 3.063/3.228 funktioner og 17.078/18.257 linjer. Den
fulde E2E-suite kræver ikke ny kørsel, fordi ændringen kun vedrører testkode. Carson kørte
...
## Seneste test-only batch – TD-290

ARCH-003-facittet udvider `githubActionsRuntimeCheck.test.ts` med en syntetisk workflow,
hvor `uses: actions/checkout` mangler `@version`. Testen kræver den præcise fejl om en
entydig action-reference og dækker dermed scriptets eksisterende fejlgrovsortering med 1/1
ny test og 8/8 tests i quality-filen. Typecheck, lint, encoding-, filnavns- og diff-
kontroller bestod. Der blev ikke ændret workflow-, produktkode-, beregnings-, UI/UX- eller
persistensadfærd, og den samlede releasegate blev ikke gentaget, fordi ændringen er
test-only. TD-290 er lukket for den konkrete `uses:` uden `@version`-partition; øvrige
ARCH-003-værn og den semantiske releasegennemgang er fortsat åbne.

## Seneste test-only batch – TD-291

ARCH-003-facittet udvider `githubActionsTriggerLiveness.test.ts` med en syntetisk workflow,
hvor `pull_request` findes, men push-triggeren kun gælder `release` og derfor ikke `main`.
Testen kræver `pushToMain: false` og den samlede quality-fil bestod med 3/3. Typecheck,
lint, encoding-, filnavns- og diff-kontroller bestod. Der blev ikke ændret workflow-,
produktkode-, beregnings-, UI/UX- eller persistensadfærd, og den samlede releasegate blev
ikke gentaget, fordi ændringen er test-only. TD-291 er lukket for den konkrete
pull-request til ikke-main push-trigger-partition; øvrige ARCH-003-værn og den semantiske
releasegennemgang er fortsat åbne.

## Seneste read-only SHELL/BUILD-tria – arbejdsrevision `72b6b969`

Darwin gennemgik standalone-buildets artifact-verifier read-only. `scripts/verify-build-artifacts.mjs`
inspicerer for MinProcesrente kun manifestets nøgler og `src`, standalone-entryen og forbudte
Mineo-kilder. Det aktuelle `dist/minprocesrente/.vite/manifest.json` indeholder også `file`,
`imports` og `dynamicImports`, men `verifyMinprocesrenteBuildArtifacts.test.ts` fastholder
kun entry- og Mineo-entrypartitionerne. En manglende dynamisk chunk kan derfor passere den
isolerede verifier. Dette er registreret som `TD-292`; en rettelse kræver først beslutning
om en udvidet releasekontrakt eller build-koblet artifacttest. Ingen filer blev ændret i
read-only-triagen.

## Seneste test-only batch – TD-293

VALID-001-facittet tilføjer `td019SfggManuelDagssatsCanonicalValidatorIndependent.test.ts`,
som bruger en fuldt håndskrevet typed fixture med `sfggBeregningskilde: 'Manuelt angivet'`
og negativ `sfggManuelDagssats`. Testen kræver præcis nested feltsti, beskeden `Beløb kan
ikke være negativt` og `error`-severity og bestod med 1/1. Typecheck, lint, encoding-,
filnavns- og diff-kontroller bestod. Der blev ikke ændret schema-, validator-, runtime-,
beregnings-, UI/UX- eller persistensadfærd; den samlede releasegate blev ikke gentaget,
fordi ændringen er test-only. TD-293 er lukket for den konkrete negative canonical
manuel-SFGG-dagssats-partition; øvrig validator-fixture-uafhængighed er fortsat åben.

## Seneste test-only batch – TD-294

DOC-001/DOC-003-facittet tilføjer `differencekravDocumentDefinitionIndependent.test.ts`.
En håndskrevet EET-case føres med 1/1 gennem den faktiske
`differencekravDocumentDefinition.project()` og definitionens `loadRenderer()` til Word-
OOXML. Testen fastholder `1.700.470 kr.` før 50 % forlig, `850.235 kr.` efter forlig,
`Differencekrav (EET)` og korrekt filnavn. Typecheck, lint, målrettet test, encoding-,
filnavns- og diff-kontroller bestod. Der blev ikke ændret produktkode, beregningslogik,
UI/UX eller persistens; den samlede releasegate blev ikke gentaget, fordi ændringen er
test-only. TD-294 er lukket for den konkrete definition → renderer-partition; fysisk
PDF-/Word-rendering og fuld kanalparitet er fortsat åbne.

## Tidligere evidensdelta – revision `d5bf5f62`

Den seneste batch tilføjer ét disjunkt test-only facit uden produktændringer:

- `TD-275`: `td019SvieSmerteAktuelPeriodeCanonicalValidatorIndependent.test.ts` fastholder
  med 1/1 negativ canonical-værdi på `svieSmerteAktuelPeriode` med præcis feltsti, besked
  og severity.
...
lint, encoding-, filnavns- og diff-kontroller bestod. `verify:release:core` bestod derefter
med 902 testfiler / 9.077 tests, coverage 90,74 / 81,99 / 94,88 / 93,54 og begge builds.
Den fulde E2E-suite kræver ikke ny kørsel, fordi ændringen kun vedrører testkode. TD-001,
TD-016, TD-014, TD-018, TD-022, B-002, TD-088 og TD-025 forbliver blokerende restpunkter
for auditkonklusionen.

## Tidligere evidensdelta – revision `07b0b340`

Den seneste batch tilføjer to disjunkte test-only facitter uden produktændringer:

- `TD-273`: `reguleringSideEffectImportBlindspot.test.ts` fastholder med 1/1 præcis finding,
  position og besked ved en ulovlig side-effect-import af en reguleringsserie.
- `TD-274`: `documentResolvedRendererForwardingIndependent.test.ts` fastholder med 1/1 den
  typed projektion, `visBrevhoved`-context og det uændrede artifact gennem den lukkede
  dokumentaktion.

Den målrettede kontrol bestod med 2/2 nye tests. Typechecks, lint, encoding-, filnavns- og
diff-kontroller bestod. `verify:release:core` bestod derefter med 901 testfiler / 9.076 tests,
coverage 90,74 / 81,99 / 94,88 / 93,53 og begge builds. Den fulde E2E-suite kræver ikke ny
kørsel, fordi ændringen kun vedrører testkode. TD-001, TD-016, TD-014, TD-018, TD-022,
B-002, TD-088 og TD-025 forbliver blokerende restpunkter for auditkonklusionen.

## Tidligere evidensdelta – revision `d89fd021`

Den seneste batch tilføjer to disjunkte test-only facitter uden produktændringer:

- `TD-271`: `reguleringDefaultImportBlindspot.test.ts` fastholder med 1/1 præcis finding,
  position og besked ved en ulovlig default-import af en reguleringsserie.
- `TD-272`: `documentDynamicImportFailureIndependent.test.ts` fastholder med 1/1 den danske
  dev-server-besked og struktureret `document:dev_server_unavailable`-diagnostik ved
  rendererens dynamic-import-fejl.

Den målrettede kontrol bestod med 2/2 nye tests. Typechecks, lint, encoding-, filnavns- og
diff-kontroller bestod. `verify:release:core` bestod derefter med 899 testfiler / 9.074 tests,
coverage 90,74 / 81,99 / 94,88 / 93,54 og begge builds. Den fulde E2E-suite kræver ikke ny
kørsel, fordi ændringen kun vedrører testkode. TD-001, TD-016, TD-014, TD-018, TD-022,
B-002, TD-088 og TD-025 forbliver blokerende restpunkter for auditkonklusionen.

## Tidligere evidensdelta – revision `7394d80b`

Den tidligere batch tilføjede ét disjunkt test-only facit uden produktændringer:

- `TD-270`: `td019EoAngivetLoenFeriePctUpperRangeValidatorIndependent.test.ts` fastholder med
  1/1 værdien 101 som præcis negativ canonical-rangeissue på
  `eoAngivetLoenLoenudvikling.feriePct` med besked og severity.

Den målrettede kontrol bestod med 1/1 ny test. Typechecks, lint, encoding-, filnavns- og
diff-kontroller bestod. `verify:release:core` bestod derefter med 897 testfiler / 9.072 tests,
coverage 90,74 / 81,99 / 94,88 / 93,53 og begge builds. Den fulde E2E-suite krævede ikke ny
kørsel, fordi ændringen kun vedrørte testkode. TD-001, TD-016, TD-014, TD-018, TD-022,
B-002, TD-088 og TD-025 forblev blokerende restpunkter for auditkonklusionen.

## Tidligere evidensdelta – revision `6bc1269e`

Den seneste batch tilføjer elleve disjunkte test-only facitter uden produktændringer:

- `TD-259`–`TD-263`: fem PERSIST-002-facitter for silent IndexedDB-read, standardmappe-save/load/delete
  og directory permission-request-fejl, alle med 1/1.
- `TD-264`: `fileHelpersSelectFileSuccessIndependent.test.ts` fastholder med 1/1 normal valgt `.eo`-
  fil gennem input-fallbacken og oprydning af input-elementet.
- `TD-265`–`TD-268`: fire PERSIST-002-facitter for file permission-request-, File System Access write-,
  close- og `file.text()`-fejl, alle med 1/1.
- `TD-269`: `td019EoAngivetLoenFeriePctRangeValidatorIndependent.test.ts` fastholder med 1/1 den
  negative EO-angivne ferieprocent med præcis feltsti, besked og severity.

Den målrettede kontrol bestod med 11/11 nye tests. Typechecks, lint, encoding-, filnavns- og
diff-kontroller bestod. `verify:release:core` bestod derefter med 896 testfiler / 9.071 tests,
coverage 90,74 / 81,99 / 94,88 / 93,54 og begge builds. Den fulde E2E-suite kræver ikke ny
kørsel, fordi ændringerne kun vedrører testkode. TD-001, TD-016, TD-014, TD-018, TD-022,
B-002, TD-088 og TD-025 forbliver blokerende restpunkter for auditkonklusionen.

## Tidligere evidensdelta – revision `40cf3cad`

Den seneste batch fra `a9ffd95a` til `40cf3cad` tilføjer seksten disjunkte test-only facitter uden
produktændringer:

- `TD-243`–`TD-249`: seks klientafgrænsede fil-/pending-PWA-facitter og ét crypto-fallbackfacit,
  alle med 1/1 for korrekt klientpartitionering, fallback-identitet og simuleret reload.
- `TD-250`: storage-write-fejl bevarer filoperationsklientens in-memory-identitet med 1/1.
- `TD-251`–`TD-254`: fire filnavnsmetadatafacitter for tomt loaded basis, ugyldig metadata-shape,
  genberegnet loaded basis og atomisk rollback ved skrivefejl, alle med 1/1.
- `TD-255`: pending PWA-load bevarer `unavailable`-status med 1/1.
- `TD-256`: afbrudt atomic delete bevarer begge standardmappeværdier med 1/1.
- `TD-257`–`TD-258`: persistent-storage-afslag og ikke-`Error` rejection med 1/1 hver, inklusive
  warning-/statusfacitter.

Den målrettede kontrol bestod med 16/16 nye tests. Typechecks, lint, encoding-, filnavns- og
diff-kontroller bestod. `verify:release:core` bestod derefter med 887 testfiler / 9.062 tests,
coverage 90,75 / 81,99 / 94,89 / 93,54 og begge builds. Den fulde E2E-suite kræver ikke ny
kørsel, fordi ændringerne kun vedrører testkode. TD-001, TD-016, TD-014, TD-018, TD-022,
B-002, TD-088 og TD-025 forbliver blokerende restpunkter for auditkonklusionen.

## Tidligere evidensdelta – revision `1e0dfd6a`

Den seneste batch tilføjer to disjunkte test-only facitter uden produktændringer:

- `TD-241`: `td020SatserDiverseReguleringssatsDocumentIndependent.test.ts` fastholder med
  1/1 den håndskrevne Diverse-reguleringssats `2.75` som `2,75 %` i faktisk Word-XML.
- `TD-242`: `fileOperationClientSessionInvalidStorageIndependent.test.ts` fastholder med
  1/1 rotation af en ugyldig gemt klientidentitet og bevarelse efter simuleret reload.

Den målrettede kontrol bestod med 2/2 nye tests. Typechecks, lint, encoding-, filnavns- og
diff-kontroller bestod. `verify:release:core` bestod derefter med 871 testfiler / 9.046 tests,
coverage 90,71 / 81,92 / 94,82 / 93,51 og begge builds. Den fulde E2E-suite kræver ikke ny
kørsel, fordi ændringerne kun vedrører testkode. TD-001, TD-016, TD-014, TD-018, TD-022,
B-002, TD-088 og TD-025 forbliver blokerende restpunkter for auditkonklusionen.

## Tidligere evidensdelta – revision `04a3e8b8`
...
- `TD-237`: `resetSectionHistoryIndependent.test.ts` fastholder med 1/1, at sektionsreset
  kun ændrer den valgte sektion og bevarer rejected input, øvrig sektion samt session/state
  gennem undo og redo.
- `TD-238`: `bootstrapSupportedDeviceOrderIndependent.test.tsx` fastholder med 1/1 den
  præcise supported desktop-bootstrapsekvens og render-kald.

Den målrettede kontrol bestod med 2/2 nye tests. Typechecks, lint, encoding-, filnavns- og
diff-kontroller bestod. `verify:release:core` bestod derefter med 865 testfiler / 9.040 tests,
coverage 90,71 / 81,91 / 94,82 / 93,51 og begge builds. Den fulde E2E-suite kræver ikke ny
kørsel, fordi ændringerne kun vedrører testkode. TD-001, TD-016, TD-014, TD-018, TD-022,
B-002, TD-088 og TD-025 forbliver blokerende restpunkter for auditkonklusionen.

## Tidligere evidensdelta – revision `4ab724d1`

Den seneste batch tilføjer to disjunkte test-only facitter uden produktændringer:

- `TD-235`: `td019StoreBededagPctRangeValidatorIndependent.test.ts` fastholder med 1/1 negativ
  `storeBededagPct` med præcis feltsti, besked og severity.
- `TD-236`: `td020Kapitalisering2026DocumentIndependent.test.ts` fastholder med 1/1 2026 tabel A,
  faktor `10.018` og konkrete Word-tekster gennem den faktiske dokumentgenerator.

Den målrettede kontrol bestod med 2/2 nye tests. Typechecks, lint, encoding-, filnavns- og
diff-kontroller bestod. `verify:release:core` bestod derefter med 863 testfiler / 9.038 tests,
coverage 90,72 / 81,92 / 94,82 / 93,51 og begge builds. Den fulde E2E-suite kræver ikke ny
kørsel, fordi ændringerne kun vedrører testkode. TD-001, TD-016, TD-014, TD-018, TD-022,
B-002, TD-088 og TD-025 forbliver blokerende restpunkter for auditkonklusionen.

## Tidligere evidensdelta – revision `c49c5714`

Den seneste batch tilføjer to disjunkte test-only facitter uden produktændringer:

- `TD-233`: `storageBoundaryGlobalThisNegative.test.ts` fastholder med 1/1, at rå
  `globalThis.localStorage` afvises af storage-boundary-reglen.
- `TD-234`: `satserDocumentRendererWiringIndependent.test.ts` fastholder med 1/1 Satser-definitionens
  præcise renderer-input og filoutput.

Den målrettede kontrol bestod med 2/2 nye tests. Typechecks, lint, encoding-, filnavns- og
diff-kontroller bestod. `verify:release:core` bestod derefter med 861 testfiler / 9.036 tests,
coverage 90,72 / 81,92 / 94,82 / 93,51 og begge builds. Den fulde E2E-suite kræver ikke ny
kørsel, fordi ændringerne kun vedrører testkode. TD-001, TD-016, TD-014, TD-018, TD-022,
B-002, TD-088 og TD-025 forbliver blokerende restpunkter for auditkonklusionen.

## Tidligere evidensdelta – revision `f46b1164`

Den seneste batch tilføjer to disjunkte test-only facitter uden produktændringer:

- `TD-231`: `fileHandleStorage.persistenceGrant.test.ts` fastholder med 1/1 succesfuld
  `navigator.storage.persist()` uden warning.
- `TD-232`: `eoInspektionSammentaellingMissingAbsenceIndependent.test.ts` fastholder med 1/1,
  at manglende øvrige fraværsdage giver `null`/`-` i beregningsgrundlaget, mens tabelgrundlaget
  fortsat er fem dage.

Den målrettede kontrol bestod med 2/2 nye tests. Typechecks, lint, encoding-, filnavns- og
diff-kontroller bestod. `verify:release:core` bestod derefter med 859 testfiler / 9.034 tests,
coverage 90,71 / 81,91 / 94,82 / 93,51 og begge builds. Den fulde E2E-suite kræver ikke ny
kørsel, fordi ændringerne kun vedrører testkode. TD-001, TD-016, TD-014, TD-018, TD-022,
B-002, TD-088 og TD-025 forbliver blokerende restpunkter for auditkonklusionen.

## Tidligere evidensdelta – revision `72d9d169`

Den seneste batch tilføjer to disjunkte test-only facitter uden produktændringer:

- `TD-229`: `indexedDbStore.readRequestFailure.test.ts` fastholder med 1/1 readonly get-fejlens
  error/context-logning og lukkede IndexedDB-forbindelse.
- `TD-230`: `eoInspektionRegulationKlCanonicalIndependent.test.ts` fastholder med 1/1 den
  beregnede KL-reguleringstabel med to canonical segmenter, perioder og regulerede lønninger.

Den målrettede kontrol bestod med 7 filer / 67 tests inklusive relaterede eksisterende facitter.
Typechecks, lint, encoding-, filnavns- og diff-kontroller bestod. `verify:release:core` bestod
derefter med 857 testfiler / 9.032 tests, coverage 90,71 / 81,91 / 94,82 / 93,50 og begge
builds. Den fulde E2E-suite kræver ikke ny kørsel, fordi ændringerne kun vedrører testkode.
TD-001, TD-016, TD-014, TD-018, TD-022, B-002, TD-088 og TD-025 forbliver blokerende
restpunkter for auditkonklusionen.

## Historisk evidensdelta – revision `8e9c028e`

Den seneste batch tilføjer tre disjunkte test-only facitter uden produktændringer:

- `TD-220`: `fileLoad.normalLoad.test.ts` fastholder med 1/1 en handlingsanvisende fejl for
  `NotFoundError` ved en flyttet eller slettet PWA-fil og kræver 0 læsekald.
- `TD-221`: `eoInspektionKontrolModelMenStopIndependent.test.ts` fastholder med 1/1 Varige
  Mén-stopdatoen i EO-kontrolmodellen og tomme integritetsissues.
- `TD-222`: `eoRowIndkomstModelIndependent.test.ts` fastholder med 1/1 samling af to beløbsfrie
  Dagpenge-perioder i én advarselsrække med korrekt antal og første række-id.

Den målrettede kontrol bestod med 3 filer / 29 tests. Typechecks, lint, encoding-, filnavns- og
diff-kontroller bestod. `verify:release:core` bestod derefter med 849 testfiler / 9.024 tests,
coverage 90,60 / 81,76 / 94,76 / 93,38 og begge builds. Den fulde E2E-suite kræver ikke ny
kørsel, fordi ændringerne kun vedrører testkode. TD-001, TD-016, TD-014, TD-018, TD-022, B-002,
TD-088 og TD-025 forbliver blokerende restpunkter for auditkonklusionen.

## Historisk evidensdelta – revision `e50fb1e5`

Den seneste batch tilføjer tre disjunkte test-only facitter uden produktændringer:

- `TD-217`: `fileSaveTarget.test.ts` fastholder med 1/1 oprydning af et forældreløst file handle
  uden sessionStorage-filnavn og det fail-safe picker-flow.
- `TD-218`: `eoInspektionLoenCoreModelIndependent.test.ts` fastholder med 1/1, at manglende valgt
  svie/smerte-satsår ikke producerer en afledt satsdag.
- `TD-219`: `eoInspektionLoenCoreModelPublicBoundaryIndependent.test.ts` fastholder med 1/1,
  at offentlig løn før første gældende regulering ikke producerer en løndag.

Den målrettede kontrol bestod med 3 filer / 17 tests. Typechecks, lint, encoding-, filnavns- og
diff-kontroller bestod. `verify:release:core` bestod derefter med 847 testfiler / 9.021 tests,
coverage 90,58 / 81,70 / 94,76 / 93,36 og begge builds. Den fulde E2E-suite kræver ikke ny
kørsel, fordi ændringerne kun vedrører testkode. TD-001, TD-016, TD-014, TD-018, TD-022, B-002,
TD-088 og TD-025 forbliver blokerende restpunkter for auditkonklusionen.

## Historisk evidensdelta – revision `02c5ef2a`

Den seneste batch tilføjer tre disjunkte test-only facitter uden produktændringer:

- `TD-214`: `scrollToSection.test.ts` fastholder selector-success, attention, callback og konkret
  retry-failure med 4/4.
- `TD-215`: `scrollToFieldAddress.test.ts` fastholder field-/collection-lookup, shared blink,
  callback og konkret retry-failure med 5/5.
- `TD-216`: `eoInspektionLoenTypesIndependent.test.ts` fastholder kontroltabel-id-generatorer,
  beløbsklassifikation samt gyldig og ugyldig arbejdsstedsparser med 5/5.

Den målrettede kontrol bestod med 3 filer / 14 tests. Typechecks, lint, encoding-, filnavns- og
diff-kontroller bestod. `verify:release:core` bestod derefter med 845 testfiler / 9.018 tests,
coverage 90,56 / 81,66 / 94,76 / 93,35 og begge builds. Den fulde E2E-suite kræver ikke ny
kørsel, fordi ændringerne kun vedrører testkode. TD-001, TD-016, TD-014, TD-018, TD-022, B-002,
TD-088 og TD-025 forbliver blokerende restpunkter for auditkonklusionen.

## Historisk evidensdelta – revision `33013239`

Den seneste batch tilføjer tre disjunkte test-only facitter uden produktændringer:

- `TD-211`: `fileOperationClientSession.persistence.test.ts` fastholder med 1/1 samme
  sessionStorage-klientidentitet efter simuleret reload.
- `TD-212`: `td019TidligereModtagetTafCanonicalValidatorIndependent.test.ts` fastholder med 1/1
  et håndskrevet negativt `tidligereModtagetTaf`-facit med præcis issue.
- `TD-213`: `rowId.test.ts` tilføjer 3/3 uafhængige facitter for `randomUUID`,
  `getRandomValues` og tidsstempel/`Math.random`-fallback, mens eksisterende facitter bevares.

Den målrettede kontrol bestod med 3 filer / 12 tests. Typechecks, lint, encoding-, filnavns- og
diff-kontroller bestod. `verify:release:core` bestod derefter med 842 testfiler / 9.004 tests,
coverage 90,43 / 81,56 / 94,39 / 93,22 og begge builds. Den fulde E2E-suite kræver ikke ny
kørsel, fordi ændringerne kun vedrører testkode. TD-001, TD-016, TD-014, TD-018, TD-022, B-002,
TD-088 og TD-025 forbliver blokerende restpunkter for auditkonklusionen.

## Historisk evidensdelta – revision `188c7ea7`

Den seneste batch tilføjer to disjunkte test-only facitter uden produktændringer:

- `TD-209`: `aarsloenAmountIndependentOracle.test.ts` fastholder med 1/1 direkte
  tillægsbeløb i Beløb-tilstand og korrekt måned-til-år-omregning.
- `TD-210`: `safeLocalStorage.browserFallback.test.ts` fastholder med 1/1 fungerende
  in-memory browserfallback, når `window.localStorage`-getter’en kaster.

Den målrettede kontrol bestod med 2/2 tests. Typechecks, lint, encoding-, filnavns- og
diff-kontroller bestod. `verify:release:core` bestod derefter med 840 testfiler / 8.999 tests,
coverage 90,38 / 81,54 / 94,33 / 93,16 og begge builds. Den fulde E2E-suite kræver ikke ny
kørsel, fordi ændringerne kun vedrører testkode. TD-001, TD-016, TD-014, TD-018,
TD-022, B-002, TD-088 og TD-025 forbliver blokerende restpunkter for auditkonklusionen.

## Historisk evidensdelta – revision `ef38caa1`

Den seneste batch tilføjer tre disjunkte test-only facitter uden produktændringer:

- `TD-206`: `rejectedSettleNoOpHistory.test.ts` fastholder med 1/1 gentaget format-rejected
  settle som no-op med intakt history og XOR gennem undo/redo.
- `TD-207`: `eoFileCodecUnavailableCrypto.test.ts` fastholder med 1/1 fail-closed decode
  og korrekt fejllogning, når browserens `crypto` mangler.
- `TD-208`: `beregnMaanedPeriodeAarsskifteIndependentOracle.test.ts` fastholder med 1/1
  december 2024 + januar 2025 som 62 sammenhængende kalenderdage.

Den målrettede kontrol bestod med 3/3 tests. Typechecks, lint, encoding-, filnavns- og
diff-kontroller bestod. `verify:release:core` bestod derefter med 838 testfiler / 8.997 tests,
coverage 90,37 / 81,52 / 94,33 / 93,16 og begge builds. Den fulde E2E-suite kræver ikke ny
kørsel, fordi ændringerne kun vedrører testkode. TD-001, TD-016, TD-014, TD-018,
TD-022, B-002, TD-088 og TD-025 forbliver blokerende restpunkter for auditkonklusionen.

## Historisk evidensdelta – revision `73081df5`

Den seneste batch tilføjer tre disjunkte test-only facitter uden produktændringer:

- `TD-203`: `eoInspektionLedighedsydelseRowPartitionIndependent.test.ts` fastholder med 1/1
  to ledighedsydelsesrækker over månedsskifte og weekend med korrekt kontrolsum.
- `TD-204`: `reguleringReExportBlindspot.test.ts` fastholder med 1/1, at wildcard-re-export
  ikke kan omgå reguleringsseriernes arkitekturværn.
- `TD-205`: `td019SfggAlleredeBetaltBeloebValidatorIndependent.test.ts` fastholder med 1/1
  præcis nested feltsti, besked og `error`-severity for negativt allerede-betalt-beløb.

Den målrettede kontrol bestod med 3/3 tests. Typechecks, lint, encoding-, filnavns- og
diff-kontroller bestod. `verify:release:core` bestod derefter med 835 testfiler / 8.994 tests,
coverage 90,37 / 81,50 / 94,33 / 93,15 og begge builds. Den fulde E2E-suite kræver ikke ny
kørsel, fordi ændringerne kun vedrører testkode. TD-001, TD-016, TD-014, TD-018,
TD-022, B-002, TD-088 og TD-025 forbliver blokerende restpunkter for auditkonklusionen.

## Historisk evidensdelta – revision `d3f3c771`

Den seneste batch tilføjer tre disjunkte test-only facitter uden produktændringer:

- `TD-200`: `reguleringImportEqualsBlindspot.test.ts` fastholder med 1/1,
  at TypeScript import-equals ikke kan omgå reguleringsseriernes arkitekturværn.
- `TD-201`: `renteberegningEngineLatestRatePeriodIndependent.test.ts` fastholder med 1/1,
  at den seneste referencesatsperiode slutter 31.12.2024 i et håndberegnet procesrentefacit.
- `TD-202`: `td019PensionPctRangeValidatorIndependent.test.ts` fastholder med 1/1
  præcis nested feltsti, besked og `error`-severity for pensionsprocent over 100.

Den målrettede kontrol bestod med 4 filer / 194 tests. Typechecks, lint, encoding-,
filnavns- og diff-kontroller bestod. `verify:release:core` bestod derefter med 832 testfiler /
8.991 tests, coverage 90,37 / 81,50 / 94,33 / 93,15 og begge builds. Den fulde E2E-suite
kræver ikke ny kørsel, fordi ændringerne kun vedrører testkode; seneste fulde kørsel er fortsat
`71746917` med 221/223.
TD-001, TD-016, TD-014, TD-018, TD-022, B-002, TD-088 og TD-025 forbliver blokerende
restpunkter for auditkonklusionen.

## Historisk evidensdelta – revision `41bd0b61`

Den seneste batch tilføjer tre disjunkte test-only facitter uden produktændringer:

- `TD-197`: `eoInspektionWageMonthIndependentOracle.test.ts` fastholder med 1/1
  månedsløn gennem EO-kontroltabellen i kalenderdags-tilstand med intakte lønkolonner og kontrolsum.
- `TD-198`: `td019OevrigeKravValidatorIndependent.test.ts` fastholder med 1/1
  nulbeløbets præcise feltsti, besked og `error`-severity for øvrige krav.
- `TD-199`: `e2eLoginAwaitGuard.test.ts` fastholder med 6/6, at den fælles loginrejse await-es,
  at modcases fanges, og at den levende E2E-suite ikke har uawaitede login-kald.

Den målrettede kontrol bestod med 9/9 tests. Typechecks, lint, encoding-, filnavns- og
diff-kontroller bestod. `verify:release:core` bestod derefter med 829 testfiler / 8.988 tests,
coverage 90,37 / 81,50 / 94,33 / 93,15 og begge builds. Den fulde E2E-suite er ikke gentaget,
fordi batchen kun ændrede testkode; seneste fulde kørsel er fortsat `71746917` med 221/223.
TD-001, TD-016, TD-014, TD-018, TD-022, B-002, TD-088 og TD-025 forbliver blokerende
restpunkter for auditkonklusionen.

## Historisk evidensdelta – revision `ed6bd4e9`

Den seneste batch tilføjer tre disjunkte test-only facitter uden produktændringer:

- `TD-194`: `aarsloenYearBoundaryIndependentOracle.test.ts` fastholder med 1/1
  native UTC-datoer over december/januar, 62 unikke dage, to hele måneder og `360.000 kr.` årligt.
- `TD-195`: `renteberegningEngineHistoricalSurchargeIndependent.test.ts` fastholder med 1/1
  skæringsdatoen 01.03.2013, 10 % samlet rente og håndberegnet afrundet rente `54,79 kr.`.
- `TD-196`: `documentDeliveryFailureLifecycleIndependent.test.ts` fastholder med 1/1,
  at en kastet filaflevering bliver runtimefejl i `deliver`-fasen og rapporteres med korrekt fase.

Den målrettede kontrol bestod med 3/3 tests. Typechecks, lint, encoding-, filnavns- og
diff-kontroller bestod. `verify:release:core` bestod derefter med 827 testfiler / 8.980 tests,
coverage 90,37 / 81,50 / 94,33 / 93,15 og begge builds. Den fulde E2E-suite er ikke gentaget,
fordi batchen kun ændrede testkode; seneste fulde kørsel er fortsat `71746917` med 221/223.
TD-001, TD-016, TD-014, TD-018, TD-022, B-002, TD-088 og TD-025 forbliver blokerende
restpunkter for auditkonklusionen.

## Historisk evidensdelta – revision `09023d77`

Den seneste batch tilføjer tre disjunkte test-only facitter uden produktændringer:

- `TD-191`: `forsoergertabCalculation.test.ts` fastholder med 1/1 et håndberegnet
  skuddag → marts-facit på `0,0667` måned og `667 kr.` gennem ASL-dagbrøken.
- `TD-192`: `td019ValidatorSchemaIndependence.test.ts` fastholder med 1/1 den separate
  nested feltsti og præcise besked/severity for manglende lønregulering ved `Angivet månedsløn`.
- `TD-193`: `ui-010-save-picker-cancel-preserves-case.spec.ts` fastholder med 1/1 i Chrome,
  gentaget 3/3, at annullering af Gem-picker bevarer aktiv sag og URL uden succes, teknisk fejl,
  runtimefejl, runtime-signaler eller eksterne requests.

Den målrettede Vitest-kontrol bestod med 49/49 tests. Typechecks, lint, encoding-, filnavns- og
diff-kontroller bestod. `verify:release:core` bestod derefter med 824 testfiler / 8.977 tests,
coverage 90,37 / 81,50 / 94,33 / 93,15 og begge builds. Den fulde E2E-suite er ikke gentaget,
fordi batchen kun ændrede testkode; seneste fulde kørsel er fortsat `71746917` med 221/223.
TD-001, TD-016, TD-014, TD-018, TD-022, B-002, TD-088 og TD-025 forbliver blokerende
restpunkter for auditkonklusionen.

## Historisk evidensdelta – revision `b588581a`

Den seneste batch tilføjer tre disjunkte test-only facitter uden produktændringer:

- `TD-188`: `periodeBeregningDagCenturyIndependentOracle.test.ts` fastholder med 1/1,
  at 28.02.–01.03.1900 er præcis to inklusive dage og ikke behandler 1900 som skudår.
- `TD-189`: `renteberegningEngineDayBoundaryIndependent.test.ts` fastholder med 1/1 et
  håndberegnet dagtillæg på `94,94 kr.` ved 30.06.2024 og referencesatsskiftet 01.07.
- `TD-190`: `ui-010-file-picker-cancel-preserves-case.spec.ts` fastholder med 1/1 i Chrome,
  at annullering af den native Hent-picker bevarer aktiv sag og URL uden loadresultat,
  runtimefejl, runtime-signaler eller eksterne requests.

Den målrettede batch bestod med 3/3 nye tests. Typechecks, lint, encoding-, filnavns- og
diff-kontroller bestod. `verify:release:core` bestod derefter med 824 testfiler / 8.975 tests,
coverage 90,37 / 81,50 / 94,33 / 93,15 og begge builds. Den fulde E2E-suite er ikke gentaget,
fordi batchen kun ændrede testkode; seneste fulde kørsel er fortsat `71746917` med 221/223.
TD-001, TD-016, TD-014, TD-018, TD-022, B-002, TD-088 og TD-025 forbliver blokerende
restpunkter for auditkonklusionen.

## Historisk evidensdelta – revision `fa52a9c9`

Den seneste batch tilføjer tre disjunkte test-only facitter uden produktændringer:

- `TD-185`: `td019SvieSmerteDelvisSygemeldingSatsValidatorIndependent.test.ts` fastholder
  med 1/1 præcis path, besked og `error`-severity for manglende sats ved delvis sygemelding.
- `TD-186`: `td020EfterloenRowPartitionIndependent.test.ts` fastholder med 1/1 to
  håndberegnede efterlønsrækker i samme EO-kolonne over månedsskift og weekend samt samlet
  kontrolsum uden integrity issues.
- `TD-187`: `sfggEngineReExportBoundaryBlindspot.test.ts` fastholder med 1/1, at en forbudt
  SFGG-engine-re-export giver finding, mens ejerens re-export accepteres; arkitektursuiten
  bestod med 192/192.

Den målrettede batch bestod med 3/3 nye tests. Typechecks, lint, encoding-, filnavns- og
diff-kontroller bestod. `verify:release:core` bestod derefter med 822 testfiler / 8.973 tests,
coverage 90,37 / 81,50 / 94,33 / 93,15 og begge builds. Den fulde E2E-suite er ikke gentaget,
fordi batchen kun ændrede testkode; seneste fulde kørsel er fortsat `71746917` med 221/223.
TD-001, TD-016, TD-014, TD-018, TD-022, B-002, TD-088 og TD-025 forbliver blokerende
restpunkter for auditkonklusionen.

## Historisk evidensdelta – revision `649536ff`

Den seneste batch tilføjer to disjunkte test-only facitter uden produktændringer:

- `TD-183`: `procesrenteYearBoundaryIndependentOracle.test.ts` fastholder med 1/1 et
  håndberegnet referencesatsskift ved 1. januar, inklusive 365 dage i 2023, 366 dage i 2024
  og samlet rente `120,3682910397485`.
- `TD-184`: `ui-010-file-load-invalid-preserves-case.spec.ts` fastholder med 1/1 i Chrome,
  at en ugyldig filtype viser `Valgt fil er ikke en .eo fil`, bevarer aktivt journalnummer og
  URL og ikke giver runtimefejl, runtime-signaler eller eksterne requests.

Den målrettede procesrentetest, E2E-testen, E2E-typecheck, målrettet ESLint og diff-kontrol
bestod. `verify:release:core` bestod derefter med 819 testfiler / 8.970 tests, coverage
90,37 / 81,50 / 94,33 / 93,15 og begge builds. Den fulde E2E-suite er ikke gentaget, fordi
batchen kun ændrede testkode; seneste fulde kørsel er fortsat `71746917` med 221/223.
TD-001, TD-016, TD-014, TD-018, TD-022, B-002, TD-088 og TD-025 forbliver blokerende
restpunkter for auditkonklusionen.

## Historisk evidensdelta – revision `d39076e1`

Den seneste batch lukker tre nye, afgrænsede testfacitter uden produktændringer:

- `TD-153`: `td020PensionDownstreamIndependent.test.ts` fører 1.200 kr. fra 18. til 20. maj
  2024 gennem kalenderdagsconsumeren med 400 kr. pr. dag, weekendmarkering, kontrolsum og
  ingen integrity issues.
- `TD-154`: `td019OffentligeYdelserReguleringsValidatorIndependent.test.ts` kræver præcis
  path, besked og `error`-severity for reguleringsbase før første satsår.
- `TD-155`: `reguleringRequireImportBlindspot.test.ts` kræver, at CommonJS `require(...)` af
  reguleringsserien giver en finding i architecture-harnesset.

De tre målrettede tests bestod med 3/3. Den fulde Vitest-suite bestod med 790 testfiler og
8.888 tests, og den efterfølgende releasegate er dokumenteret nedenfor. `TD-019`, `TD-020`
og `ARCH-002` er fortsat `I gang`, fordi disse facitter kun lukker de konkrete partitioner.
...
## Seneste test-only quality-, validator- og UI-batch efter revision `8321fdb2`

- `githubActionsLaneMatrixLiveness.test.ts` fastholder de seks almindelige E2E-lanes med deres
  deklarerede browsere og har et negativt facit for en fjernet viewportlane; målrettet kontrol
  bestod med 2/2.
- `td019OevrigeFravaersdageRangeValidatorIndependent.test.ts` bruger en håndskrevet typed fixture
  og kræver præcis afvisning af 367 øvrige fraværsdage med 1/1.
- `ui-006-aarsloen-beregningsprincipper-browserrejse.spec.ts` gennemfører Dato-periode,
  helårsomregning, afhængige ferie-/SH-valg, SH-dage og faktisk PDF-download; målrettet E2E
  bestod med 1/1 i Chrome uden runtimefejl, runtime-signaler eller eksterne requests.
- Den fulde `npm run test` bestod med 776 testfiler / 8.871 tests, og den fulde valgte E2E-suite
  bestod med 214 tests og 2 forventede skips ud af 216. Ændringerne er test-only.

## Seneste samlede releasegate efter revision `8321fdb2`

- `npm run verify:release:core` bestod med 776 testfiler / 8.871 beståede Vitest-tests uden
  forventede `it.fails`, coverage 90,26 / 81,42 / 94,03 / 93,07 og begge produktionsbuilds.
  Coverage målte 18.583/20.587 statements, 13.104/16.094 branches, 3.027/3.219 funktioner
  og 16.969/18.232 linjer. Dependency-, runtime-, type-, lint-, data-, kontrakt-, lane-,
  ledger- og artefaktkontroller bestod; Vite rapporterede kun de kendte native-config- og
  chunk-størrelsesadvarsler.
- E2E-kørslen er grøn på samme revision med 214 beståede tests og 2 forventede skips ud af
  216 på 10 projektbaner. Der blev ikke registreret ukontrollerede runtimefejl, runtime-signaler
  eller eksterne requests.

## Seneste test-only EET-, ydelses- og namespace-batch efter revision `38df665f`

- `focusTraversalNamespaceImportBlindspot.test.ts` og det udvidede focus-navigation-harness
  afviser namespace-imports med 1/1, så en namespace-import ikke kan omgå focus-primitiveimportreglen.
- `td020UddannelseshjaelpDownstreamIndependent.test.ts` fører et håndskrevet 2024-facit gennem
  kalenderdagsconsumeren med 1/1: 1.500 kr. fordelt over fire kalenderdage giver 375 kr. pr. dag.
- `ui-003-eet-fem-faner-reload.spec.ts` gennemgår alle fem EET-faner, browser-reload og bevaret
  afsluttet input; målrettet Chrome bestod med 1/1 uden runtimefejl, runtime-signaler eller eksterne
  requests.
- Den fulde `npm run test` bestod med 778 testfiler / 8.873 tests, og den fulde valgte E2E-suite
  bestod med 215 tests og 2 forventede skips ud af 217. Ændringerne er test-only.

## Seneste samlede releasegate efter revision `38df665f`

- `npm run verify:release:core` bestod med 778 testfiler / 8.873 beståede Vitest-tests uden
  forventede `it.fails`, coverage 90,26 / 81,42 / 94,03 / 93,07 og begge produktionsbuilds.
  Coverage målte 18.583/20.587 statements, 13.104/16.094 branches, 3.027/3.219 funktioner
  og 16.969/18.232 linjer. Dependency-, runtime-, type-, lint-, data-, kontrakt-, lane-,
  ledger- og artefaktkontroller bestod; Vite rapporterede kun de kendte native-config- og
  chunk-størrelsesadvarsler.
- E2E-kørslen er grøn på samme revision med 215 beståede tests og 2 forventede skips ud af
  217 på 10 projektbaner. Der blev ikke registreret ukontrollerede runtimefejl, runtime-signaler
  eller eksterne requests.

## Seneste test-only dato-, validator- og ydelsesbatch efter revision `4a2c6b09`

- `periodeBeregningDagLeapYearIndependentOracle.test.ts` fastholder fire unikke dage ved
  overlappende perioder omkring skuddagen med 1/1.
- `td019TafDagsloenValidatorIndependent.test.ts` kræver præcis afvisning af manglende dagsløn
  ved Angivet dagsløn med 1/1.
- `td020RevalideringsydelseDownstreamIndependent.test.ts` fører et håndskrevet skudårs-/weekendfacit
  gennem EO-inspektionens kalenderdagsconsumer med 1/1.
...
## Seneste samlede releasegate efter revision `c68068fa`

- `npm run verify:release:core` bestod med 781 testfiler / 8.876 beståede Vitest-tests uden
  forventede `it.fails`, coverage 90,26 / 81,41 / 94,03 / 93,06 og begge produktionsbuilds.
  Coverage målte 18.582/20.587 statements, 13.103/16.094 branches, 3.027/3.219 funktioner
  og 16.968/18.232 linjer. Dependency-, runtime-, type-, lint-, data-, kontrakt-, lane-,
  ledger- og artefaktkontroller bestod; Vite rapporterede kun de kendte native-config- og
...
## Seneste test-only input-, UI- og ydelsesbatch efter revision `9043e591`
...
## Seneste samlede releasegate efter revision `9043e591`
...
## Seneste test-only validator-, data- og UI-batch efter revision `89a2d3e0`
...
## Seneste samlede releasegate efter revision `89a2d3e0`
...
## Seneste test-only data-, validator- og inputbatch efter revision `d7224884`
...
## Seneste samlede releasegate efter revision `d7224884`
...
## Seneste test-only data-, validator- og arkitekturbatch efter revision `d39076e1`

`td020PensionDownstreamIndependent.test.ts`, `td019OffentligeYdelserReguleringsValidatorIndependent.test.ts`
og `reguleringRequireImportBlindspot.test.ts` bestod samlet med 3/3. Den fulde `npm run test`
bestod med 790 testfiler / 8.888 tests. Ingen E2E-kørsel blev gentaget, da ingen E2E-spec eller
browserrejse blev ændret. Ændringerne er test-only.

## Seneste samlede releasegate efter revision `d39076e1`
...
Den efterfølgende `verify:release:core` og fulde valgte E2E-suite er dokumenteret på `71746917` nedenfor.

## Seneste samlede releasegate og E2E efter arbejdsrevision `71746917`

`verify:release:core` bestod med 808 testfiler / 8.948 Vitest-tests uden forventede `it.fails`,
coverage 90,37 / 81,50 / 94,33 / 93,15 og begge builds. Den fulde valgte E2E-suite bestod med
...
## Seneste samlede releasegate efter arbejdsrevision `2bec9ac8`

`verify:release:core` bestod med 811 testfiler / 8.956 Vitest-tests uden forventede `it.fails`,
...
## Seneste samlede releasegate efter arbejdsrevision `8d1f2007`

`verify:release:core` bestod med 814 testfiler / 8.965 Vitest-tests uden forventede `it.fails`,
coverage 90,36 / 81,49 / 94,33 / 93,15 og begge produktionsbuilds. Den fulde valgte E2E-suite
er fortsat senest grøn på `71746917` med 221 tests og 2 forventede skips ud af 223 på 10
...
persistens er ændret. Den efterfølgende samlede releasegate er dokumenteret nedenfor.
