...
export const buildEODebugTafBeregningsgrundlagRows = (
  ...
  const beregningsperiodeDisplay = (() => {
    if (!isBeregningsperiode) return { displayValue: '-', status: 'ok' as DebugStatus };
  ...
  const maanedsloenDisplay = (() => {
    if (beregnesUdFra !== 'Angivet månedsløn') return { displayValue: '-', status: 'ok' as DebugStatus };
  ...
  const dagsloenDisplay = (() => {
    if (beregnesUdFra !== 'Angivet dagsløn') return { displayValue: '-', status: 'ok' as DebugStatus };
