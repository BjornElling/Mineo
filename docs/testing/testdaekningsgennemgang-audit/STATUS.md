...
| Aktuel fuld Vitest-retest | `npm run test:coverage` | Bestået | 669 filer, 8.690 tests bestået samt 17 forventede `it.fails`; coverage-fase 345,79 s på `90c4870f` |
| E2E-baner | `npm run test:e2e` | Bestået | 191 beståede tests og 2 forventede skips ud af 193, 4,1 min, 3 workers; `playwright-report/` og `test-results/` på `90c4870f` |
| Aktuel samlet release-gate | `npm run verify:release:core` | Bestået | Dependency-, runtime-, type-, lint-, data-, kontrakt-, lane-, ledger-, coverage- og begge build-gates bestået på `90c4870f`; coverage 669 filer / 8.690 beståede tests / 17 forventede `it.fails` |
...
| Aktuel coverage-retest | `npm run test:coverage` | Bestået | 393 instrumenterede filer; 90,13 % statements, 81,20 % branches, 93,97 % functions, 92,95 % lines; 18.554 / 20.584, 13.066 / 16.090, 3.024 / 3.218 og 16.946 / 18.230 målte enheder; `coverage/index.html`, `coverage/clover.xml` og `coverage/coverage-final.json`; Node `v24.18.0`/Windows |
