...
export type GreenfieldSvieSmerteTableProps = Readonly<{
...
const GreenfieldSvieSmerteTable = React.memo(({ committedRows, derivedById, saveOrderPath }: GreenfieldSvieSmerteTableProps) => {
    ...
    <StandardLooseTable sx={{ width: '760px', tableLayout: 'fixed', mb: 3, '& .MuiTableCell-root': { textAlign: 'center', whiteSpace: 'nowrap' }, '& thead th': { textAlign: 'center' } }}>
      ...
      <TableBody>{renderRows.map((row) => {
        ...
        return <TableRow key={row.rowId} data-mineo-row-id={row.rowId}>
          <TableCell><GreenfieldGridDateCell gridCell={{ rowId: row.rowId, colIndex: 0 }} cell={table.buildCellSpec(row, eoSvieSmertePeriodeFraField, 0)} /></TableCell>
          <TableCell><GreenfieldGridDateCell gridCell={{ rowId: row.rowId, colIndex: 1 }} cell={table.buildCellSpec(row, eoSvieSmertePeriodeTilField, 1)} /></TableCell>
...
GreenfieldSvieSmerteTable.displayName = 'GreenfieldSvieSmerteTable';
export default GreenfieldSvieSmerteTable;
