...
| Aktuel fuld Vitest-retest | `npm run test` | Bestået | 824 filer, 8.975 tests bestået og ingen forventede `it.fails` på `b588581a` |
...
| Aktuel samlet release-gate | `npm run verify:release:core` | Bestået | Dependency-, runtime-, type-, lint-, data-, kontrakt-, lane-, ledger-, coverage- og begge build-gates bestået på `b588581a`; coverage 824 filer / 8.975 beståede tests / ingen forventede `it.fails` |
...
| Aktuel coverage-retest | `npm run test:coverage` | Bestået | 394 instrumenterede filer; 90,37 % statements, 81,50 % branches, 94,33 % functions, 93,15 % lines; coverage-artefakterne blev genereret under `coverage/`; `b588581a`, Node `v24.18.0`/Windows |
...
| Senest tilføjet lukket | `TD-153`, `TD-154`, `TD-155`, `TD-157`–`TD-190` – test-only facitter dokumenteret til og med `b588581a`; `TD-156` er fortsat åbent |

## Aktuel evidensdelta – revision `b588581a`
...
Den seneste samlede releasegate på `b588581a` er retestet grønt; den fulde E2E-suite er senest grøn
på `71746917` med 221/223. Den seneste arbejdsbatch tilføjer tre test-only facitter, og både
målrettet retest og samlet releasegate er grønne på `b588581a`. Næste arbejdsenhed er en ny
