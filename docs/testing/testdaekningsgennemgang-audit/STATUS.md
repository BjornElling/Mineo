...
- Evidensrevision: `40cf3cad` for seneste samlede releasegate og målrettede test-only batch; seneste fulde E2E er `71746917`
- Branch: `main`
...
- Seneste test-only batch: PERSIST-facitter TD-243–TD-258; målrettet kontrol 16 nye tests / 16 tests; ingen produktkode, beregningslogik, UI/UX eller persistens ændret.
...
| `PERSIST-002` | Storage/settings, filhåndtag, PWA-filflows og reel session-reload gennemgået | 35 filer / 200 målrettede unit-/hook-tests; PWA-/filkørsel 4/4; reel input + aktiv fane + reload 1/1; supplerende 8-filers platformssuite med 89/89 beståede tests efter callable picker-detektion; `safeLocalStorage.test.ts` bestod med 11/11 direkte Node-fallback-isolationsassertions; `fileHandleVerification.test.ts` bestod med 22/22; `fileHandleKvStore.test.ts` bestod med 2/2; den efterfølgende test-only batch tilføjer 16/16 uafhængige klient-, session-, filmetadata-, pending-PWA-, atomic-delete- og persistent-storage-facitter, herunder `unavailable`-status, storage-write-fejl og transaction-abort; ugyldige `Date`-inputs har 8/8 grønne negative modcases | Fuld browser-/platformssammenhæng, herunder installeret PWA-filaflevering; `TD-022` | `I gang` |
...
Aktuel inventarrevision på `40cf3cad`: `PERSIST-002` omfatter nu også den nye klient-, session-,
filmetadata-, pending-PWA-, atomic-delete- og persistent-storage-batch med 16/16 målrettede
facitter. `CALC-006` omfatter
`eoInspektionSammentaellingMissingAbsenceIndependent.test.ts` med 1/1 facit for manglende øvrige fraværsdage.
De tilsvarende målrettede inventartal er dermed 35 filer / 200 tests for PERSIST-002 og 171 filer /
...
| Senest tilføjet lukket | `TD-153`, `TD-154`, `TD-155`, `TD-157`–`TD-258` – test-only facitter dokumenteret til og med `40cf3cad`; `TD-156` er fortsat åbent |

## Aktuel evidensdelta – revision `40cf3cad`
