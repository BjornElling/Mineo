...
| Baseline: antal kildefiler, testfiler, tests, E2E-specs og mutationsscore | Ved auditstart: 923 produktionsfiler (`.ts/.tsx`), 638 testfiler i alt, 8.421 Vitest-tests, 36 E2E-specs og 174 E2E-tests. Aktuel testrevision `1956eb8d`: 920 produktionsfiler, 663 Vitest-testfiler, 8.604 beståede tests samt 17 forventede `it.fails`, 42 E2E-specs og 185 beståede lane-tests. Mutationsrunneren er kvalificeret modulvist: money 56/58 dræbt, `dateCommit.ts` 4/6 dræbt, årsløn 108/113 dræbt, varige mén 94/114 dræbt, forsørgertab 21/21 dræbt og procesrente 104/136 dræbt med 23 triagerede survivors og 9 dokumenterede timeouts; alle ikke-dræbte mutationer er triageret. |
| Baseline: `test:coverage`-rapport og de dækkede/udeladte mapper | `coverage/` fra releasekørsel på `83abfdad`: 396 instrumenterede filer i `src/domain`, `src/utils`, `src/hooks` og `src/contexts`; 89,23 % statements, 80,48 % branches, 92,35 % functions og 92,02 % lines – 18.369 / 20.584, 12.950 / 16.090, 2.972 / 3.218 og 16.776 / 18.230 målte enheder. Den fulde Vitest-retest på `1956eb8d` gav 8.604 beståede tests og 17 forventede `it.fails`, men coverage er ikke genkørt efter testtilføjelserne. Coverage-konfigurationen omfatter ikke de øvrige produktionsfiler; de skal klassificeres i inventaret. |
...
| Fund-ID'er, beslutninger og opfølgning | B-001 er lukket som `TD-005`. B-002 om at E2E bygger et separat `--mode e2e`-artefakt er delvist dækket af en lokal smoke mod det eksakte produktionsbuild, men kræver fortsat særskilt CI-/releaseafklaring. |
| Reviewer / dato / slutstatus | Codex / 2026-09-10 / `I gang` |
...
- [x] `npm run test:coverage` er kørt som del af `npm run verify:release:core` på `83abfdad` og rapporteret med instrumenteret filmængde; den fulde Vitest-retest på `1956eb8d` er kørt separat.
