...
describe('Greenfield numeriske presets', () => {
  it('GreenfieldIntegerField viser committed heltalsværdi', () => {
    dispatchInput(store, catalog, settleField(aargangField.bind(), '2020'));
    renderField(<GreenfieldIntegerField field={aargangField.bind()} location={{ locationId: 'int-1' }} name="aargang" />);
    expect(screen.getByRole('textbox')).toHaveValue('2020');
  });

  it('GreenfieldPercentField viser "%"-adornment', () => {
    // aargang er et number|undefined-felt; procent-skallen accepterer samme værditype (adornment-smoke).
    renderField(<GreenfieldPercentField field={aargangField.bind()} location={{ locationId: 'pct-1' }} name="pct" />);
    expect(screen.getByText('%')).toBeInTheDocument();
  });

  it('GreenfieldAmountField viser "kr."-adornment', () => {
...
describe('Greenfield immediate-commit control (radio)', () => {
  it('GreenfieldRadioField committer det valgte som setImmediateField', () => {
