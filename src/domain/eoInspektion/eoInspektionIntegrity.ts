/**
 * Integrity validation for EOInspektion
 *
 * Niveau A invariants (implementeret nu):
 * 1. PERIOD_OVERLAP - TAF-perioder overlapper
 * 2. DATE_HOLES - Huller i kontroltabel ift. forventet interval
...
import { IntegrityInvariant, type RowDay, type IntegrityIssue, type DateRange } from '../eoRowEvaluation/eoRowTypes';
import type { InspektionModelInput } from './eoInspektionCoreModel';
import { getOverlap, getIsoRange, tryParseIso } from './dateUtils';
 ...
 */
const checkPeriodOverlap = (input: InspektionModelInput): IntegrityIssue[] => {
...
/**
 * Tjek for huller i kontroltabel
 *
 * Verificer at alle dage mellem min og max er inkluderet
 */
const checkDateHoles = (inspektionDays: readonly RowDay[]): IntegrityIssue[] => {
  const issues: IntegrityIssue[] = [];

  if (inspektionDays.length === 0) return issues;

  const firstIso = inspektionDays[0]?.iso;
  const lastIso = inspektionDays[inspektionDays.length - 1]?.iso;
  ...
  const actualDates = new Set(inspektionDays.map((d) => d.iso));
  ...
  if (missing.length > 0) {
    issues.push({
      ...
      message: `Kontroltabel mangler ${missing.length} dag(e): ${missing
        .slice(0, 5)
 ...
 *
 * Sammenlign antal dage i TAF-perioder med antal dage markeret i kontrol-model
 */
const checkTafDaysMismatch = (
  ...
  for (const periode of tafPerioder) {
    ...
    // Faktisk antal dage markeret i kontrol-model
    const actualCount = inspektionDays.filter((d) => d.tafFlags.has(periode.id))
      .length;
    ...
    if (actualCount !== expectedCount) {
      issues.push({
        ...
        message: `TAF-periode ${periode.id}: Forventet ${expectedCount} dage, fandt ${actualCount} i kontroltabel`,
 ...
 *
 * Sammenlign antal dage i svie/smerte-perioder med antal dage markeret i kontrol-model
 */
const checkSvieSmerteMismatch = (
  ...
  for (const periode of ssPerioder) {
    ...
    const actualCount = inspektionDays.filter((d) => {
    ...
    if (actualCount !== expectedCount) {
      issues.push({
        ...
        message: `Svie/smerte-periode ${periode.id} (${expectedNiveau}): Forventet ${expectedCount} dage, fandt ${actualCount} i kontroltabel`,
        expected: expectedCount,
        actual: actualCount,
      });
    }
...
/**
 * Validér kontrol-model for integrity issues
 *
 * Kører alle Niveau A integrity checks automatisk.
 *
 * @param inspektionDays - RowDay array fra buildInspektionCoreModel
 * @param input - Oprindelige input-data
 * @returns Array af integrity issues (tom hvis alt OK)
 */
export function validateInspektionModel(
  inspektionDays: readonly RowDay[],
  input: InspektionModelInput
  ...
  issues.push(...checkDateHoles(inspektionDays));
  issues.push(...checkBaseDateConsistency(input));
  issues.push(...checkTafDaysMismatch(inspektionDays, input));
  issues.push(...checkSvieSmerteMismatch(inspektionDays, input));
