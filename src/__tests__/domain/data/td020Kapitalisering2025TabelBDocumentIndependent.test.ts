...
const stamdata: StamdataValues = {
  journalnr: 'TD-020-kapitalisering-2025-tabel-B',
  advokat: '',
  sagsbehandler: '',
  skadelidte: 'Uafhængigt facit',
  skadestype: 'Arbejdsulykke',
  skadedato: iso('2021-01-01'),
  skadelidteFodselsdato: iso('1963-01-01'),
};
...
const erhvervsevnetab: ErhvervsevnetabValues = {
  ...
  koen: 'Kvinde',
  aslAfgoerelser: [{
    id: 'td020-kapitalisering-2025-tabel-b-row',
    afgoerelsesDato: iso('2024-12-15'),
    virkningsDato: iso('2024-12-15'),
    eetPct: 25,
    kapDato: iso('2025-01-01'),
    kapPct: 25,
...
const project = (): KapitaliseringDocumentInput => {
  ...
  if (result.status !== 'ready') {
    throw new Error('Kapitaliseringsdefinitionen blev blokeret i tabel-B-facittet');
  }
  return result.input;
};

describe('TD-020 – 2025-tabel B gennem Word-generatoren', () => {
  it('fører 10029/2024 tabel B fra typed input til beregning og Word', async () => {
    const input = project();
    const afgoerelse = input.computation.afgoerelser[0];

    expect(afgoerelse).toEqual(expect.objectContaining({
      rowId: 'td020-kapitalisering-2025-tabel-b-row',
      afgoerelsesdato: '2024-12-15',
      kapitaliseringsdato: '2025-01-01',
      kapitaliseringsbekendtgoerelseLabel: 'Vejl. 10029/2024, tabel B',
      tabelLabel: 'B',
    ...
    expect(text).toContain('Kapitaliseringsdato01-01-2025');
    expect(text).toContain('Vejl. 10029/2024, tabel B');
    expect(text).toContain('Alder ved kapitalisering62 år, 0 måneder');
    expect(text).toContain('Folkepensionsalder68 år');
    expect(text).toContain('Kapitaliseringsfaktor3,784');
  });
});
