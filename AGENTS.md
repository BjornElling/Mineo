...
**Nuværende feature-flade.** Den offentliggjorte feature-flade og de overordnede beregningstyper er implementeret. Der findes dog planlagte, afgrænsede fremtidige udviklingsprojekter, som ikke er en del af den aktuelle funktionalitet, blandt andet beregning af særligt ferietillæg. De beskrives i `docs/implementation/` eller relevante kontrakter og kan implementeres ved lejlighed efter udviklerens prioritering. Autofill-suggest er implementeret 2026-09-07 og beskrives nu normativt i `input-field-behavior-contract.md` §1.5. Antag ikke yderligere hypotetiske beregningstyper eller features i arkitekturen – favorisér forenkling og konsolidering af det eksisterende frem for udvidelsespunkter uden et konkret projekt (jf. [Konvergens](#konvergens-rød-tråd)).
...
## Afhængigheder og advisories
- **`npm audit` er tidsafhængig, ikke ændringsafhængig.** Den spørger et eksternt advisory-register, som ændrer sig uden at repoet gør det, og `package-lock.json` er samtidig fastfrosset: hverken `npm ci` eller `npm install` flytter en transitiv pakke, der allerede opfylder sin range. Et uændret træ kan derfor gå fra grønt til rødt af sig selv. Derfor findes `.github/workflows/dependency-advisories.yml`, som kører `npm run check:dependencies` mod main hver mandag, så et fund lander som vedligeholdelse og ikke som en blokeret udrulning. Advisory-kontrollen bliver ved med at være en del af `verify:release:core`; tilsynet erstatter den ikke, det flytter kun fundet frem i tid.
- **Registret kan svare «nej tak» i stedet for at svare.** `npm audit` bruger exitkode 1 til to helt forskellige udfald: «jeg fandt sårbarheder» og «advisory-endpointet returnerede en fejl». Kaldt direkte væltede et forbigående 503 fra npm-registret derfor hele `verify:release:core` i dens allerførste trin (skete 2026-09-04). Auditen kaldes af den grund gennem `npm run check:vulnerabilities` (`scripts/check-vulnerabilities.mjs`), som gentager opslaget nogle gange med voksende ventetid og kun tolererer det udfald, hvor registret aldrig svarede – med en tydelig advarsel om, at kørslen så IKKE har kontrolleret sårbarhederne. Et ægte fund fejler stadig med det samme og prøver ikke igen. Grænsen mellem de to udfald er dækket af `src/__tests__/quality/vulnerabilityCheckRetry.test.ts` og må ikke sløres: kald aldrig `npm audit` direkte i en gate igen.
- **Rækkefølge når auditen er rød.** Find først, hvordan pakken hænger i træet (`npm ls <pakke>`), og hvilken range forælderen kræver:
  1. Tillader forældrens range allerede den rettede version, er `npm update <pakke>` hele rettelsen. Så flytter kun den låste version sig, og opdateringen forsvinder af sig selv, når forælderen selv rykker.
  2. Udelukker forældrens range den rettede version, er en post i `overrides` det rigtige instrument – men den er en varig, manuelt vedligeholdt afvigelse fra træet og skal derfor begrundes i commit-beskeden og fjernes igen, når forælderen har rettet sin range.
  3. Findes der ingen rettet version, eller kræver rettelsen en major-opdatering af en direkte dependency, er det en godkendelsesgate: forelæg valget frem for at gætte.
- **Aldrig som genvej:** `npm audit fix --force` (den opgraderer direkte dependencies på tværs af major uden vurdering), en hævet `--audit-level`, `--omit=dev` på auditen, eller en `overrides`-post i tilfælde 1. Alle fire gør gaten grøn uden at gøre træet rigtigt.
- Kør `npm ci` efter en `--package-lock-only`-opdatering. npm opdaterer i det tilfælde både `package-lock.json` og den skjulte `node_modules/.package-lock.json`, men ikke selve filerne på disken – derefter mener `npm install`, at træet er «up to date», mens den gamle version stadig ligger der. En verifikation kørt oven på det træ måler den forkerte version.
...
| **Afhængigheder** | `npm run check:dependencies` | Når `package.json`, `package-lock.json` eller `.agents/tools`-manifestet er ændret, og før en release. Kontrollerer også, at ingen to top-level pakker slås om det samme kommandonavn i `node_modules/.bin`. Advisory-delen kan blive rød uden en manifestændring – se [Afhængigheder og advisories](#afhængigheder-og-advisories) for hvorfor og for rettelsesrækkefølgen. |
