...
| Aktuel fuld Vitest-retest | `npm run test:coverage` | Bestået | 694 filer, 8.741 tests bestået samt 17 forventede `it.fails`; coverage 90,14 / 81,24 / 93,97 / 92,96; coverage-fase 351,67 s på `b19ef4b6` |
...
| Aktuel samlet release-gate | `npm run verify:release:core` | Bestået | Dependency-, runtime-, type-, lint-, data-, kontrakt-, lane-, ledger-, coverage- og begge build-gates bestået på `b19ef4b6`; coverage 694 filer / 8.741 beståede tests / 17 forventede `it.fails` |
...
| Aktuel coverage-retest | `npm run test:coverage` | Bestået | 393 instrumenterede filer; 90,14 % statements, 81,24 % branches, 93,97 % functions, 92,96 % lines; 18.556 / 20.584, 13.072 / 16.090, 3.024 / 3.218 og 16.948 / 18.230 målte enheder; `coverage/index.html`, `coverage/clover.xml` og `coverage/coverage-final.json`; `b19ef4b6`, Node `v24.18.0`/Windows |
