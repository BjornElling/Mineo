...
- Evidensrevision: `d9d1c5f2` for seneste samlede releasegate og målrettede test-only batch; seneste fulde E2E er `71746917`
- Branch: `main`
...
- Seneste test-only batch: VALID-/ARCH-facitter TD-280–TD-281; målrettet kontrol 2 nye tests / 2 tests; ingen produktkode, beregningslogik, UI/UX eller persistens ændret.
...
| Aktuel fuld Vitest-retest | `npm run test:coverage` | Bestået | 908 filer, 9.083 tests bestået og ingen forventede `it.fails` på `d9d1c5f2` |
...
| Aktuel samlet release-gate | `npm run verify:release:core` | Bestået | Dependency-, runtime-, type-, lint-, data-, kontrakt-, lane-, ledger-, coverage- og begge build-gates bestået på `d9d1c5f2`; coverage 908 filer / 9.083 beståede tests / ingen forventede `it.fails` |
...
| Aktuel coverage-retest | `npm run test:coverage` | Bestået | 394 instrumenterede filer; 90,74 % statements, 81,99 % branches, 94,88 % functions, 93,54 % lines; coverage-artefakterne blev genereret under `coverage/`; `d9d1c5f2`, Node `v24.18.0`/Windows |
...
Aktuel inventarrevision på `d9d1c5f2`: `PERSIST-002` omfatter nu også den nye silent-read-,
...
| Senest tilføjet lukket | `TD-153`, `TD-154`, `TD-155`, `TD-157`–`TD-281` – test-only facitter dokumenteret til og med `d9d1c5f2`; `TD-156` er fortsat åbent |

## Aktuel evidensdelta – revision `d9d1c5f2`

Den seneste batch tilføjer to disjunkte test-only facitter uden produktændringer:

- `TD-280`: `td019MaanedsloenCanonicalAmountValidatorIndependent.test.ts` fastholder med 1/1
  negativ canonical månedsløn på `maanedsloenenUdgoer` med præcis feltsti, besked og severity.
- `TD-281`: `e2eServerIdentityLiveness.test.ts` fastholder med 1/1 fælles E2E-serveridentitet og
  rækkefølgen preflight → portoprydning → Playwright.

Den målrettede kontrol bestod med 2/2 nye tests. Typechecks, lint, encoding-, filnavns- og
diff-kontroller bestod. `verify:release:core` bestod derefter med 908 testfiler / 9.083 tests,
