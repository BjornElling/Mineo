import { normalizeCsvHeader, toCsvScalar } from './eoDebugCsv';
import { hashDebugValue } from './eoDebugHash';

export type DebugTableParityDiff = Readonly<{
  rowIndex: number;
  colId: string;
  expectedHash: string;
  actualHash: string;
...
type DebugTableColumnLike = Readonly<{
  id: string;
  header: string;
  getCell: (rowIndex: number) => unknown;
}>;

type DebugTableModelLike = Readonly<{
  columns: readonly DebugTableColumnLike[];
  rowCount: number;
  getRowKey: (rowIndex: number) => string;
}>;

const hash = (value: string): string => hashDebugValue(value);

const diff = (rowIndex: number, colId: string, expected: string, actual: string): DebugTableParityDiff => ({
  rowIndex,
  colId,
...
export const findFirstDebugTableParityDiff = (
  expected: DebugTableModelLike,
  actual: DebugTableModelLike
): DebugTableParityDiff | null => {
