...
| Baseline: antal kildefiler, testfiler, tests, E2E-specs og mutationsscore | Ved auditstart: 923 produktionsfiler (`.ts/.tsx`), 638 testfiler i alt, 8.421 Vitest-tests, 36 E2E-specs og 174 E2E-tests. Aktuel testrevision `bc3f0f3a`: 920 produktionsfiler, 664 Vitest-testfiler med grøn kørsel, 8.605 beståede tests samt 17 forventede `it.fails`, 42 E2E-specs og 185 beståede lane-tests. Mutationsrunneren er kvalificeret modulvist: money 56/58 dræbt, `dateCommit.ts` 4/6 dræbt, årsløn 108/113 dræbt, varige mén 94/114 dræbt, forsørgertab 21/21 dræbt og procesrente 104/136 dræbt med 23 triagerede survivors og 9 dokumenterede timeouts; alle ikke-dræbte mutationer er triageret. |
| Baseline: `test:coverage`-rapport og de dækkede/udeladte mapper | `coverage/` fra aktuel retest på `bc3f0f3a`: 393 instrumenterede filer i `src/domain`, `src/utils`, `src/hooks`, `src/rowDrafts` og `src/contexts`; 89,28 % statements, 80,50 % branches, 92,38 % functions og 92,07 % lines – 18.378 / 20.584, 12.954 / 16.090, 2.973 / 3.218 og 16.785 / 18.230 målte enheder. Den fulde coverage-kørsel gav 8.605 beståede tests og 17 forventede `it.fails`. Coverage-konfigurationen omfatter ikke de øvrige produktionsfiler; de skal klassificeres i inventaret. |
...
| PERSIST-002 | Browserlagre, settings, filhåndtag og sessionbevaring – `src/settings/**`, `src/utils/*Storage*`, `src/hooks/useFileSaveLoad.ts` | Reload, storage-fejl eller filkapabilitetsmangel kan nulstille input eller skjule fejl. | `persistence-contract.md`, `app-settings.md`, `app-shell-contract.md` | unit/integration/E2E | `settings/**`, `utils/safe*Storage.test.ts`, `fileHandleStorage.test.ts`, `utils/fileSystemAccess.test.ts`, `utils/fileHandleStorage.failurePaths.test.ts`, `hooks/useFileSaveLoad.test.tsx`, `hooks/usePersistedActiveTab.test.tsx`, `persistence-reload-session.spec.ts`, PWA-E2E | 15 fokuserede filer / 180 unit- og hook-tests samt 5/5 målrettede PWA-/reload-E2E-tests er grønne. Supplerende platformskontrol har 89 beståede tests og 6 forventede negative modcases for picker-adfærden. Reel input + aktiv fane + browser-reload er bevist; capability-detektion og fuld browser-/IndexedDB-sammenhæng står fortsat åbne i `TD-017`. | I gang |
...
| CALC-004 | Varige mén – `src/domain/varigemen/**`, `src/components/pages/VarigeMen.tsx` | Forkert méngrad, satsår, datoafgrænsning eller gate giver forkert erstatning eller dokument. | `varigemen-contract.md`, `date-contract.md`, `amount-contract.md` | unit/integration/E2E | `domain/varigemen/**`, `components/pages/varigemen/**`, `e2e/download-tooltip-classes.spec.ts` | Målrettet varige mén-flade bestod med 10 filer / 82 tests. Den dækker bl.a. méngrad 1/120/121, satsår, alderstrin 39–70, datoorden, afrunding, gate/projektion og en uafhængig totalsag. Command-runner-mutationsprøven dræbte 94/114 mutationer uden timeout/fejl; de 20 overlevere er triageret som ækvivalente eller redundante guards. Fuld E2E-rejse og outputparitet mangler. | I gang |
| CALC-005 | Forsørgertab – `src/domain/forsoergertab/**`, `src/components/pages/Forsoergertab.tsx` | Forkert kønsgren, periode, fælles årsløn eller difference ændrer resultatet. | `forsoergertab-snapshot-contract.md`, `date-contract.md` | unit/integration/E2E | `domain/forsoergertab/**`, `components/pages/Forsoergertab.integration.test.tsx`, `e2e/forsoergertabResterendePeriode.spec.ts` | Målrettet flade bestod med 8 filer / 83 tests. Den dækker snapshot-/reader-gates, kønsgrene, perioder, minimumssats, maksimum, restperiode, EAL/ASL-afhængigheder og en uafhængig totalsag. Command-runneren dræbte 21/21 mutationer uden timeout eller tekniske fejl; den blokerede EAL-grenes sentinelfelter er nu hævdet eksplicit. Fuld E2E-rejse og dokumentparitet mangler. | I gang |
...
| MIN-001 | Standalone MinProcesrente – `src/apps/minprocesrente/**`, shared renteberegning | Standalone kan blande hovedappens auth/settings/persistens ind eller vise et andet resultat end den delte engine. | `app-shell-contract.md`, `renteberegning-contract.md` | unit/integration/E2E | `apps/minprocesrente/**`, `quality/minprocesrenteStandaloneIsolation.test.ts`, `apps/minprocesrente/standaloneCalculatorPage.test.tsx`, `e2e/minprocesrente-recovery-and-focus.spec.ts`, `e2e/minprocesrente-valid-download.spec.ts`, `e2e/minprocesrente-namespace-isolation.spec.ts` | Isolation, namespace, error boundary, reset/fokus, valid beregning, aktive gates, faktisk PDF-download, exit-guard og browserbaseret namespace-/runtime-isolation er gennemgået. Den samlede valgte E2E-suite bestod med 185/185; resterende fuld outputparitet og mutation er fortsat åbne. | I gang |
| BUILD-001 | Build, entry points, lazy chunks, PWA-assets og scripts – `vite.*.config.ts`, `scripts/**`, `public/**`, `sw/**` | En release kan bygge grønt men levere forkert app, manglende asset, forkert chunk eller ikke-startbar PWA. | `app-shell-contract.md`, build-/release-scripts | statisk/build/E2E | `scripts/**`, `quality/pwaHeaders.test.ts`, `quality/architecture/rules/**`, `apps/shared/vitePreloadRecovery.test.ts`, E2E-buildserveren | `verify-build-artifacts.mjs` verificerer nu, at alle PWA-assetstier findes i `outDir`, og PWA-pluginet udelader Vites nul-byte CSS-facade-chunks, som ikke skrives til buildet. Syntetisk positiv/negativ test bestod 2/2, almindeligt Mineo-build bestod, E2E-buildserveren startede med EET-browserrejsen 6/6 grøn, og den eksakte produktionsbuilds lokale preview-smoke bestod 1/1. B-001 er lukket. Chunk-advarslen, Vite native-advarslen og den bredere CI-/deploy-artifactkobling står åbne til triage. | I gang |
...
| Felt | Udfyldes |
| --- | --- |
| Inventar-ID / gennemgået revision | `PERSIST-002` / arbejdsrevision efter `e867ab99` |
...
| Felt | Udfyldes |
| --- | --- |
| Inventar-ID / gennemgået revision | `INPUT-001`, `INPUT-002`, `VALID-001` / arbejdsrevision efter `e867ab99` |
...
| Kørte kommandoer, miljø og artefaktlink | `npm run test:mutation` – Node `v24.18.0`/Windows, én worker, omkring 2 minutter. Supplerende `npx --no-install stryker run .tmp-date-commit-stryker.json --logLevel info` kørte `dateCommit.ts`: 6 mutationer, 4 dræbt, 2 ækvivalente. Rapporter: `coverage/mutation/mutation.json` og `coverage/mutation/mutation.html` (ignorerede lokale artefakter). |
...
| Felt | Udfyldes |
| --- | --- |
| Inventar-ID / gennemgået revision | `DATA-001`, `CALC-001` / `5c88267f` |
...
| Eksisterende testfiler og testnavne | 23 filer / 477 tests under `src/__tests__/data`, `src/__tests__/domain/satser` og `quality/consumerInventory.test.ts`. De dækker katalogets 14 registrerede kilder, fingerprints, huller, overlap, sortering, bounds, missing-entry og udvalgte historiske særtilfælde. `uafhaengigSatsFacitmatrix.test.ts` har nu literal-facit for udvalgte KRL-, offentlig løn- og overenskomstperioder samt de tidligere lovbestemte satser, referencesats og ILON12/SBLON2-kvartaler. `uafhaengigeDataEndepunkter.test.ts` tilføjer 25 statiske endpoint-/partitionfacitter. `satserDownstreamIndependentOracle.test.ts` følger desuden en statisk 2025/2026-reguleringsfacit gennem beregning og dokumenttabel. `td020KrlDownstreamIndependent.test.ts` er en separat downstream-test med 1/1 test. |
...
| Kørte kommandoer, miljø og artefaktlink | `npx vitest run src/__tests__/data/uafhaengigSatsFacitmatrix.test.ts src/__tests__/domain/satser/satserDownstreamIndependentOracle.test.ts --reporter=dot` – de uafhængige facitspor bestod. `npx vitest run src/__tests__/data/uafhaengigeDataEndepunkter.test.ts --reporter=dot` – 25/25. `npx vitest run src/__tests__/domain/data/td020FolkepensionDownstream.test.ts --reporter=dot` – 2/2. `npx vitest run src/__tests__/domain/data/td020KrlDownstreamIndependent.test.ts --reporter=dot` – 1/1. Den samlede data-/satssuite bestod med 23 filer / 477 tests på Node `v24.18.0`/Windows; KRL-downstreamprøven er kørt separat. |
| Fund-ID'er, beslutninger og opfølgning | `TD-020` er delvist lukket for de statisk bevægelige endpoints og de nu to målrettede downstream-facitspor. Validatorpartitioner og komplet konsumentparitet står åbne. Sygedagpenge-2005-afvigelsen er fortsat et afklaringspunkt; runtime-data eller beregningsændringer forelægges udvikleren. |
| Reviewer / dato / slutstatus | Codex / 2026-09-10 / `I gang` |
...
| Felt | Udfyldes |
| --- | --- |
| Inventar-ID / gennemgået revision | `CALC-001` / `e867ab99` |
...
| Eksisterende testfiler og testnavne | `aslAarsloensmaksimum.test.ts`, `opreguleringsmotorer.test.ts`, `satserNewCaseSeed.test.ts`, `satserProjection.test.ts`, `uafhaengigeDataEndepunkter.test.ts` og `satserDownstreamIndependentOracle.test.ts`. |
| Manglende test, svage assertioner eller dubletter | Den målrettede domænesuite har konkrete facit for begge reguleringsmetoder, ikke-heltal/NaN, manglende endepunkter og mellemår, samt projektionens missing/format/bounds. Det nye downstream-orakel dækker én statisk 2025/2026-kæde til beregning og dokumenttabel. Integration, E2E, øvrig data-kildeparitet og fuld downstream-konsumentmatrix mangler. |
...
| Kørte kommandoer, miljø og artefaktlink | `npx vitest run src/__tests__/data src/__tests__/domain/satser src/__tests__/quality/consumerInventory.test.ts --reporter=dot` – 23 filer / 477 tests bestået på Node `v24.18.0`/Windows. `td020KrlDownstreamIndependent.test.ts` bestod separat med 1/1. Vite udsendte den kendte `configLoader: 'native'`-advarsel. |
| Fund-ID'er, beslutninger og opfølgning | `TD-020` er delvist lukket med de uafhængige endpointfacitter og downstream-facitprøven. Fortsæt med komponent-/E2E-rejse, validatorpartitioner og komplet downstream-paritet. |
| Reviewer / dato / slutstatus | Codex / 2026-09-10 / `I gang` |
...
| Felt | Udfyldes |
| --- | --- |
| Inventar-ID / gennemgået revision | `CALC-004` / `e8faa7ce` |
...
| Kørte kommandoer, miljø og artefaktlink | `npx vitest run src/__tests__/domain/varigemen src/__tests__/components/pages/varigemen --reporter=dot` – 10 filer / 82 tests bestået – samt `npx --no-install stryker run .tmp-varigemen-mutation.json --logLevel info`; Node `v24.18.0`/Windows. Den samlede CALC-004/CALC-005-kontrol bestod med 18 filer / 164 tests. Rapport: `coverage/mutation/varigemen-mutation.json` (ignoreret lokalt artefakt). Vite udsendte den kendte `configLoader: 'native'`-advarsel. |
...
| Felt | Udfyldes |
| --- | --- |
| Inventar-ID / gennemgået revision | `CALC-005` / `5c88267f` |
...
| Manglende test, svage assertioner eller dubletter | Den nye uafhængige totalsag i `forsoergertabIndependentOracle.test.ts` fastholder håndberegnede EAL-/ASL-facitter, kønsgren, dagbrøk, maksimum, difference og nettokrav gennem projection og download-gate. Mutationstesten viste desuden, at en blokeret EAL-grenes tomme/null-sentinelfelter skulle hævdes eksplicit. Den samlede gennemgang skal stadig vurdere overlapning samt PDF-/Word-/E2E-paritet. |
...
| Manglende test, svage assertioner eller dubletter | Den målrettede suite har bred branch-, issue-, dependency- og gate-dækning, og det nye orakel hævder EAL, ASL-/løbende ydelse, differencekrav og alle fire gates med konkrete beløb. `eetPageAudit.spec.ts` følger nu løbende ydelser og kapitalisering fra synligt resultat til Word-indhold; differencekrav, øvrige outputvarianter og PDF-paritet mangler fortsat. |
...
| Kørte kommandoer, miljø og artefaktlink | `npx vitest run src/__tests__/domain/erhvervsevnetab src/__tests__/components/pages/Erhvervsevnetab.integration.test.tsx --reporter=dot` – 23 filer / 420 tests bestået på Node `v24.18.0`/Windows. `npx vitest run src/__tests__/domain/erhvervsevnetab/eetIndependentOracle.test.ts --reporter=dot` – 1/1. De seks EET-specs `eetEfterEalSpecifikation`, `eetForligAnsvarsgrad`, `eetIssueDeduplication`, `eetLoebendeYdelserSpecifikation`, `eetPageAudit` og `eetStamdataDependency` bestod med 24/24; `eetPageAudit.spec.ts` bestod med 6/6 i `npm run test:e2e -- e2e/eetPageAudit.spec.ts --project=chrome-desktop`. Vite udsendte den kendte `configLoader: 'native'`-advarsel. |
| Fund-ID'er, beslutninger og opfølgning | Den uafhængige totalsag lukker det konkrete facit-hul. `eetPageAudit.spec.ts` følger nu løbende ydelser og kapitalisering fra synligt resultat til Word-indhold, mens `TD-018` fortsat registrerer den manglende engine → snapshot → UI → dokumentrejse for øvrige faner og generel PDF↔Word-paritet. Tilføj kun test-only dokumentassertions næste gang; eventuelle ændringer af EET-tal eller gateadfærd forelægges udvikleren før implementering. |
| Reviewer / dato / slutstatus | Codex / 2026-09-10 / `I gang` |
...
| Felt | Udfyldes |
| --- | --- |
| Inventar-ID / gennemgået revision | `DOC-001` / `1956eb8d` |
...
| Kørte kommandoer, miljø og artefaktlink | Fokuseret lifecycle-/katalogkontrol: `npx vitest run src/__tests__/document/documentDefinitionFixtureRegistry.test.ts src/__tests__/document/documentLifecycleMatrix.test.ts src/__tests__/document/documentCatalogCompleteness.test.ts --reporter=dot` – 3 filer / 66 tests bestået. Supplerende `npx vitest run src/__tests__/document/documentLifecycleMatrix.test.ts src/__tests__/document/documentLifecycleCoordinatorIntegration.test.tsx --reporter=dot` – 2 filer / 23 tests bestået. Den samlede dokumentmappe bestod med 25 filer / 243 tests på Node `v24.18.0`/Windows. |
...
| Felt | Udfyldes |
| --- | --- |
| Inventar-ID / gennemgået revision | `DOC-002`, `DOC-003` / `1956eb8d` |
...
| Eksisterende testfiler og testnavne | 25 filer / 162 tests dækker PDF-/Word-generatorgrene, tekst/tal, valgfri sektioner, filnavne, PDF-writerens cursor/spacing/tabel/pagebreak og Word-blobs pakket ud som ZIP/XML. `documentTextChannelParity.test.ts` kører en reel Varige mén-generator gennem PDF-writerens tekstkald og Word-writerens `document.xml` og sammenholder den normaliserede tekst i rækkefølge. `documentGateFormatInvariance.test.ts` dækker gate/routing-invarians. `eetPageAudit.spec.ts` downloader nu Word-dokumenter gennem hovedappens formatvalg og kontrollerer løbende ydelser og kapitalisering med konkrete synlige værdier i OOXML. |
| Manglende test, svage assertioner eller dubletter | PDF-goldens bruger mocked jsPDF/autoTable og syntetisk cellegeometri, Word-goldens snapshotter rå XML, og EO-goldens bygger forventningsgrundlag via produktionsprojektion. `documentTextChannelParity.test.ts` lukker den tekstlige kanalparitet for ét output, men EET-E2E-beviset og testen kontrollerer fortsat ikke PDF-bytes. Der mangler reel PDF-parse/render, Word-åbning/OOXML-validering ud over ZIP/XML-strukturen og en fuld semantisk PDF↔Word-paritet for alle outputs, herunder headers/footers og rækkefølge. Lokalt er Poppler/LibreOffice og tilsvarende parser/render-værktøjer ikke installeret; det er registreret som et konkret restpunkt, ikke som bestået fysisk bevis. |
| Valgt testniveau og begrundelse | Unit/golden er velegnet til generator- og layoutinvarianter, men fysisk artefaktkvalitet kræver parsing og rendering af de faktiske bytes. Den nye tekstparitetsprøve bruger samme produktionsgenerator gennem begge writers og sammenholder det observerbare tekstforløb; den er et kanaldriftsværn, ikke et uafhængigt domænefacit. Tværgående semantic parity for alle outputs skal fortsat bruge et fælles, uafhængigt facit, ikke to snapshots af hver sin writer. |
| Mutation/modprøve og resultat | Der er ikke kørt mutation på dokumentfladen. De eksisterende 162 tests er grønne, men den fysiske PDF-/Word-grænse og semantiske parity er højere-niveau-beviser. |
| Kørte kommandoer, miljø og artefaktlink | `npx vitest run src/__tests__/document/documentTextChannelParity.test.ts src/__tests__/docx/generators/varigeMenWordContent.test.ts src/__tests__/document/tableChannelParity.golden.test.ts --reporter=dot` – 3 filer / 23 tests bestået på Node `v24.18.0`/Windows. `npx tsc -p tsconfig.test.json --noEmit` og `npx eslint src/__tests__/document/documentTextChannelParity.test.ts` bestod. `npm run test:e2e -- e2e/eetPageAudit.spec.ts --project=chrome-desktop` viste tidligere 6/6 beståede tests, herunder faktisk Word-download med løbende ydelser og kapitalisering samt OOXML-indhold. Fysisk PDF-parse/render og generel PDF↔Word-paritet er ikke kørt. |
| Fund-ID'er, beslutninger og opfølgning | `TD-012` er lukket for per-output lifecycle. `TD-013`–`TD-014` dækker fortsat lifecycle-matrix og semantisk parity; den fysiske PDF-/Word-parse/render-risiko er fortsat åben. En ny parser-/renderer-dependency skal begrundes særskilt, men kræver ikke ændring af produktkode i sig selv. |
| Reviewer / dato / slutstatus | Codex / 2026-09-10 / `I gang` |
...
| Felt | Udfyldes |
| --- | --- |
| Inventar-ID / gennemgået revision | `ARCH-003` / `5c88267f` |
...
| Kørte kommandoer, miljø og artefaktlink | `npx vitest run src/__tests__/quality/e2eSuiteConventions.test.ts src/__tests__/quality/githubActionsRuntimeCheck.test.ts --reporter=dot` – 25/25 tests bestået; `npm run check:e2e-lanes` og `npm run check:github-actions-runtime` – bestået på Node `v24.18.0`/Windows. `npm run verify:release:core` bestod på `83abfdad` med dependency-, runtime-, typescript-, lint-, data-, kontrakt-, ledger-, coverage- og build-gates. |
| Fund-ID'er, beslutninger og opfølgning | `TD-006` er lukket for den konkrete lane-parserfejl. Runtime-værnet er styrket mod vakuøs grøn status. B-002 om separat E2E-/deploy-artefakt og øvrige release-/CI-spor står fortsat åbne. |
| Reviewer / dato / slutstatus | Codex / 2026-09-11 / `I gang` |
...
| Manglende test, svage assertioner eller dubletter | Før ændringen kontrollerede scriptet assetmanifestets format og sti-regex, men ikke at stierne fandtes i buildet. Den negative regressionstest beviser nu den manglende asset-vej. Den integrerede E2E-build afslørede desuden fem nul-byte CSS-facade-chunks, som Vite havde i `bundle`, men ikke skrev til `dist`; de bliver nu filtreret fra PWA-manifestet. CI har nu en særskilt smoke mod deploy-artefaktet med robust proceskontrol, men den fulde E2E-suite mod samme artefakt og øvrige lazy chunk-referencer er fortsat ikke fuldt dækket under B-002. |
...
| Kørte kommandoer, miljø og artefaktlink | `npx vitest run src/__tests__/quality/verifyBuildArtifacts.test.ts --reporter=dot` – 2/2 tests bestået; `npm run build:mineo` bestået; `npm run test:e2e -- e2e/eetPageAudit.spec.ts --project=chrome-desktop` bestået med 6/6 tests på Node `v24.18.0`/Windows. |
| Fund-ID'er, beslutninger og opfølgning | B-001 er lukket som `TD-005`. B-002 er delvist dækket af en lokal smoke mod det eksakte produktionsbuild og en CI-jobsektion, der starter samme artefakt med fast port, timeout og cleanup; faktisk workflowkørsel og fuld E2E mod deploy-artefaktet kræver fortsat særskilt releaseafklaring. |
| Reviewer / dato / slutstatus | Codex / 2026-09-10 / `I gang` |
...
| --- | --- | --- | --- | --- | --- |
| Samlet Vitest-flade på `bc3f0f3a` efter seneste audit-testene | Ja | 89,28 % / 80,50 % / 92,38 % / 92,07 % – 18.378 / 20.584, 12.954 / 16.090, 2.973 / 3.218 og 16.785 / 18.230 målte enheder | 664 testfiler / 8.605 tests bestod; 17 forventede `it.fails` dokumenterer de kendte ikke-callable File API-properties og ugyldige `Date`-prober. Coverage viste fortsat lave platform-/storagehelpers; det er ikke behandlet som en grund til at hæve eller sænke tærskler. | relevant / højere niveau | `npm run test:coverage` på Node `v24.18.0`/Windows, 450,68 s. Rapport: `coverage/index.html`, `coverage/coverage-final.json` og `coverage/clover.xml` (lokale, ignorerede artefakter). De 393 instrumenterede filer omfatter `src/domain`, `src/utils`, `src/hooks`, `src/rowDrafts` og `src/contexts`; resterende lave platformflader er triageret som højere-niveau-bevis og åbne fund `TD-017`/`TD-022`. |
...
| Platform-/persistens- og dokumentgrænser | Ja | `fileSystemAccess.ts` 47,69 / 50 / 62,5 / 49,2; `fileHandleKvStore.ts` 30,30 / 0 / 0 / 31,25; `fileHandleStorage.ts` 97,77 / 66,66 / 100 / 100; `fileHelpers.ts` 44,73 / 10,44 / 41,17 / 45,94 | De lave tal svarer til browser-/IndexedDB-/File API-grene, som primært er mockede eller kun rammes af E2E-fallbacks. Det er et reelt højere-niveau-hul, ikke automatisk død kode. | relevant / højere niveau | Platformsevidens er tilføjet under `PERSIST-002`; fysisk PDF-/Word-parse/render og semantisk paritet er fortsat åbent under `DOC-002`/`DOC-003`. |
...
- [x] `npm run test:coverage` er kørt på den aktuelle auditrevision `bc3f0f3a` og rapporteret med instrumenteret filmængde; den tidligere releasekørsel på `83abfdad` står som historisk sammenligningspunkt.
...
| `CALC-004` | Varige mén-engine, projection og gate | Håndberegnet totalsag med særskilt aldersreduktion og afrunding | `10.530 × 37`, 22 % reduktion og ceil-afrunding | Engine → projection → gate giver `303.896` kr.; målrettet flade: 82/82; mutation: 94/114 dræbt | Delvist – browserrejse og dokumentparitet åbne |
| `CALC-005` | Forsørgertab-engine og snapshot-gates | Håndberegnet ASL/EAL-totalsag med maksimumsbegrænsning | `1.000.000 × 10`, 2013-maksimum `8.432.000`, 30 % EAL og dagbrøk | Håndfacit giver nettokrav `2.158.185` kr.; målrettet flade: 83/83; mutation: 21/21 dræbt | Delvist – browserrejse og dokumentparitet åbne |
...
| `CALC-007` | EET-totaler, snapshot og dokumentgates | Håndberegnet samlet totalsag samt tværgående tabelkanal | EAL, ASL/løbende ydelse, differencekrav og EET-løbende-ydelser | `eetIndependentOracle.test.ts`: 1/1; PDF-resolved table cells matcher Word XML for 19 cases; EET-Word-E2E har konkrete indholdsassertions for løbende ydelser og kapitalisering | Delvist – øvrige outputs og fysisk PDF/Word-paritet åbne |
...
| `DOC-001` / `DOC-003` | Dokumentdefinitioner, gates, lifecycle og Word-writer | Separat lifecycle-koordinator-test og resolved PDF-tabelceller mod Word `document.xml` | Alle 21 registrerede outputs for lifecycle; EET-tabelcase for kanalparitet | Lifecycle-/coordinator: 23/23; dokumentgruppe: 29/29; Varige mén-tekstparitet: 1/1; `verify:release:core` og E2E er grønne | Delvist – generel semantisk og fysisk PDF/Word-paritet åben |
| `SHELL-001` / `MIN-001` | Browserrejser på valgte motorer/viewporter | Samme brugerinput gennem login, reload, standalone-navigation og hard-stop | Chrome, Edge, Firefox, WebKit, markerede viewporter, fallback og touch-hard-stop | Fuld valgt E2E: 185/185; namespace-isolation: 1/1; runtimeErrors og eksterne requests kontrolleret; lazy-chunk recovery: 4/4 | Bestået med rest-risiko – native launchQueue og fuld PWA-/lazy-recoveryparitet er ikke afsluttet |
...
| B-002 | 2026-09-11 | `BUILD-001` / `ARCH-003` | Høj | CI's fulde E2E-kørsel bygger fortsat et separat `--mode e2e`-artefakt og beviser derfor ikke alene, at deploy-artefaktet kan starte og gennemføre en brugerrejse. En fejl i den almindelige produktionsbuild kan dermed overleve E2E-gaten. | `production-artifact-smoke.spec.ts` gennemfører synligt login, route-mount, runtime-fejlcheck og ekstern-request-check mod det uploadede build. CI-workflowen starter nu `vite preview` med `--strictPort`, kontrollerer at processen lever, bruger afgrænsede curl-timeouts og rydder processen op. Lokal smoke mod det eksakte produktionsbuild bestod med 1/1; GitHub Actions-kørsel mod den nye jobsektion er endnu ikke foretaget i auditmiljøet. | Delvist lukket. Den særskilte production-artifact-smoke er koblet på CI-workflowen som test-only releaseværn. Den fulde E2E-suite mod samme deploy-artefakt og en faktisk CI-kørsel mangler fortsat; ingen produktkode eller brugeradfærd er ændret. | Ikke påkrævet for test-/CI-ændringen. | Delvist retestet efter `bc3f0f3a`: lokal production smoke 1/1 og `check:github-actions-runtime` bestået; CI-/deploykobling skal bekræftes ved en faktisk workflowkørsel. |
...
| TD-014 | 2026-09-11 | `DOC-001` | Mellem | Der findes ingen generel semantisk PDF↔Word-paritetstest for alle 21 outputs. Udvalgte goldens og isolerede Word-tests kan derfor ikke bevise, at samme sag giver samme labels, tal, sektioner og rækkefølge i begge kanaler. | `documentGateFormatInvariance.test.ts` håndhæver primært struktur og gate-formatadskillelse. `eetPageAudit.spec.ts` downloader nu Word gennem hovedappens formatvalg og kontrollerer titel, beløb, procent og afgørelsessektion i OOXML. `documentTextChannelParity.test.ts` sammenholder desuden Varige méns fulde tekstforløb fra PDF-writerens faktiske tekstkald med Word-writerens `document.xml` og bestod med 1/1. | Åbent. Varige mén-prøven er et repræsentativt kanalbevis, ikke generel dækning. Fuld PDF-parse/render og normaliseret tværgående paritet mangler fortsat; eventuel ny parser-/renderer-dependency skal begrundes særskilt. | Ikke påkrævet for testændringen. | Delvist retestet – Varige mén-tekstparitet 1/1 og EET-Word-bevis bestået, generel paritet åben |
| TD-017 | 2026-09-11 | `PERSIST-002` | Høj | File System Access-checken ser kun efter properties og kan derfor vælge picker-grenen, selv om picker-værdien er `undefined` eller ikke-callable. Hook-/storage-/File API-lagene er fortsat kun delvist bevist sammen. | `fileSystemAccess.test.ts`, `fileHandleStorage.failurePaths.test.ts` og den udvidede `indexedDbStore.test.ts` dækker unavailable-, exception-, request- og transaction-fejl med 89 beståede tests. Seks `it.fails` reproducerer de ikke-callable picker-properties. Browser-reload med afsluttet input og aktiv fane består med 1/1. | Åbent. Testlaget dokumenterer nu fejlvejene og reproducerer capability-afvigelsen. Ændring af capability-detektion eller fallback forelægges udvikleren, fordi brugerens filvalg-/fejloplevelse ændres. | Ja – produktændring i File API/fallback kræver udviklerens godkendelse; test-only browserbevis gør ikke. | Delvist retestet – fejlveje og reload bestået, produktfund åbent |
| TD-018 | 2026-09-11 | `CALC-007` | Høj | EET har bred unit-/snapshot-/gate-dækning, men ingen generel kontrol af engine → snapshot → UI → dokumentindhold. Differencekravsfanen har fortsat ikke en asymmetrisk dokumentrejse, og PDF-paritet mangler. | `eetIndependentOracle.test.ts` bevarer den samlede håndfacitprøve. `eetPageAudit.spec.ts` downloader nu Word fra hovedappen og kræver løbende ydelsers titel, afgørelsesdata, beløb og faktisk `.docx`-indhold samt kapitaliseringens synlige afgørelse og samme beregnede kapitalbeløb i `word/document.xml`. Den samlede spec bestod med 6/6; den fulde valgte E2E-suite bestod med 185/185 på `1956eb8d`. | Delvist lukket for EET-Word-download og observerbart dokumentindhold i to outputvarianter. Fuld engine → snapshot → UI → dokumentkæde for alle relevante faner og PDF↔Word-paritet er åben. | Ikke påkrævet for testændringen; ingen produktkode er ændret. | Delvist retestet – `eetPageAudit.spec.ts` 6/6 og fuld valgt E2E 185/185 på `1956eb8d`, samlet fund åbent |
| TD-019 | 2026-09-11 | `INPUT-001` / `INPUT-002` / `VALID-001` | Mellem | Form/grid-pariteten er nu bundet til en reel produktionsdescriptor, nested row-schemaernes unknown/missing/invalid-partitioner er dækket, og faktiske `ZodError.issues` bruges. Flere validatorfixtures genbruger fortsat produktionsdefaults. | `erstatningsopgoerelseValidator.independent.test.ts` tilføjer 10 små, literalbaserede cases med konkrete `ZodError.issues` og uden runtime-defaults. `td019ValidatorSchemaIndependence.test.ts` tilføjer en komplet, håndskrevet typed runtime-fixture med 1/1 domænegrænsecase. Den samlede uafhængige validatorprøve bestod med 11/11; den øvrige validatorflade genbruger fortsat enkelte produktionsfixtures. | Delvist lukket. Yderligere validator- og downstream-partitioner skal fortsat vurderes; ingen runtime- eller loadændring er foretaget. | Ikke påkrævet for testændringen, så længe runtime-/loadadfærd ikke ændres. | Delvist lukket efter `bc3f0f3a` – 11 uafhængige cases |
| TD-020 | 2026-09-11 | `DATA-001` / `CALC-001` | Mellem | Data-/sats-suiterne har stærke katalog-, fingerprint- og integritetsguards, men flere facitassertions læser produktionsdata som forventning. Validatorpartitioner og komplet downstream-paritet er derfor ikke uafhængigt bevist. | `uafhaengigeDataEndepunkter.test.ts` tilføjer 25 statiske endpoint-/partitionfacitter med historiske og nyere grænser. `td020FolkepensionDownstream.test.ts` tilføjer 2/2 statiske folkepensionsfacitter for fem perioder og manglende dækning. `td020KrlDownstreamIndependent.test.ts` tilføjer 1/1 statisk KRL-region-downstreamfacit. Den uafhængige satsmatrix og downstream-kæde består fortsat med den samlede data-/satssuite på 23 filer / 477 tests. Sygedagpenge-2005-afvigelsen er fortsat uafklaret. | Delvist lukket for de registrerede statiske endpoints og de konkrete folkepensions- og KRL-forbrugere. Åbent for validatorpartitioner og komplet konsumentparitet; runtime-data/validatorer ændres ikke som del af auditten. | Ikke påkrævet for test-only styrkelse. | Delvist lukket – endpoint- og downstream-facitter tilføjet, øvrige downstream-huller åbne |
...
- [x] Relevante kommandoer er kørt rent i et reproducerbart miljø: typechecks, lint, Vitest/coverage,
