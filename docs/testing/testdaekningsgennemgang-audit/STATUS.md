...
- Evidensrevision: `1e0dfd6a` for seneste samlede releasegate og målrettede test-only batch; seneste fulde E2E er `71746917`
- Branch: `main`
...
- Seneste test-only batch: DATA-/PERSIST-facitter TD-241–TD-242; målrettet kontrol 2 nye tests / 2 tests; ingen produktkode, beregningslogik, UI/UX eller persistens ændret.
...
| `PERSIST-002` | Storage/settings, filhåndtag, PWA-filflows og reel session-reload gennemgået | 19 filer / 184 målrettede unit-/hook-tests; PWA-/filkørsel 4/4; reel input + aktiv fane + reload 1/1; supplerende 8-filers platformssuite med 89/89 beståede tests efter callable picker-detektion; `safeLocalStorage.test.ts` bestod med 11/11 direkte Node-fallback-isolationsassertions; `fileHandleVerification.test.ts` bestod med 22/22; `fileHandleKvStore.test.ts` bestod med 2/2; `fileOperationClientSession.persistence.test.ts` tilføjer 1/1 sessionStorage-klientidentitetsfacit efter simuleret reload; `fileSaveTarget.test.ts` tilføjer 1/1 orphaned-handle-facit før picker-flowet; `fileSaveTargetPermissionApiFailure.test.ts` tilføjer 1/1 permission-API-facit med handle-oprydning og picker-fallback; `fileHandleStorage.pendingRequestValidation.test.ts` tilføjer 1/1 fail-closed-facit for negativt ignoredFileCount; `indexedDbStore.readRequestFailure.test.ts` tilføjer 1/1 readonly get-fejlfacit med logning og forbindelseslukning; `fileHandleStorage.persistenceGrant.test.ts` tilføjer 1/1 facit for `navigator.storage.persist()`-succes uden warning; ugyldige `Date`-inputs har 8/8 grønne negative modcases | Fuld browser-/platformssammenhæng, herunder installeret PWA-filaflevering; `TD-022` | `I gang` |
...
Aktuel inventarrevision på `1e0dfd6a`: `PERSIST-002` omfatter nu også `fileHandleStorage.persistenceGrant.test.ts`
...
| Senest tilføjet lukket | `TD-153`, `TD-154`, `TD-155`, `TD-157`–`TD-242` – test-only facitter dokumenteret til og med `1e0dfd6a`; `TD-156` er fortsat åbent |

## Aktuel evidensdelta – revision `1e0dfd6a`
