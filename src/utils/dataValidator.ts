 ...
 */
const validateSection = (section: unknown, sectionName: string): FileValidationResult => {
  const errors: ValidationError[] = [];
  const warnings: ValidationError[] = [];

  // Tjek at sektion er et objekt
  if (typeof section !== 'object' || section === null) {
    errors.push({ path: sectionName, message: 'Sektion er ikke et objekt' });
    return { isValid: false, errors, warnings };
  }

  // Tjek array (tilladt, men skal være top-level objekt, ikke array)
  if (Array.isArray(section)) {
    errors.push({ path: sectionName, message: 'Sektion må ikke være et array (skal være et objekt)' });
    return { isValid: false, errors, warnings };
  }

  // Tjek dybde (beskyttelse mod stack overflow)
  if (!validateDepth(section)) {
    errors.push({ path: sectionName, message: 'For dyb nesting eller for mange elementer' });
    return { isValid: false, errors, warnings };
  }
    ...
    if (valueType === 'string') {
      if (!isValidString(value)) {
        errors.push({
          ...
          message: `String for lang (max ${MAX_STRING_LENGTH} tegn)`,
        });
      }
      ...
      if (key.toLowerCase().includes('dato') && value !== '' && !isValidDate(value)) {
        errors.push({
          path: `${sectionName}.${key}`,
          message: 'Ugyldig dato-format (skal være dd-mm-åååå eller tom)',
        });
      }
      ...
      if (!isValidNumber(value)) {
        errors.push({
          path: `${sectionName}.${key}`,
          message: 'Ugyldigt tal (Infinity/NaN ikke tilladt)',
        });
      }
      ...
      if (!validateDepth(value)) {
        errors.push({
          path: `${sectionName}.${key}`,
          message: 'For dyb nesting eller for mange elementer',
        });
      }
      ...
      errors.push({
        path: `${sectionName}.${key}`,
        message: `Ulovlig type '${valueType}' (function/symbol ikke tilladt)`,
      });
    }
 ...
 */
const validateAarsloen = (aarsloenData: unknown): FileValidationResult => {
  const errors: ValidationError[] = [];
  const warnings: ValidationError[] = [];

  // Tjek at det er et objekt
  if (typeof aarsloenData !== 'object' || aarsloenData === null) {
    errors.push({ path: 'aarsloen', message: 'Sektion er ikke et objekt' });
    return { isValid: false, errors, warnings };
  }

  // Tjek array (tilladt i årsløn-sektion)
  if (Array.isArray(aarsloenData)) {
    errors.push({ path: 'aarsloen', message: 'Sektion må ikke være et array (skal være et objekt)' });
    return { isValid: false, errors, warnings };
  }

  // Tjek dybde
  if (!validateDepth(aarsloenData)) {
    errors.push({ path: 'aarsloen', message: 'For dyb nesting eller for mange elementer' });
    return { isValid: false, errors, warnings };
  }
  ...
  if (data.loenperiode !== undefined && data.loenperiode !== null) {
    ...
    if (typeof data.loenperiode !== 'string' || !validPeriods.includes(data.loenperiode)) {
      errors.push({
        path: 'aarsloen.loenperiode',
        message: 'Ugyldig lønperiode (skal være "maaned", "uge" eller "dag")',
      });
    }
  }
  ...
  if (data.tableData !== undefined && data.tableData !== null) {
    if (!Array.isArray(data.tableData)) {
      errors.push({ path: 'aarsloen.tableData', message: 'Skal være et array' });
    } else {
      // Tjek array-størrelse
      if (data.tableData.length > MAX_ARRAY_LENGTH) {
        errors.push({
          path: 'aarsloen.tableData',
          message: `For mange elementer (max ${MAX_ARRAY_LENGTH})`,
        });
      }
      ...
      for (let i = 0; i < Math.min(data.tableData.length, 10); i++) {
        ...
        if (typeof row !== 'object' || row === null) {
          errors.push({
            path: `aarsloen.tableData[${i}]`,
            message: 'Element er ikke et objekt',
          });
        }
      }
    }
 ...
 */
export const validateFileData = (data: unknown): FileValidationResult => {
  const errors: ValidationError[] = [];
  const warnings: ValidationError[] = [];

  // 1. Tjek at data er et objekt
  if (typeof data !== 'object' || data === null) {
    errors.push({ path: '(root)', message: 'Data er ikke et objekt' });
    return { isValid: false, errors, warnings };
  }
  ...
  if (!validateDepth(data)) {
    errors.push({ path: '(root)', message: 'Data-struktur er for dybt nested eller har for mange elementer' });
    return { isValid: false, errors, warnings };
  }
  ...
  for (const section of dataSections) {
    if (!ALLOWED_SECTIONS.includes(section)) {
      warnings.push({
        path: section,
        message: `Ukendt sektion '${section}' ignoreres`,
        severity: 'warning',
      });
    }
  }
