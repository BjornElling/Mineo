...
**Dato:** 2026-07-01
**Grundlag:** Systematisk gennemgang af `code-review-plan.md` + samtlige 70 review-docs i `docs/review/`, krydset med egne kode-undersøgelser (verifikation/udfordring af review-fundene mod den *nuværende* kode, ikke kun review-teksten). Opdateret 2026-07-01 efter review af den aktuelle implementering af A1/A4/A5/B9/B8/C17 samt efterfølgende brugerrettelser.
...
- **A4 download-gate-adoption** — forelæggelses-pligtig; **B9 er efterfølgende lukket**, se shortlist.
...
|---|---|---|
| **A1** | ✅ Loenindkomst-opfølgningen lukket 2026-06-30 | (Historik:) `useLoenindkomstViewModel` var en god-hook flyttet bag en kontekst uden isolations-tests, og VM'en lækkede rå `eoValues`. **Lukket 2026-06-30:** den fulde afledning er udskilt til en React-fri `deriveLoenindkomstVm`, rå `eoValues` (og rå `stamdataValues`) er fjernet fra konteksten, og isolations-tests realiserer VM-lagets primære gevinst (afledning testbar uden render). **Afgrænsning (verificeret 2026-06-30):** den fulde React-fri afledningsmodel gælder **kun** Loenindkomst. `EOOplysninger` har et `useEoOplysningerViewModel` + sektioner via kontekst (prop-boring fjernet), men eksponerer fortsat rå `values`/`setValues`/`setFieldValue` gennem konteksten — det er den **kanoniske `usePersistedForm`-redigeringsflade** (hver sektion redigerer sine egne felter), ikke en aflednings-lækage som Loenindkomsts tidligere rå `eoValues`. Det er en bevidst, accepteret A1-rest (dokumenteret i `eoOplysningerContext.ts`), ikke et åbent restpunkt. `EOberegning` er bevidst ikke yderligere splittet. |
| **B9** | ✅ implementeret, men med ny residual-kandidat | Lukket 2026-06-25 via single-source relocation: række-evaluerings-motoren er flyttet fra `domain/debug/` til autoritativt `domain/eoRowEvaluation/`, gaten bruger den derfra, DEV-debug er nedstrøms, over-block-fixet er anvendt, og parallel `eoBlockingValidation` er fjernet. Eftergennemgang 2026-06-29 lukkede status-/kontraktdrift og tilføjede direkte regressionstest. **Nyt fund 2026-06-30:** relocationen løser debug-lagets placering, men ikke at EO-output-gating stadig er sammensat af snapshot-projektion + row-evaluering + EET-issues i `useEoBeregningViewModel`; se A5. |
...
| **B8** | ✅ implementeret (afgrænset omfang) | Type-seglet er ægte og implementeret (`Omit`-præsentationstyper, ingen cast/`any`; section-byggerne bærer ikke længere totaler — TAF-byggeren returnerer nu også `TabtArbejdsfortjenesteSectionPresentation`, jf. 2026-06-30). **Hvad seglet dækker — og bevidst ikke:** `buildEoComputedTotals` *viderebringer* engine/sektion-totaler (clamp + forlig-skalering); den re-deriverer dem ikke uafhængigt. Seglet stopper derfor divergens-ved-tastefejl/parallel-genberegning, men ikke divergens-ved-engine-fejl (et forkert engine-tal flyder identisk til begge). Ægte krydsudledning ville kræve en anden engine og er ikke berettiget mod den låste flade. Omfanget er afgrænset, men arbejdet er gjort — derfor ✅, ikke 🟡. |
...
1. **A5 — saml EO-output-gating i én autoritativ issue-/output-projektion.** ✅ **GENNEMFØRT 2026-07-01.** Gate-beslutningen for de fire EO-dokumenter ejes nu af den rene `evaluateEoDocumentDownloadGate` (ét output-gate-resultat pr. dokument); `useEoBeregningViewModel` kalder den og videregiver resultatet til `documentService`, som nu fail-closer på det (tidligere fail-open for `collectAllEoRows`-række-fejl). Den lokale `createPdfGate` + de fire duplikerede `disabledReason`-memo er fjernet. Se efter-review-runde 2026-07-01.
2. **B9 — træk produktions-blokerings-validering UD af debug-laget.** ✅ **GENNEMFØRT 2026-06-25** — via **single-source relocation** (ikke en parallel `eoBlockingValidation`): række-evaluerings-motoren er flyttet `domain/debug/` → `src/domain/eoRowEvaluation/` (autoritativ, debug-fri), gaten konsumerer den derfra, DEV-debug-laget er nedstrøms, og isolations-værnet er omskrevet (ENGINE-invariant + inverteret invariant C). Over-block-fixet (arbejdsstatus/helbredsstatus kun-når-relevant) er anvendt. Den parallelle `eoBlockingValidation`-infrastruktur er retireret som død kode.
3. **A1 — gør `useLoenindkomstViewModel` til et ægte VM.** ✅ **FÆRDIGGJORT 2026-06-30:** den fulde `deriveLoenindkomstVm`-udskillelse er gennemført (`viewModel/loenindkomstDerivations.ts`, React-fri, 19 isolations-tests), og rå `eoValues` er fjernet fra konteksten.
4. **A4 — migrér de to rå-boolean-download-gates** (`RenteberegningTab` + `MenberegningTab`) til `documentGateTypes`-primitivet. ✅ **FÆRDIGGJORT 2026-06-30:** begge gates bygges nu på primitivet via rene domæne-funktioner (`renteberegningDownloadGate.ts`, `varigeMenDownloadGate.ts`); adfærdsbevarende (sandhedstabel-tests).
5. **C17 — løft tripleret `writeRows` ind i `documentGeneratorSetup.ts`.** ✅ Gjort 2026-06-24 (`writeLabelValueRows`).

### Efter-review-runde (2026-06-30) — eksternt review-fund behandlet

En ekstern review-omgang leverede en række konkrete fund (dokument-/kontrakt-drift, et fail-open-hul i download-gaten, samt fem nye kandidatforslag). Behandlet således:

**Implementeret (adfærdsbevarende korrekthed/struktur/test/doc):**
- **EO-download-gate fail-open (Høj) — rettet.** `collectAllEoRows` modtog aldrig `pdfModel`, så de resultat-afhængige SFGG-fejlrækker (fx `sfgg.dagssats.*` for direkte overenskomstsats) blev evalueret i DEV-debug-fanen, men IKKE i produktions-download-gaten — gaten var fail-open for dem. `pdfModel` føres nu igennem (`eoRowAggregator.ts` + `useEoBeregningViewModel`); ny non-vacuous integrationstest beviser at fejlen kun gates når wiringen er på plads.
- **B8 type-segl udvidet til TAF.** `buildTabtArbejdsfortjenesteModel` returnerer nu `TabtArbejdsfortjenesteSectionPresentation` (Omit) og bærer ikke længere de døde total-felter — section-byggeren kan dermed heller ikke lække et total til output (kommentarens "compile-fejl" er nu reel også på bygger-niveau).
- **Loenindkomst-VM (A1-opfølgning).** `satsErrors` afledes nu rent via `useMemo` fra committed state (de handler-lokale revalideringer mod stale closure-state er fjernet — i tråd med form-kernereglen "valider kun fra committed"); maks-grænsen håndhæves i selve commit-updateren (ikke kun UI); rå `stamdataValues` er fjernet fra konteksten (kun `skadedato`/`skadestype` eksponeres, samme A1-princip som rå `eoValues`); `StandardLoenTableSatser`-typen flyttet `components/` → `types/table.ts` (domæne→UI-kobling brudt); dødt re-export fjernet; nye hook-/commit-sti-tests.
- **Inbound persistence-konsolidering (E3).** Den trust-kritiske inbound-kæde (migrator → sanitize → schema-parse) er samlet i `parseInboundPersistedSection` (modstykke til `buildPersistedSection`), brugt af både `.eo`-load og session-hydrering, så samme rå sektionsdata ikke kan transformeres forskelligt afhængigt af kilden. Rapporteringen forbliver bevidst kilde-specifik.
- **Dokument-/kontrakt-drift + test-dækning.** `downloadTafKravGrafDokument` tilføjet committed-state-guarden (alle fire EO-downloads dækket); `EOberegningTab.eetIssues`-testen udvidet til alle fire downloads; eo-snapshot-kontrakten opdateret (alle fire downloads + graf-target-deling dokumenteret); stale "PDF"-kommentarer/test-navne/fake-writer-typing ryddet; doc-status gjort intern konsistent (A1-afgrænsning, B8-status, startsekvens-historik, forkastet lagbruds-påstand).
- **Efterkontrol 2026-07-01.** To B9-værnstests (`eoBlockingGateCatalog`, `eoRowGateUniqueContribution`) kører nu række-evalueringen med samme snapshot-canonical/pdfModel som produktions-gaten, så resultat-afhængige SFGG-rækker ikke kan falde ud af regressionstests. Række-builderens exception-rækker er samtidig omdøbt fra `debug.builder.*` til `eo.rowBuilder.*`, så den autoritative row-motor ikke fastholder debug-lagets gamle terminologi. Det sidste døde `StandardLoenTableSatser`-re-export er reelt fjernet fra komponentlaget.

**Tidligere forelæggelses-pligtigt — nu gennemført (2026-07-01, brugeren forhåndsgodkendte alle UI/UX-ændringer i denne omgang):**
- **A5 / EO-output-gating-konsolidering — ✅ lukket.** Gate-beslutningen for de fire EO-dokumenter ejes nu af én ren domæne-funktion, [`evaluateEoDocumentDownloadGate`](../../src/domain/erstatningsopgoerelse/snapshot/eoDocumentDownloadGate.ts) (ét autoritativt output-gate-resultat pr. dokument, jf. shortlist-punkt 1). `useEoBeregningViewModel` kalder den for hvert dokument (med live AppSettings/felt-fejl til række-evalueringen) og **videregiver det resulterende `DocumentDownloadGateResult` til service-grænsen**. `documentService` fail-closer nu på dette gate UD OVER sin egen projektions-re-tjek — den var tidligere **fail-open** for de række-niveau EO-fejl (`collectAllEoRows`, fx resultat-afhængige SFGG-fejlrækker) der ikke er snapshot-invarianter. C15-grænsen er bevaret (dokument-laget kender stadig ikke AppSettings; det modtager kun det færdige gate). Adfærdsbevarende på download-knappen (beslutnings-præcedensen er løftet byte-for-byte ud i funktionen); ny non-vacuous service-test beviser fail-closed-på-gate selv når projektionen er ok. Den fulde fold-in i snapshot-cachen blev fravalgt: AppSettings ligger i React-kontekst (ikke en getState-læsbar store), så et gate beregnet hvor AppSettings er live + videregivet til grænsen er den C15-rene realisering.
- **Adskil produktions-kontrol-model fra `domain/debug` — ✅ lukket.** Kontrol-/audit-kernen (sammenlignings-status + besked-format + række-typer) er flyttet til den produktions-ejede [`eoControlMismatch.ts`](../../src/domain/erstatningsopgoerelse/control/eoControlMismatch.ts); `eoSnapshot.ts` importerer nu `collectSammentaellingControlMismatchMessages` derfra (ikke fra `domain/debug`). Debug-laget *bygger* fortsat sammentællings-rækkerne (den "tabel"-side der læser den committede EO-debug-tabel-projektion — bevidst i debug, da en flytning ville trække `EODebugModel` ind i produktion) og re-eksporterer kontrakten bagudkompatibelt. `debugLayerIsolation.test.ts` har en ny CONTROL-invariant (kontrol-filen er debug-fri + snapshottet henter gate-logikken fra produktion). Ren kode-flytning, adfærdsbevarende.
- **EET-kapitaliseringsvisning UI ↔ dokument — ✅ lukket (primær blok).** Kapitaliserings-afgørelsesblokken (rækkefølge, felt-udvælgelse, betinget synlighed) ejes nu af den delte [`eetKapitaliseringRows.ts`](../../src/domain/erhvervsevnetab/eetKapitaliseringRows.ts); både `EetKapitaliseringTab` og `kapitaliseringDocument` itererer rækkerne i hvert sit idiom. De få bevidste forskelle (grundydelse-layout, reguleringsdato-datoformat, særfaktor `<`/`≤`, Køn-rækkens synlighed, UI'ens ekstra Beregningsdato-række) er bevaret via eksplicitte options/række-`kind` — output er byte-identisk (Word/PDF-tests grønne) + ny UI-test for den tidligere utestede fane. *Afgrænsning:* differencekrav-proforma- og mer-erstatnings-varianterne har egne beregningsformer og deler fortsat (kun) udtryks-byggerne (B12); den fælles række-model er ikke udvidet til dem.
- **Delt dropdown-interaktionskerne — ✅ lukket.** Den verbatim-identiske typeahead-matchnings-algoritme (første-bogstav, dansk locale, cirkulær wrap) + de to tastatur-prædikater er løftet til [`dropdownInteractionCore.ts`](../../src/components/inputs/dropdownInteractionCore.ts) (`findTypeaheadMatchIndex`/`isTypeaheadCharKey`/`isClearKey`); `StyledDropdown` + `TableDropdown` bruger den nu. Den divergerende adfærd (open/close-livscyklus, fokus-genskabelse, portal/positionering, celle-editor-integration) er bevidst ladt i hver komponent. Adfærdsbevarende (keyboard-navigation-kontrakt-tests grønne) + ny kerne-unittest.
...
### B9. Produktions-blokerings-validering ud af debug-laget — ✅ IMPLEMENTERET (2026-06-25; eftergennemgået 2026-06-29)

> **Status:** Lukket via single-source relocation. Række-evaluerings-motoren er flyttet til
> `src/domain/eoRowEvaluation/`, download-gaten konsumerer `collectAllEoRows` derfra, og
> `src/domain/debug/` er ren nedstrøms DEV-visning. Over-block-fixet er anvendt, parallel
> `eoBlockingValidation` er fjernet, og eftergennemgangen 2026-06-29 lukkede kontrakt-/testdrift.
...
**Forløb (historik):** Den oprindeligt anbefalede startsekvens var A3 → A2 → A1 (de tre UI-strukturelle, i stigende omfang), sideløbende med B10/B11 som små, forelæggelses-pligtige korrekthedssnit. Den sekvens er nu fuldført: alle A-/B-/C-kandidater er behandlet (implementeret eller verificeret-og-forkastet), så der er ingen åben startsekvens at følge. Afsnittet bevares som historik.

**Status:** Alle kandidater i *prioritetstabellen* er behandlet og lukkede (implementeret eller verificeret-og-nedjusteret/forkastet). Pr. 2026-06-30 er de to sidste åbne tabel-opfølgninger lukket: **A1** (fuld React-fri `deriveLoenindkomstVm` + rå `eoValues`/`stamdataValues` fjernet fra Loenindkomst-konteksten) og **A4** (begge download-gates flyttet onto `documentGateTypes`-primitivet, adfærdsbevarende). De tilbageværende 🟡-markeringer (B7, C14, C16, C17) er bevidste delvise/forkastede udfald, ikke åbne handlinger.

**Ingen åbne forelæggelses-pligtige arkitektur-kandidater tilbage (pr. 2026-07-01).** Den sidste — shortlist-punkt 1, *saml EO-output-gating i én autoritativ issue-/output-projektion* — er nu lukket: `evaluateEoDocumentDownloadGate` leverer ét output-gate-resultat pr. dokument, og `documentService` fail-closer på det (service-grænsen var tidligere fail-open for `collectAllEoRows`-række-fejl). Samtidig blev de tre øvrige forelæggelses-pligtige kandidater fra efter-review-runden 2026-06-30 gennemført (kontrol-model ud af `domain/debug`, EET-kapitaliserings-række-model, delt dropdown-interaktionskerne) — se efter-review-runde 2026-07-01. Brugeren forhåndsgodkendte alle UI/UX-ændringer i den omgang.
...
6. **"`buildMidlertidigtEetPdfGroupsForTafRanges` er et lagbrud i PDF-sektionsfilen" (period-clamping/månedsbrøk/afrunding i `offentligeYdelserSection.ts`).** **Forkastet efter verifikation (2026-06-30).** Påstanden byggede på et forældet billede: funktionen ligger **allerede** i domæne-/helper-laget (`src/domain/erstatningsopgoerelse/helpers/midlertidigtEetBilagGroups.ts`) og *importeres* blot af `offentligeYdelserSection.ts` (PDF-laget) — præcis den placering, kandidaten ville flytte den til. At en domænetest importerer den fra samme helper-fil bekræfter, at den hører hjemme i domænelaget. Der er intet lagbrud at rette; ingen kode-ændring.
