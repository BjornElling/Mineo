...
1. **A5 — saml EO-output-gating i én autoritativ issue-/output-projektion.** Åben kandidat (se A5). B9 flyttede produktions-valideringen ud af debug, men `useEoBeregningViewModel` orkestrerer stadig selve gate-beslutningen ved at kombinere snapshot-projektioner, `collectAllEoRows`, EET-import-issues og en lokal `createPdfGate`. Hvis dette var bygget fra bunden, ville snapshot/issue-laget levere ét output-gate-resultat pr. dokument.
...
**Forelæggelses-pligtigt / ikke gennemført (kræver brugerens godkendelse — rører trust-kritisk gating, beregningsnær pipeline eller synlig UI/dokument-output):**
- **A5 / EO-output-gating-konsolidering.** Service-grænsen (`documentService.ts`) er ikke selv fail-closed mod række-/EET-fejl (re-tjekker kun dokument-projektionen). En fuldt selv-fail-closed grænse kræver at runtime-felt-fejl + fuld AppSettings + manuel-regulerings-input-fejl indgår i snapshottet (beregningsnær pipeline-ændring). Gate-modellen er dokumenteret ved grænsen; selve konsolideringen forelægges.
- **Adskil produktions-kontrol-model fra `domain/debug`.** `eoSnapshot.ts` bruger stadig `collectSammentaellingControlMismatchMessages` fra `domain/debug` til snapshot-invarianten `debug:control_mismatch` (gater output). At trække kontrol-/audit-kernen ud af debug-laget rører den trust-kritiske invariant-pipeline — forelæggelses-pligtigt + kræver stærk test-baseline (jf. B9-noten).
- **EET-kapitaliseringsvisning UI ↔ dokument.** Samme kapitaliseringsblok i `EetKapitaliseringTab`/`EetDifferencekravTab` og dokumentgeneratorerne. En fælles domæne-præsentationsmodel ville berøre synlig UI **og** dokument-output → forelægges.
- **Delt dropdown-interaktionskerne.** `StyledDropdown` + `TableDropdown` ejer hver deres open/close/keyboard/clear-motor. En fælles core-hook rører tastatur-/fokus-adfærd (keyboard-navigation-kontrakten) → forelægges.
...
**Én åben, forelæggelses-pligtig arkitektur-kandidat tilbage:** shortlist-punkt 1 — *saml EO-output-gating i én autoritativ issue-/output-projektion* (det "A5" shortlisten henviser til, ikke det forkastede TAF-år-sæt). EO-download-gaten er stadig sammensat i `useEoBeregningViewModel` af snapshot-projektion + `collectAllEoRows` + EET-issues, og **service-grænsen** (`documentService.ts`) er derfor ikke selv fail-closed mod række-/EET-fejl: den re-tjekker kun dokument-projektionen. Den UI-nære gate er fail-closed i praksis, men en fuldt selv-fail-closed service-grænse kræver, at snapshot/issue-laget leverer ét output-gate-resultat pr. dokument — det forudsætter at runtime-felt-fejl, fuld AppSettings og manuel-regulerings-input-fejl indgår i snapshottet (calc-nær pipeline-ændring). Gate-modellen er dokumenteret ved service-grænsen (2026-06-30); selve konsolideringen er **forelæggelses-pligtig** og ikke gennemført.
