...
const INDEPENDENT_FERIEPERIODE_VALUES: ErstatningsopgoerelseValues = {
  eoNummer: undefined,
...
describe('CALC-006 – uafhængigt facit for gyldig TAF-ferieperiode', () => {
  it('viser den udfyldte ferieperiode i TAF-oversigten', () => {
    const rows = buildEoTaftRows(INDEPENDENT_FERIEPERIODE_VALUES, EMPTY_FIELD_ISSUE_SET, {
    ...
    expect(rows.find((row) => row.id === 'taf.ferie.ferie-facit')).toEqual({
      id: 'taf.ferie.ferie-facit',
      label: 'Ferieperiode',
      displayValue: '02-01-2024 - 03-01-2024',
      status: 'ok',
    });
  });
});
