...
| Felt | Udfyldes |
| --- | --- |
| Inventar-ID / gennemgået revision | `DOC-002`, `DOC-003` / arbejdsrevision efter `e867ab99` |
...
| Eksisterende testfiler og testnavne | 24 filer / 161 tests dækker PDF-/Word-generatorgrene, tekst/tal, valgfri sektioner, filnavne, PDF-writerens cursor/spacing/tabel/pagebreak og Word-blobs pakket ud som ZIP/XML. `documentGateFormatInvariance.test.ts` dækker gate/routing-invarians. `eetPageAudit.spec.ts` downloader nu Word-dokumenter gennem hovedappens formatvalg og kontrollerer løbende ydelser og kapitalisering med konkrete synlige værdier i OOXML. |
| Manglende test, svage assertioner eller dubletter | PDF-goldens bruger mocked jsPDF/autoTable og syntetisk cellegeometri, Word-goldens snapshotter rå XML, og EO-goldens bygger forventningsgrundlag via produktionsprojektion. EET-E2E-beviset kontrollerer Word-OOXML, men ikke PDF-bytes eller fuld kanalparitet. Der mangler reel PDF-parse/render, Word-åbning/OOXML-validering ud over ZIP/XML-strukturen og en uafhængig semantisk PDF↔Word-paritet for tekst, tal, sektioner, headers/footers og rækkefølge. Lokalt er Poppler/LibreOffice og tilsvarende parser/render-værktøjer ikke installeret; det er registreret som et konkret restpunkt, ikke som bestået fysisk bevis. |
...
| Kørte kommandoer, miljø og artefaktlink | Fokuseret dokumentkontrol: 24 filer / 161 tests bestået på Node `v24.18.0`/Windows. `npm run test:e2e -- e2e/eetPageAudit.spec.ts --project=chrome-desktop` viste 6/6 beståede tests, herunder faktisk Word-download med løbende ydelser og kapitalisering samt OOXML-indhold. Fysisk PDF-parse/render og generel PDF↔Word-paritet er ikke kørt. |
...
| TD-014 | 2026-09-11 | `DOC-001` | Mellem | Der findes ingen generel semantisk PDF↔Word-paritetstest for alle 21 outputs. Udvalgte goldens og isolerede Word-tests kan derfor ikke bevise, at samme sag giver samme labels, tal, sektioner og rækkefølge i begge kanaler. | `documentGateFormatInvariance.test.ts` håndhæver primært struktur og gate-formatadskillelse. `eetPageAudit.spec.ts` downloader nu Word gennem hovedappens formatvalg og kontrollerer titel, beløb, procent og afgørelsessektion i OOXML. | Åbent. Den nye EET-kontrol er kun et repræsentativt kanalbevis. Fuld PDF-parse/render og normaliseret tværgående paritet mangler fortsat; eventuel ny parser-/renderer-dependency skal begrundes særskilt. | Ikke påkrævet for testændringen. | Delvist retestet – EET-Word bevis bestået, generel paritet åben |
