...
- Seneste samlede retest: `verify:release:core` på revision `81c095d0` er grøn med 736 testfiler og 8.822 beståede tests uden tilbageværende forventede `it.fails`; coverage er 90,23 / 81,38 / 93,97 / 93,05, og begge builds bestod. Den seneste fulde E2E-suite er grøn på `e0f9d340` med 201 beståede tests og 2 forventede skips ud af 203 på 10 projektbaner; siden da er kun tests og dokumentation ændret. TD-003, TD-017 og den konkrete TD-043-afrundingsforskel er lukket; åbne produkt-, proveniens-, outputparitets- og TD-025-stabilitetsfund forhindrer fortsat afslutning.
...
| Aktuel fuld Vitest-retest | `npm run test:coverage` | Bestået | 736 filer, 8.822 tests bestået og ingen forventede `it.fails`; coverage 90,23 / 81,38 / 93,97 / 93,05 på `81c095d0` |
| E2E-baner | `npm run test:e2e` | Bestået | 201 beståede tests og 2 forventede skips ud af 203, 4,9 min, 3 workers og 10 projektbaner på `e0f9d340`; den udvidede kørsel omfatter de fire dedikerede browserprojekter |
| Aktuel samlet release-gate | `npm run verify:release:core` | Bestået | Dependency-, runtime-, type-, lint-, data-, kontrakt-, lane-, ledger-, coverage- og begge build-gates bestået på `81c095d0`; coverage 736 filer / 8.822 beståede tests / ingen forventede `it.fails` |
...
| Aktuel coverage-retest | `npm run test:coverage` | Bestået | 393 instrumenterede filer; 90,23 % statements, 81,38 % branches, 93,97 % functions, 93,05 % lines; `coverage/index.html`, `coverage/clover.xml` og `coverage/coverage-final.json`; `81c095d0`, Node `v24.18.0`/Windows |
