...
| MIN-001 | Standalone MinProcesrente – `src/apps/minprocesrente/**`, shared renteberegning | Standalone kan blande hovedappens auth/settings/persistens ind eller vise et andet resultat end den delte engine. | `app-shell-contract.md`, `renteberegning-contract.md` | unit/integration/E2E | `apps/minprocesrente/**`, `quality/minprocesrenteStandaloneIsolation.test.ts`, `apps/minprocesrente/standaloneCalculatorPage.test.tsx`, `e2e/minprocesrente-recovery-and-focus.spec.ts` | Isolation, namespace, error boundary, reset/fokus og exit-guard er gennemgået; E2E-baseline er grøn. Et komplet valid input → beregning → gate → PDF-flow mangler og er registreret som `TD-023`. | I gang |
...
| Felt | Udfyldes |
| --- | --- |
| Inventar-ID / gennemgået revision | `PERSIST-002` / arbejdsrevision efter `084ea6d4` med reload-forløbet |
...
| Felt | Udfyldes |
| --- | --- |
| Inventar-ID / gennemgået revision | `DATA-001`, `CALC-001` / arbejdsrevision efter `1cb0b07b` |
...
| Eksisterende testfiler og testnavne | 22 filer / 472 tests under `src/__tests__/data`, `src/__tests__/domain/satser` og `quality/consumerInventory.test.ts`. De dækker katalogets 14 registrerede kilder, fingerprints, huller, overlap, sortering, bounds, missing-entry og udvalgte historiske særtilfælde. |
| Manglende test, svage assertioner eller dubletter | Katalog- og integritetsværnene er strukturelt stærke, men flere facitassertions genbruger produktionsdata eller helpers: `satserProjection.test.ts`, `aslAarsloensmaksimum.test.ts`, `opreguleringsmotorer.test.ts` og `satserNewCaseSeed.test.ts`. Der mangler en lille håndskrevet facitmatrix for alle registre og deres første/sidste år samt en repræsentativ sats → beregning → dokument-projektion. `assertOffentligLoenDataIntegritet` og statistik-/overenskomstguards har også konkrete validatorpartitioner, der kan styrkes med testfixtures. |
...
| Kørte kommandoer, miljø og artefaktlink | `npx vitest run src/__tests__/data src/__tests__/domain/satser src/__tests__/quality/consumerInventory.test.ts --reporter=dot` – 22 filer / 472 tests bestået på Node `v24.18.0`/Windows. |
| Fund-ID'er, beslutninger og opfølgning | `TD-020` registrerer manglende uafhængig payload-/endpointfacit, downstream-kæde og validatorpartitioner. Test-only matrix- og fixturestyrkelser kan tilføjes; runtime-data eller beregningsændringer forelægges udvikleren. |
| Reviewer / dato / slutstatus | Codex / 2026-09-10 / `I gang` |
...
| Felt | Udfyldes |
| --- | --- |
| Inventar-ID / gennemgået revision | `SHELL-001`, `SHELL-002` / arbejdsrevision efter `084ea6d4` |
...
| Manglende test, svage assertioner eller dubletter | Desktop-gate-testen hævder kun, at `renderApp` ikke kaldes; den observerer ikke, om stylesheets, PWA-setup/install-capture eller `beforeDesktopRender` også er fraværende, og der mangler ægte browser-hard-stop for mobil. Lazy/PWA-recovery er jsdom-/mockbaseret, ikke en faktisk manglende chunk → synlig recovery → kritisk reload-rejse. Native `launchQueue` kan ikke fuldt fremkaldes i Playwright. `pwa-file-open.spec.ts` bruger desuden egen console-opsamling i stedet for den fælles `runtimeErrors`-fixture. |
...
| Kørte kommandoer, miljø og artefaktlink | Målrettede unit-/integrationstests: 26 filer / 125 tests. Isolerede E2E-kørsler for shell/404, smoke, minimumsviewporter, PWA, Firefox-fallback og MinProcesrente bestod som angivet ovenfor på Node `v24.18.0`/Windows. |
| Fund-ID'er, beslutninger og opfølgning | `TD-021` dækker manglende assertion af alle desktop-gate-sideeffekter og ægte mobil-hard-stop. `TD-022` dækker manglende end-to-end lazy/PWA-recovery og fælles runtime-fejlopsamling. Test-only styrkelse kan tilføjes; ændringer af gate-, recovery- eller PWA-adfærd forelægges udvikleren. |
| Reviewer / dato / slutstatus | Codex / 2026-09-10 / `I gang` |
...
| Felt | Udfyldes |
| --- | --- |
| Inventar-ID / gennemgået revision | `MIN-001` / arbejdsrevision efter `084ea6d4` |
...
| Eksisterende testfiler og testnavne | `quality/minprocesrenteStandaloneIsolation.test.ts`, `apps/minprocesrente/standaloneCalculatorPage.test.tsx`, `MinProcesrenteApp.fullRender.test.tsx` og `e2e/minprocesrente-recovery-and-focus.spec.ts`. Den målrettede E2E-test bestod med 1/1. |
| Manglende test, svage assertioner eller dubletter | Den eksisterende browsertest dækker afvist dato, reset og fokus, mens full-render primært kontrollerer heading. Der mangler valid input → beregning → gate → PDF-download og et browserbevis for namespace-/runtime-isolation. |
...
| Kørte kommandoer, miljø og artefaktlink | `e2e/minprocesrente-recovery-and-focus.spec.ts` bestod med 1/1 i isoleret browserkørsel. Den øvrige shell-/standalonekontrol bestod med 26 filer / 125 unit-/integrationstests samt de øvrige angivne E2E-kørsler. |
| Fund-ID'er, beslutninger og opfølgning | `TD-023` er åbent for valid input → beregning → gate → PDF og browserbaseret isolation. Test-only styrkelse kan tilføjes; ændringer af procesrente, gate eller synlig outputadfærd forelægges udvikleren. |
| Reviewer / dato / slutstatus | Codex / 2026-09-10 / `I gang` |
...
| `PERSIST-002` | Browserstorage, settings, filhandles og sessionbevaring | Unit-/hook-tests mocker flere platforme, og der mangler reel reload/session-kontrol; File API-checken accepterer property-navne uden at kræve funktioner | Styrk browser-/platformsgrænsen og fasthold åbne produktbeslutninger særskilt | E2E med input + aktiv fane + reload samt test-only integration af File API/IndexedDB kan tilføjes. Capability-detektionen må ikke ændres uden forelæggelse | I gang – `TD-017` |
...
| TD-020 | 2026-09-10 | `DATA-001` / `CALC-001` | Mellem | Data-/sats-suiterne har stærke katalog-, fingerprint- og integritetsguards, men flere facitassertions læser produktionsdata som forventning. Alle historiske endpoints, validatorpartitioner og sats → beregning → dokument-kæden er derfor ikke uafhængigt bevist. | 22 filer / 472 tests bestod. Reviewet identificerede genbrug af produktionsdata i `satserProjection.test.ts`, `aslAarsloensmaksimum.test.ts`, `opreguleringsmotorer.test.ts` og `satserNewCaseSeed.test.ts` samt manglende komplet validatorfacit for offentlig løn/statistik/overenskomst. | Åbent. Tilføj en lille håndskrevet payload-/endpointmatrix og én uafhængig downstream-kædetest; runtime-data/validatorer ændres ikke som del af auditten. | Ikke påkrævet for test-only styrkelse. | Ikke retestet – åben |
| TD-021 | 2026-09-10 | `SHELL-001` / `SHELL-002` | Mellem | Desktop-gate-testen hævder kun, at `renderApp` ikke kaldes. Den beviser ikke, at app-stylesheet, PWA-setup/install-capture eller `beforeDesktopRender` heller ikke kaldes, og der mangler ægte browser-hard-stop for mobil. | Agentreview af 26 fokuserede filer / 125 tests og isolerede shell-/viewport-/PWA-kørsler: auth, 11 routes, 404, minimumsviewporter og UnsupportedDevice-isolation er grønne; den tidlige sideeffektkæde er ikke observeret fuldt. | Åbent. Tilføj test-only assertions for alle tidlige sideeffekter og, hvis browserharnesset kan gøre det uden falsk positiv, et mobil-hard-stop-forløb. Ingen ændring af desktop-gate eller synlig stopside uden udviklerens godkendelse. | Ja ved produktændring; ikke ved rent testbevis. | Ikke retestet – åben |
...
| TD-023 | 2026-09-10 | `MIN-001` | Mellem | Standalone MinProcesrente har isolation-, reset-, fokus- og afvist-dato-bevis, men ikke en samlet valid input → beregning → gate → PDF-download-rejse eller browserbaseret namespace-/runtime-isolation. | `minprocesrente-recovery-and-focus.spec.ts` bestod med 1/1, og den øvrige standalone-/shellkontrol er grøn. `MinProcesrenteApp.fullRender.test.tsx` kontrollerer primært heading, så et forkert validt output eller en manglende dokumentrejse kan overleve. | Åbent. Tilføj kun observerbare testcases, der bruger den eksisterende standalone-UI og fælles E2E-fixtures. Eventuelle ændringer af procesrente, gate eller dokumentadfærd forelægges udvikleren. | Ikke påkrævet for test-only styrkelse. | Ikke retestet – åben |
