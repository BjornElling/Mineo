...
- Evidensrevision: `e50fb1e5` for seneste samlede releasegate og målrettede test-only batch; seneste fulde E2E er `71746917`
- Branch: `main`
...
| Aktuel fuld Vitest-retest | `npm run test:coverage` | Bestået | 847 filer, 9.021 tests bestået og ingen forventede `it.fails` på `e50fb1e5` |
...
| Aktuel samlet release-gate | `npm run verify:release:core` | Bestået | Dependency-, runtime-, type-, lint-, data-, kontrakt-, lane-, ledger-, coverage- og begge build-gates bestået på `e50fb1e5`; coverage 847 filer / 9.021 beståede tests / ingen forventede `it.fails` |
...
| Aktuel coverage-retest | `npm run test:coverage` | Bestået | 394 instrumenterede filer; 90,58 % statements, 81,70 % branches, 94,76 % functions, 93,36 % lines; coverage-artefakterne blev genereret under `coverage/`; `e50fb1e5`, Node `v24.18.0`/Windows |
...
| Senest tilføjet lukket | `TD-153`, `TD-154`, `TD-155`, `TD-157`–`TD-219` – test-only facitter dokumenteret til og med `e50fb1e5`; `TD-156` er fortsat åbent |

## Aktuel evidensdelta – revision `e50fb1e5`
...
Den seneste arbejdsbatch på `e50fb1e5` har grøn målrettet retest og samlet releasegate. Den fulde
