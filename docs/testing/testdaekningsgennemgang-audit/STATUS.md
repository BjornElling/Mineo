...
| Aktuel fuld Vitest-retest | `npm run test` | Bestået | 785 filer, 8.882 tests bestået og ingen forventede `it.fails` på `89a2d3e0` |
...
| Aktuel samlet release-gate | `npm run verify:release:core` | Bestået | Dependency-, runtime-, type-, lint-, data-, kontrakt-, lane-, ledger-, coverage- og begge build-gates bestået på `89a2d3e0`; coverage 785 filer / 8.882 beståede tests / ingen forventede `it.fails` |
...
| Aktuel coverage-retest | `npm run test:coverage` | Bestået | 393 instrumenterede filer; 90,26 % statements, 81,42 % branches, 94,03 % functions, 93,07 % lines; 18.583/20.587 statements, 13.104/16.094 branches, 3.027/3.219 funktioner og 16.969/18.232 linjer; `coverage/index.html`, `coverage/clover.xml` og `coverage/coverage-final.json`; `89a2d3e0`, Node `v24.18.0`/Windows |
...
Den seneste målrettede batch og samlede releasegate på `9043e591` er retestet grønt; den fulde
