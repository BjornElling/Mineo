...
| Aktuel fuld Vitest-retest | `npm run test:coverage` | Bestået | 670 filer, 8.697 tests bestået samt 17 forventede `it.fails`; coverage-fase 337,87 s på `aff8dd80` |
...
| Aktuel samlet release-gate | `npm run verify:release:core` | Bestået | Dependency-, runtime-, type-, lint-, data-, kontrakt-, lane-, ledger-, coverage- og begge build-gates bestået på `aff8dd80`; coverage 670 filer / 8.697 beståede tests / 17 forventede `it.fails` |
...
| Aktuel coverage-retest | `npm run test:coverage` | Bestået | 393 instrumenterede filer; 90,13 % statements, 81,21 % branches, 93,97 % functions, 92,95 % lines; 18.554 / 20.584, 13.067 / 16.090, 3.024 / 3.218 og 16.946 / 18.230 målte enheder; `coverage/index.html`, `coverage/clover.xml` og `coverage/coverage-final.json`; `aff8dd80`, Node `v24.18.0`/Windows |
