...
- Evidensrevision: `d5bf5f62` for seneste samlede releasegate og målrettede test-only batch; seneste fulde E2E er `71746917`
- Branch: `main`
...
- Seneste test-only batch: VALID-facit TD-275; målrettet kontrol 1 ny test / 1 test; ingen produktkode, beregningslogik, UI/UX eller persistens ændret. Den parallelle DATA/CALC-triage gav no-change, fordi kandidaterne overlappede eksisterende facitter.
...
| Aktuel fuld Vitest-retest | `npm run test:coverage` | Bestået | 902 filer, 9.077 tests bestået og ingen forventede `it.fails` på `d5bf5f62` |
...
| Aktuel samlet release-gate | `npm run verify:release:core` | Bestået | Dependency-, runtime-, type-, lint-, data-, kontrakt-, lane-, ledger-, coverage- og begge build-gates bestået på `d5bf5f62`; coverage 902 filer / 9.077 beståede tests / ingen forventede `it.fails` |
...
| Aktuel coverage-retest | `npm run test:coverage` | Bestået | 394 instrumenterede filer; 90,74 % statements, 81,99 % branches, 94,88 % functions, 93,54 % lines; coverage-artefakterne blev genereret under `coverage/`; `d5bf5f62`, Node `v24.18.0`/Windows |
...
Aktuel inventarrevision på `d5bf5f62`: `PERSIST-002` omfatter nu også den nye silent-read-,
...
| Senest tilføjet lukket | `TD-153`, `TD-154`, `TD-155`, `TD-157`–`TD-275` – test-only facitter dokumenteret til og med `d5bf5f62`; `TD-156` er fortsat åbent |

## Aktuel evidensdelta – revision `d5bf5f62`
