...
| Baseline: antal kildefiler, testfiler, tests, E2E-specs og mutationsscore | Ved auditstart: 923 produktionsfiler (`.ts/.tsx`), 638 testfiler i alt, 8.421 Vitest-tests, 36 E2E-specs og 174 E2E-tests. Slutnær revision `5c88267f`: 920 produktionsfiler, 661 Vitest-testfiler, 8.601 beståede tests samt 6 forventede `it.fails`, 40 E2E-specs og 179 beståede E2E-tests. Mutationsrunneren er kvalificeret modulvist: money 56/58 dræbt, `dateCommit.ts` 4/6 dræbt, årsløn 108/113 dræbt, varige mén 94/114 dræbt, forsørgertab 21/21 dræbt og procesrente 104/136 dræbt med 23 triagerede survivors og 9 dokumenterede timeouts; alle ikke-dræbte mutationer er triageret. |
| Baseline: `test:coverage`-rapport og de dækkede/udeladte mapper | `coverage/` fra auditrevisionen: 396 instrumenterede filer i `src/domain`, `src/utils`, `src/hooks` og `src/contexts`; den samlede releasekørsel på `5c88267f` gav 89,23 % statements, 80,48 % branches, 92,35 % functions og 92,02 % lines – 18.369 / 20.584, 12.950 / 16.090, 2.972 / 3.218 og 16.777 / 18.230 målte enheder. Coverage-konfigurationen omfatter ikke de øvrige produktionsfiler; de skal klassificeres i inventaret. |
...
| Kørte kommandoer, miljø og artefaktlink | Fokuseret dokumentkontrol: 24 filer / 161 tests bestået på Node `v24.18.0`/Windows. `npm run test:e2e -- e2e/eetPageAudit.spec.ts --project=chrome-desktop` viste 5/5 beståede tests, herunder faktisk Word-download og OOXML-indhold; testserveren blev stoppet manuelt efter summary, fordi runneren ikke afsluttede processen. Fysisk PDF-parse/render og generel PDF↔Word-paritet er ikke kørt. |
...
- [x] `npm run test:coverage` er kørt som del af `npm run verify:release:core` på `5c88267f` og rapporteret med instrumenteret filmængde.
...
| `CALC-007` | EET-totaler, snapshot og dokumentgates | Håndberegnet samlet totalsag samt tværgående tabelkanal | EAL, ASL/løbende ydelse, differencekrav og EET-løbende-ydelser | `eetIndependentOracle.test.ts`: 1/1; PDF-resolved table cells matcher Word XML for 19 cases; EET-Word-E2E har konkrete indholdsassertions | Delvist – øvrige outputs og fysisk PDF/Word-paritet åbne |
...
| TD-018 | 2026-09-11 | `CALC-007` | Høj | EET har bred unit-/snapshot-/gate-dækning, men ingen generel kontrol af engine → snapshot → UI → dokumentindhold. Kapitaliserings- og differencekravsfaner har heller ikke en asymmetrisk dokumentrejse, og PDF-paritet mangler. | `eetIndependentOracle.test.ts` bevarer den samlede håndfacitprøve. `eetPageAudit.spec.ts` downloader nu Word fra hovedappen og kræver `Løbende ydelser (EET)`, `Afgørelse`, `400.000 kr.`, `25 %` og faktisk `.docx`-indhold; den målrettede spec viste 5/5 beståede tests. | Delvist lukket for EET-Word-download og observerbart dokumentindhold. Fuld engine → snapshot → UI → dokumentkæde for alle relevante faner og PDF↔Word-paritet er åben. | Ikke påkrævet for testændringen; ingen produktkode er ændret. | Delvist retestet – EET-Word bevis bestået, samlet fund åbent |
| TD-019 | 2026-09-11 | `INPUT-001` / `INPUT-002` / `VALID-001` | Mellem | Form/grid-pariteten er nu bundet til en reel produktionsdescriptor, nested row-schemaernes unknown/missing/invalid-partitioner er dækket, og faktiske `ZodError.issues` bruges. Flere validatorfixtures genbruger fortsat produktionsdefaults. | `erstatningsopgoerelseValidator.independent.test.ts` tilføjer 8 små, literalbaserede cases med konkrete `ZodError.issues` og uden runtime-defaults. Den målrettede kontrol bestod med 8/8; den øvrige validatorflade genbruger fortsat enkelte produktionsfixtures. | Delvist lukket. Yderligere validator- og downstream-partitioner skal fortsat vurderes; ingen runtime- eller loadændring er foretaget. | Ikke påkrævet for testændringen, så længe runtime-/loadadfærd ikke ændres. | Delvist lukket – uafhængige literaltests tilføjet |
