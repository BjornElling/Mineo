...
| Validatorer og dataendepunkter | Målrettet Vitest-kørsel (se hoveddokumentet) | Bestået | Uafhængige validator-literals, 5 håndskrevne typed validator-cases, obligatorisk top-level schema-facit, statiske data-/endpointfacitter, 2 folkepensions-downstreamfacitter, 1 KRL-downstreamfacit, 1 procesrente-downstreamfacit samt KRL- og offentlig-løn-facitter 1/1 hver |
...
| `INPUT-002` / `VALID-001` | Form/grid-overflader, produktions-ChoiceField og schema-/validatorhuller gennemgået | 68 filer / 1.521 tests; produktionsbundet form/grid-paritet, 12 nested row-schemas, faktiske `ZodError.issues`, 10 uafhængige validator-literaltests, 5 håndskrevne typed runtime-cases, et obligatorisk top-level schema-facit samt selvstændige KRL- og offentlig-løn-facitter er dækket | Yderligere validator-fixture-uafhængighed og downstream-partitioner; `TD-019` | `I gang` |
...
| `CALC-002` | Method C dag gennemgået med den tidligere utestede hele-kalendermåned-branch og uafhængig mutationstest | Fem uafhængige/branchestyrkende testfiler bestod med 38/38 tests for Metode A/B/C, nul, feriegrænse, procentbaserede tillæg, ugeløn og dag-fallback; command-runner: 108/113 mutationer dræbt, 5 rapporterede overlevere, hvoraf én blev dræbt i isoleret kontrolkørsel og fire er triageret som ækvivalente | Resterende årslønsbranches, integration/downstream-paritet og fuld brugerrejse | `I gang` |
