...
| `CALC-006` | EO-snapshot, canonical totals, dokumentprojektion, inspektionsdage og sidevisning stikprøvet med tre uafhængige orakler samt række-/periodefacitter | `CALC-006`-kørslen bestod med 160 filer / 2.174 tests, heraf 8 nye uafhængige række-/periodecases; de tre EO-/inspection-orakler består samlet af 9 tests. En weekendydelse i arbejdsdagsbaseret TAF gav observeret `control:sammentaelling_mismatch`; `TD-016` er åbent | Øvrige rækkegrene, dokumentparitet, mutation og fuld E2E; udviklerens beslutning om TD-016 | `I gang` |
...
`CALC-001`, `CALC-004` og `CALC-005` har nu uafhængige facitter eller totalsager og afgrænsede mutationstests med triagerede overlevere, mens `CALC-002`/`CALC-003` har samme type mutationsevidens og nu også procent-, uge-, dag- og månedlige engine-facitspor. `CALC-006` har fået række-/periodefacitter samt et direkte Beløb-facit for lønkolonner; fortsæt med afklaring af `TD-016` og øvrige EO-grene. `CALC-007` har fået et uafhængigt EET-definition → Word-facit. `TD-019` og `TD-020` er delvist lukket med henholdsvis schema-/issuefacitter og udvalgte sats-/downstream-facitter, herunder konkrete KL-, ILON12-, SBLON2-, procesrente-, RLTN- og 2025- samt 2019/2020-sygedagpenge-facitter gennem de relevante downstream-motorer. TD-019 har nu også uafhængige facitter for modstridende overenskomstvalg, manuel lønbranches manglende eller nul grundløn og manglende dato, manuel procentsatsdato, manuel procentsats, manglende indtægtsoplysninger, manglende manuel SFGG-dagssats, manglende SFGG-beregningskilde og manglende referenceperiodes slutdato i aktiv TAF. ARCH-002 har desuden fået et dynamisk `import()`-modcase, og VALID-001 et forkert top-level-typefacit; begge ændringer er test-only og målrettet retestet på `ca0b7617`. EET-definitionen og Beløb-facittet er målrettet retestet på `c1f7c364`.
