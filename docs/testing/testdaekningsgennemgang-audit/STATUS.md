...
| Aktuel fuld Vitest-retest | `npm run test:coverage` | Bestået | 697 filer, 8.755 tests bestået samt 17 forventede `it.fails`; coverage 90,14 / 81,24 / 93,97 / 92,96; coverage-fase 368,30 s på `8133fc80` |
...
| Aktuel samlet release-gate | `npm run verify:release:core` | Bestået | Dependency-, runtime-, type-, lint-, data-, kontrakt-, lane-, ledger-, coverage- og begge build-gates bestået på `8133fc80`; coverage 697 filer / 8.755 beståede tests / 17 forventede `it.fails` |
...
| Aktuel coverage-retest | `npm run test:coverage` | Bestået | 393 instrumenterede filer; 90,14 % statements, 81,24 % branches, 93,97 % functions, 92,96 % lines; 18.556 / 20.584, 13.072 / 16.090, 3.024 / 3.218 og 16.948 / 18.230 målte enheder; `coverage/index.html`, `coverage/clover.xml` og `coverage/coverage-final.json`; `8133fc80`, Node `v24.18.0`/Windows |
...
| Validatorer og dataendepunkter | Målrettet Vitest-kørsel (se hoveddokumentet) | Bestået | Uafhængige validator-literals, 13 håndskrevne typed schema-runtime-cases inklusive negativ fritvalg-procent, obligatorisk top-level schema-facit, direkte Beregningsperiode-, KRL-, offentlig-løn-, overenskomst-, manuel-løn-, manuel-procentsats- og SFGG-facitter, statiske data-/endpointfacitter, 2 folkepensions-downstreamfacitter, 2 pensionsalder-downstreamfacitter, 1 forsørgertabs-downstreamfacit, 1 KRL-downstreamfacit, 1 Statistik/ILON12-downstreamfacit, 1 procesrente-downstreamfacit, 1 svie/smerte-downstreamfacit samt offentlig-løn-facitter 3/3, overenskomst-facit 1/1, manuel-løn-facit 1/1 og manuel-procentsats-facit 1/1 |
...
De seneste test-only styrkelser er målrettet retestet: standalone-dokumentparitet med 3/3 på `75d1a956`, pensionsalderens downstream-hændelser med 2/2 på `16ad2251`, validatorens negative fritvalg-procent med 13/13 og forsørgertabets EAL-mindstesats med 1/1 på `4418f502`. De målrettede mapperetests står nu på 29 filer / 280 dokumenttests, 33 filer / 531 data-/sats-/endpointtests og 68 filer / 1.526 form-/schema-/validator-tests. Den samlede releasegate er efterfølgende genkørt grønt på `8133fc80` med 697 testfiler / 8.755 beståede tests, 17 forventede `it.fails` og coverage 90,14 / 81,24 / 93,97 / 92,96. Næste arbejdsenhed skal fortsat vælges blandt de åbne, test-only dækningshuller.
