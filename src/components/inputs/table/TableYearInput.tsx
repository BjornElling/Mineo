  ...
  ({
    ...
    const handleBlur = React.useCallback(
      (e: React.FocusEvent<HTMLInputElement>) => {
        ...
        if (!isEditing) {
          const committed = commitYearDraft(normalizeTableDraftOnCommit(rawValue), {
            minYear: latest.current.minYear,
            maxYear: latest.current.maxYear,
            twoDigitYearPolicy: latest.current.twoDigitYearPolicy,
          });
          if (
            committed.kind === 'ok' &&
            yearFingerprintFromCanonical(committedToString(committed)) === latestCommittedPayloadRef.current.fingerprint
          ) {
            return;
          }
        }
