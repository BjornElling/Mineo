...
- Evidensrevision: `72d9d169` for seneste samlede releasegate og målrettede test-only batch; seneste fulde E2E er `71746917`
- Branch: `main`
...
- Seneste test-only batch: PERSIST-/CALC-facitter TD-229–TD-230; målrettet kontrol 7 filer / 67 tests inklusive relaterede facitter; ingen produktkode, beregningslogik, UI/UX eller persistens ændret.
...
| `PERSIST-002` | Storage/settings, filhåndtag, PWA-filflows og reel session-reload gennemgået | 18 filer / 183 målrettede unit-/hook-tests; PWA-/filkørsel 4/4; reel input + aktiv fane + reload 1/1; supplerende 8-filers platformssuite med 89/89 beståede tests efter callable picker-detektion; `safeLocalStorage.test.ts` bestod med 11/11 direkte Node-fallback-isolationsassertions; `fileHandleVerification.test.ts` bestod med 22/22; `fileHandleKvStore.test.ts` bestod med 2/2; `fileOperationClientSession.persistence.test.ts` tilføjer 1/1 sessionStorage-klientidentitetsfacit efter simuleret reload; `fileSaveTarget.test.ts` tilføjer 1/1 orphaned-handle-facit før picker-flowet; `fileSaveTargetPermissionApiFailure.test.ts` tilføjer 1/1 permission-API-facit med handle-oprydning og picker-fallback; `fileHandleStorage.pendingRequestValidation.test.ts` tilføjer 1/1 fail-closed-facit for negativt ignoredFileCount; `indexedDbStore.readRequestFailure.test.ts` tilføjer 1/1 readonly get-fejlfacit med logning og forbindelseslukning; ugyldige `Date`-inputs har 8/8 grønne negative modcases | Fuld browser-/platformssammenhæng, herunder installeret PWA-filaflevering; `TD-022` | `I gang` |
...
Aktuel inventarrevision på `72d9d169`: `PERSIST-002` omfatter nu også `indexedDbStore.readRequestFailure.test.ts`
med 1/1 readonly get-fejlfacit, og `CALC-006` omfatter `eoInspektionRegulationKlCanonicalIndependent.test.ts`
med 1/1 facit for canonical KL-segmenter i beregnet reguleringstabel. De tilsvarende målrettede inventartal er
dermed 18 filer / 183 tests for PERSIST-002 og 170 filer / 2.184 tests for CALC-006; række-/periodefacitterne
er 15/15.
...
| Senest tilføjet lukket | `TD-153`, `TD-154`, `TD-155`, `TD-157`–`TD-230` – test-only facitter dokumenteret til og med `72d9d169`; `TD-156` er fortsat åbent |

## Aktuel evidensdelta – revision `72d9d169`
