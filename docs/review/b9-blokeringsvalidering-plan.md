...
7. **Test-flytning (gennemført 2026-06-25).** De 18 motor-test-filer er flyttet `__tests__/domain/debug/` → **`__tests__/domain/eoRowEvaluation/`** og omdøbt `eoDebug…`→`eoRow…` (fx `eoRowAggregator.test.ts`, `eoRowBuilderRegistry.test.ts`, `eoRowSeverity.test.ts`; `svieSmerteBeregning.test.ts` beholdt navn). Opdelingen følger importerne: en test flyttes hvis dens emne-under-test er et `eoRowEvaluation/`-modul; de 15 tilbageværende `__tests__/domain/debug/`-filer tester alle et `domain/debug/`-DEV-visnings-modul og bliver. Begge mappe-spejle nu deres kilde. Test-filernes interne indhold (`describe`-strenge, symboler) var allerede omdøbt i punkt 6 — kun fil-placering/-navn udestod. `__tests__/domain/debug/` og `__tests__/domain/eoRowEvaluation/` ligger i samme dybde under `src`, så de relative import-stier var uændret-gyldige (`git mv` alene, ingen sti-rettelser). Verifikation: 457 tests grønne (33 filer), typecheck:test ✓, lint ✓. Adfærds-neutralt.

> **Note (separat doc-drift, ikke rørt):** `docs/architecture/debug-builder-architecture.md` peger stadig på den GAMLE kilde-sti `src/domain/debug/eoDebugBuilderRegistry.ts` (forældet siden kilde-relocationen i §8 pkt. 2, ikke denne test-flytning). Lav-værdi doc-opdatering, kan tages ved lejlighed.
