...
| Eksisterende testfiler og testnavne | 23 filer i `src/__tests__/domain/erhvervsevnetab` og `src/__tests__/components/pages/Erhvervsevnetab.integration.test.tsx`, bl.a. `eetSnapshot.test.ts`, `eetEngineGate.test.ts`, `eetEalCalculation.test.ts`, `eetLoebendeYdelserCalculation.test.ts`, `eetCanonicalOutput.test.ts`, `erhvervsevnetabDownloadGate.test.ts` og `Erhvervsevnetab.integration.test.tsx`. Browserfladen er dækket af `e2e/eetPageAudit.spec.ts`.
| Manglende test, svage assertioner eller dubletter | Den målrettede suite har bred branch-, issue-, dependency- og gate-dækning og hævder mange konkrete numeriske delresultater. Den mangler stadig en uafhængig, samlet håndberegnet totalsag, der følger input gennem engine, snapshot, alle fem faner og dokumentgate. En fuld browserrejse med faneskift og dokumentvalg er heller ikke dokumenteret som ét sammenhængende facitforløb. |
...
| Mutation/modprøve og resultat | Der er ikke kørt mutation på CALC-007. Den eksisterende suite er en bred modprøve af delbranches, men dokumenterer ikke mutationsstyrke eller en separat håndfacitkontrol af de samlede kroner. |
| Kørte kommandoer, miljø og artefaktlink | `npx vitest run src/__tests__/domain/erhvervsevnetab src/__tests__/components/pages/Erhvervsevnetab.integration.test.tsx --reporter=dot` – 23 filer / 420 tests bestået på Node `v24.18.0`/Windows. `npm run test:e2e -- e2e/eetPageAudit.spec.ts` – 4/4 tests bestået med isoleret E2E-buildserver. Vite udsendte den kendte `configLoader: 'native'`-advarsel. |
| Fund-ID'er, beslutninger og opfølgning | Ingen ny produktfejl er fastslået i denne baseline. Tilføj en uafhængig totalsag og en sammenhængende fem-fane-/dokumentrejse, og vurder derefter mutation og PDF/Word-paritet. Eventuelle ændringer af EET-tal eller gateadfærd forelægges udvikleren før implementering. |
| Reviewer / dato / slutstatus | Codex / 2026-09-10 / `I gang` |
