...
| Validatorer og dataendepunkter | Målrettet Vitest-kørsel (se hoveddokumentet) | Bestået | Uafhængige validator-literals, 6 håndskrevne typed validator-cases, obligatorisk top-level schema-facit, statiske data-/endpointfacitter, 2 folkepensions-downstreamfacitter, 1 KRL-downstreamfacit, 1 procesrente-downstreamfacit og det nye KRL-validatorfacit 1/1 |
...
| `CALC-002` | Method C dag gennemgået med den tidligere utestede hele-kalendermåned-branch og uafhængig mutationstest | Fire uafhængige/branchestyrkende testfiler bestod med 37/37 tests for Metode A/B/C, nul, feriegrænse, procentbaserede tillæg og ugeløn; command-runner: 108/113 mutationer dræbt, 5 rapporterede overlevere, hvoraf én blev dræbt i isoleret kontrolkørsel og fire er triageret som ækvivalente | Resterende årslønsbranches, dag-fallback, integration/downstream-paritet og fuld brugerrejse | `I gang` |
...
- `td019ValidatorSchemaIndependence.test.ts` har nu 5/5 håndskrevne typed runtime-cases.
...
`CALC-001`, `CALC-004` og `CALC-005` har nu uafhængige facitter eller totalsager og afgrænsede mutationstests med triagerede overlevere, mens `CALC-002`/`CALC-003` har samme type mutationsevidens og nu også procent-, uge- og månedlige engine-facitspor. `CALC-006` har fået række-/periodefacitter; fortsæt med afklaring af `TD-016` og øvrige EO-grene. `TD-019` og `TD-020` er delvist lukket med henholdsvis schema-/issuefacitter og udvalgte sats-/downstream-facitter, herunder et konkret KL-facit gennem EO-inspektionen, et SBLON2-facit gennem lønudviklingsmotoren og et procesrente-facit gennem aggregationsmotoren.
