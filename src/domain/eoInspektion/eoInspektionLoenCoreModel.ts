...
export function buildLoenTimeline(input: LoenCoreInput): LoenTimeline {
  ...
  for (const inspektionDay of input.inspektionDays) {
    // Svie/smerte: kalenderdage, separat
    if (inspektionDay.svieSmerte !== 'Ingen') {
      ...
      if (baseSats !== undefined && baseSats > 0) {
        ...
        const amount =
          inspektionDay.svieSmerte === 'Delvis' && delvisMode === 'halv' ? baseSats / 2 : baseSats;
        svieSmerteDays.push({
          iso: inspektionDay.iso,
          niveau: inspektionDay.svieSmerte,
          amount,
        });
      }
    }
    ...
    if (!inspektionDay.isArbejdsdag || !eoRange) continue;
    if (inspektionDay.iso < eoRange.fra || inspektionDay.iso > eoRange.til) continue;
    if (!af?.overenskomstId) continue;

    const danishDate = toDanishOrUndefined(inspektionDay.iso);
