...
| Baseline: antal kildefiler, testfiler, tests, E2E-specs og mutationsscore | Ved auditstart: 923 produktionsfiler (`.ts/.tsx`), 638 testfiler i alt, 8.421 Vitest-tests, 36 E2E-specs og 174 E2E-tests. Slutnær revision `e8faa7ce`: 920 produktionsfiler, 660 Vitest-testfiler, 8.593 beståede tests samt 6 forventede `it.fails`, 40 E2E-specs og 179 beståede E2E-tests. Mutationsrunneren er kvalificeret modulvist: money 56/58 dræbt, `dateCommit.ts` 4/6 dræbt, årsløn 108/113 dræbt og varige mén 94/114 dræbt; alle overlevere er triageret. |
| Baseline: `test:coverage`-rapport og de dækkede/udeladte mapper | `coverage/` fra auditrevisionen: 396 instrumenterede filer i `src/domain`, `src/utils`, `src/hooks` og `src/contexts`; den seneste kørsel på `e8faa7ce` gav 89,22 % statements, 80,47 % branches, 92,35 % functions og 92,01 % lines – 20.584 / 16.090 / 3.218 / 18.230 målte enheder. Coverage-konfigurationen omfatter ikke de øvrige produktionsfiler; de skal klassificeres i inventaret. |
...
| Felt | Udfyldes |
| --- | --- |
| Inventar-ID / gennemgået revision | `DATA-001`, `CALC-001` / arbejdsrevision efter `e867ab99` |
...
| Manglende test, svage assertioner eller dubletter | De nye literal-facitter og downstream-oraklet styrker de registrerede endepunkter, men ikke alle downstream-forbrugere. Sygedagpengeregistret er bevidst udeladt, fordi den fundne officielle 2005-kilde (`https://www.retsinformation.dk/eli/mt/2004/102`) angiver 88,30 kr./time, mens produktdata angiver 88,51 kr./time; facit må ikke gættes. Flere eksisterende facitassertions genbruger fortsat produktionsdata eller helpers: `satserProjection.test.ts`, `aslAarsloensmaksimum.test.ts`, `opreguleringsmotorer.test.ts` og `satserNewCaseSeed.test.ts`. Validatorpartitioner for offentlig løn/statistik/overenskomst og komplet downstream-paritet mangler. |
...
| Kørte kommandoer, miljø og artefaktlink | `npx vitest run src/__tests__/data/uafhaengigSatsFacitmatrix.test.ts src/__tests__/domain/satser/satserDownstreamIndependentOracle.test.ts --reporter=dot` – de uafhængige facitspor bestod. `npx vitest run src/__tests__/data/uafhaengigeDataEndepunkter.test.ts --reporter=dot` – 25/25. `npx vitest run src/__tests__/data src/__tests__/domain/satser src/__tests__/quality/consumerInventory.test.ts --reporter=dot` – 23 filer / 477 tests bestået på Node `v24.18.0`/Windows. |
| Fund-ID'er, beslutninger og opfølgning | `TD-020` er delvist lukket for de statisk bevægelige endpoints og den målrettede downstream-kæde. Validatorpartitioner og komplet konsumentparitet står åbne. Sygedagpenge-2005-afvigelsen er fortsat et afklaringspunkt; runtime-data eller beregningsændringer forelægges udvikleren. |
| Reviewer / dato / slutstatus | Codex / 2026-09-10 / `I gang` |
...
| Felt | Udfyldes |
| --- | --- |
| Inventar-ID / gennemgået revision | `CALC-005` / `a6d85e00` |
...
- [x] `npm run test:coverage` er kørt på `e8faa7ce` efter auditens test- og død-kodeændringer og rapporteret med instrumenteret filmængde.
...
| `CALC-003` | Procesrente-breakdown og engine-tests | Uafhængig kalender-/kontinuitetsmodel og håndberegnet rente | 30. juni/1. juli, årsskifte, 2013-grænse og inklusivt dagtal | Oracle-sporene: 11/11; ingen produktkode ændret | Delvist – øvrige branches og dokumentkanaler åbne |
...
| TD-020 | 2026-09-11 | `DATA-001` / `CALC-001` | Mellem | Data-/sats-suiterne har stærke katalog-, fingerprint- og integritetsguards, men flere facitassertions læser produktionsdata som forventning. Validatorpartitioner og komplet downstream-paritet er derfor ikke uafhængigt bevist. | `uafhaengigeDataEndepunkter.test.ts` tilføjer 25 statiske endpoint-/partitionfacitter med historiske og nyere grænser. Den målrettede test bestod med 25/25. Den uafhængige satsmatrix og downstream-kæde består fortsat med den samlede data-/satssuite på 23 filer / 477 tests. Sygedagpenge-2005-afvigelsen er fortsat uafklaret. | Delvist lukket for de registrerede statiske endpoints. Åbent for validatorpartitioner og komplet konsumentparitet; runtime-data/validatorer ændres ikke som del af auditten. | Ikke påkrævet for test-only styrkelse. | Delvist lukket – endpointfacitter tilføjet, øvrige downstream-huller åbne |
