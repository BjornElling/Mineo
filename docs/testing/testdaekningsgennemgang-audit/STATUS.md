...
- Evidensrevision: `188c7ea7` for seneste samlede releasegate og målrettede test-only batch; seneste fulde E2E er `71746917`
- Branch: `main`
...
| Aktuel fuld Vitest-retest | `npm run test` | Bestået | 840 filer, 8.999 tests bestået og ingen forventede `it.fails` på `188c7ea7` |
...
| Aktuel samlet release-gate | `npm run verify:release:core` | Bestået | Dependency-, runtime-, type-, lint-, data-, kontrakt-, lane-, ledger-, coverage- og begge build-gates bestået på `188c7ea7`; coverage 840 filer / 8.999 beståede tests / ingen forventede `it.fails` |
...
| Aktuel coverage-retest | `npm run test:coverage` | Bestået | 394 instrumenterede filer; 90,38 % statements, 81,54 % branches, 94,33 % functions, 93,16 % lines; coverage-artefakterne blev genereret under `coverage/`; `188c7ea7`, Node `v24.18.0`/Windows |
...
| Senest tilføjet lukket | `TD-153`, `TD-154`, `TD-155`, `TD-157`–`TD-210` – test-only facitter dokumenteret til og med `188c7ea7`; `TD-156` er fortsat åbent |

## Aktuel evidensdelta – revision `188c7ea7`
...
Den seneste arbejdsbatch på `188c7ea7` har grøn målrettet retest og samlet releasegate. Den fulde
