...
- **Undgå kirurgisk staging.** Brug ikke `git stash`, `git add -p`, hunk-opdeling eller fil-for-fil-staging for at gøre opdelingen pæn. Hvis en ren `git add -A` + tematisk opdeling i nogle få `git commit` ikke rækker, så vælg den simplere gruppering frem for at finkæmme. Manuel hunk-patch er kun til at adskille dine egne ændringer fra andres (jf. ovenfor), ikke til at pynte på commit-grænser.
...
- En syntaktisk parsebar tal-/år-/ugeværdi uden for feltets aktive commit-interval er rejected rå tekst, ikke canonical input. Ved et ugyldigt settle ryddes feltets canonical slot til dets tomværdi, og den rå fejlende tekst gemmes atomisk — samme felt kan aldrig samtidig have en ikke-tom canonical værdi og rejected råtekst (XOR-invarianten). Den tidligere canonical værdi findes derefter **kun** i undo-historikken, aldrig maskeret i den aktuelle tilstand, og hverken den eller den rejected værdi må nå en beregning, selector, save-model eller dokumentmodel. Kronologiske datobounds og tværgående domæneregler forbliver derimod canonical input med et afledt rødt feltissue og følger fortsat deres specifikke kontrakter.
