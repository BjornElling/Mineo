...
| DATA-001 | Satser, reguleringer og data-/coverage-kataloger – `src/data/**`, `src/data/catalog/**` | Manglende, overlappende eller forkert gyldighedsdata giver forkert opslag eller skjult beregningshul. | `calculation-data-contract.md`, domænecontracts | unit/statisk/integration | `data/**`, `data/calculationDataCatalog.test.ts`, `data/rateSeriesIntegrity.test.ts`, `data/uafhaengigSatsFacitmatrix.test.ts`, `domain/satser/satserDownstreamIndependentOracle.test.ts`, `quality/consumerInventory.test.ts` | 23 data-/satserfiler med 477 tests er grønne og dækker katalog, fingerprints, huller, overlap, sortering, bounds, udvalgte historiske særtilfælde, literal-facit for flere registre samt en uafhængig sats → beregning → dokument-tabel-kæde. Alle kilderegistrenes endepunkter og validatorpartitioner mangler fortsat. Sygedagpengeregistret er bevidst ikke føjet til facitmatricen, fordi den fundne officielle 2005-kilde og produktdata afviger. | I gang |
...
| Felt | Udfyldes |
| --- | --- |
| Inventar-ID / gennemgået revision | `INPUT-001`, `INPUT-002`, `VALID-001` / arbejdsrevision efter `b40d3e27` |
...
| Kørte kommandoer, miljø og artefaktlink | `npx vitest run src/__tests__/inputCore src/__tests__/quality/acceptanceMatrix.test.ts src/__tests__/quality/contractCoverageMatrix.test.ts src/__tests__/quality/errorContractIsolation.test.ts src/__tests__/quality/eoFieldVisibilitySingleSource.test.ts src/__tests__/quality/eoRowGateUniqueContribution.test.ts src/__tests__/quality/freshSectionDefaults.test.ts src/__tests__/quality/freshCaseChoiceSweep.test.ts src/__tests__/schemas src/__tests__/validators --reporter=dot` – 65 filer / 1.518 tests bestået på Node `v24.18.0`/Windows. Den supplerende schema-/formatteringskontrol bestod med 3 filer / 56 tests. |
| Fund-ID'er, beslutninger og opfølgning | `TD-019` er delvist lukket: production dropdown-paritet, nested Zod-partitioner og faktiske `ZodError.issues` er nu dækket. Validator-fixture-uafhængighed står åben. Test-only styrkelser kan fortsat tilføjes uden ændring af runtime; ingen beregnings- eller loadadfærd må ændres som del af dette spor. |
| Reviewer / dato / slutstatus | Codex / 2026-09-10 / `I gang` |
...
| Felt | Udfyldes |
| --- | --- |
| Inventar-ID / gennemgået revision | `DATA-001`, `CALC-001` / arbejdsrevision efter `b40d3e27` |
...
| Eksisterende testfiler og testnavne | 23 filer / 477 tests under `src/__tests__/data`, `src/__tests__/domain/satser` og `quality/consumerInventory.test.ts`. De dækker katalogets 14 registrerede kilder, fingerprints, huller, overlap, sortering, bounds, missing-entry og udvalgte historiske særtilfælde. `uafhaengigSatsFacitmatrix.test.ts` har nu literal-facit for udvalgte KRL-, offentlig løn- og overenskomstperioder samt de tidligere lovbestemte satser, referencesats og ILON12/SBLON2-kvartaler. `satserDownstreamIndependentOracle.test.ts` følger desuden en statisk 2025/2026-reguleringsfacit gennem beregning og dokumenttabel. |
| Manglende test, svage assertioner eller dubletter | De nye literal-facitter og downstream-oraklet styrker udvalgte endepunkter, men ikke alle 14 kilder. Sygedagpengeregistret er bevidst udeladt, fordi den fundne officielle 2005-kilde (`https://www.retsinformation.dk/eli/mt/2004/102`) angiver 88,30 kr./time, mens produktdata angiver 88,51 kr./time; facit må ikke gættes. Flere eksisterende facitassertions genbruger fortsat produktionsdata eller helpers: `satserProjection.test.ts`, `aslAarsloensmaksimum.test.ts`, `opreguleringsmotorer.test.ts` og `satserNewCaseSeed.test.ts`. Validatorpartitioner for offentlig løn/statistik/overenskomst og komplet downstream-paritet mangler. |
...
| Kørte kommandoer, miljø og artefaktlink | `npx vitest run src/__tests__/data/uafhaengigSatsFacitmatrix.test.ts src/__tests__/domain/satser/satserDownstreamIndependentOracle.test.ts --reporter=dot` – de uafhængige facitspor bestod. `npx vitest run src/__tests__/data src/__tests__/domain/satser src/__tests__/quality/consumerInventory.test.ts --reporter=dot` – 23 filer / 477 tests bestået på Node `v24.18.0`/Windows. |
| Fund-ID'er, beslutninger og opfølgning | `TD-020` er delvist lukket for de udvalgte literal-endpoints og den målrettede downstream-kæde. Øvrige kilder, validatorpartitioner og komplet konsumentparitet står åbne. Sygedagpenge-2005-afvigelsen er fortsat et afklaringspunkt; runtime-data eller beregningsændringer forelægges udvikleren. |
| Reviewer / dato / slutstatus | Codex / 2026-09-10 / `I gang` |
...
| Felt | Udfyldes |
| --- | --- |
| Inventar-ID / gennemgået revision | `CALC-001` / `b40d3e27` |
...
| Eksisterende testfiler og testnavne | `aslAarsloensmaksimum.test.ts`, `opreguleringsmotorer.test.ts`, `satserNewCaseSeed.test.ts`, `satserProjection.test.ts` og `satserDownstreamIndependentOracle.test.ts`. |
...
| Fund-ID'er, beslutninger og opfølgning | `TD-020` er delvist lukket med den uafhængige downstream-facitprøve. Fortsæt med komponent-/E2E-rejse, dataregistrenes resterende endepunkter, validatorpartitioner og downstream-paritet. |
| Reviewer / dato / slutstatus | Codex / 2026-09-10 / `I gang` |
...
| Felt | Udfyldes |
| --- | --- |
| Inventar-ID / gennemgået revision | `DOC-001` / `af7acf2c` |
...
| Manglende test, svage assertioner eller dubletter | Det nye selvstændige spor lukker TD-011 for den generiske lifecycle-mekanisme og TD-012 for per-output/standalone-bevis: alle 18 Mineo- og 3 standalone-outputs observeres separat gennem gate, lazy renderer-load, session, render og fil-I/O, og blocked stopper før lazy-load. TD-013–TD-014 står åbne for flere lifecycle-faser og generel semantisk PDF/Word-paritet. |
...
| Kørte kommandoer, miljø og artefaktlink | Fokuseret lifecycle-/katalogkontrol: `npx vitest run src/__tests__/document/documentDefinitionFixtureRegistry.test.ts src/__tests__/document/documentLifecycleMatrix.test.ts src/__tests__/document/documentCatalogCompleteness.test.ts --reporter=dot` – 3 filer / 66 tests bestået. Den samlede dokumentmappe bestod med 23 filer / 242 tests på Node `v24.18.0`/Windows. |
| Fund-ID'er, beslutninger og opfølgning | TD-011 og TD-012 er lukket for henholdsvis generisk gate-vs-download og alle registrerede outputs. TD-013–TD-014 er åbne: udvidet lifecycle-fejlmatrix og tværgående PDF/Word-paritet. |
| Reviewer / dato / slutstatus | Codex / 2026-09-10 / `I gang` |
...
| `DATA-001` / `CALC-001` | Rate-/satskatalog og downstream-facitter | Flere eksisterende facitassertions genbruger produktionsdata, og sygedagpengeregistret har en uafklaret 2005-afvigelse | Styrk med uafhængige literal-facit og behold afvigelsen som åbent beslutningspunkt | `uafhaengigSatsFacitmatrix.test.ts` er udvidet med udvalgte literal-perioder, og `satserDownstreamIndependentOracle.test.ts` følger en statisk reguleringssats gennem beregning og dokumenttabel; øvrige kilder og komplet downstream-kæde står i `TD-020` | Delvist – `TD-020` |
...
| `SHELL-001` / `SHELL-002` | Auth, bootstrap, browsergate, PWA og recovery | Gate-sideeffekter er nu dækket, men ægte mobil-hard-stop og manglende lazy-chunk recovery mangler | Styrk kun observerbare browser-/recoveryforløb | `bootstrapUnsupportedDeviceSideEffects.test.tsx` dækker sideeffekterne; shell-/PWA-E2E er grønne; `TD-021`/`TD-022` står åbne | Delvist – `TD-021`/`TD-022` |
...
| TD-019 | 2026-09-10 | `INPUT-001` / `INPUT-002` / `VALID-001` | Mellem | Form/grid-pariteten er nu bundet til en reel produktionsdescriptor, nested row-schemaernes unknown/missing/invalid-partitioner er dækket, og faktiske `ZodError.issues` bruges. Flere validatorfixtures genbruger fortsat produktionsdefaults. | Den målrettede kontrol bestod med 65 filer / 1.518 tests; `nestedRowSchemas.test.ts` dækker 12 schemas, og `zodIssueFormatting.test.ts` bruger faktiske Zod-fejl. Validator-fixture-uafhængighed står fortsat åben. | Delvist lukket. Styrk validatorfixtures med små uafhængige literal-inputs, uden runtime- eller loadændring. | Ikke påkrævet for testændringen, så længe runtime-/loadadfærd ikke ændres. | Delvist lukket – validatorfixtures mangler |
| TD-020 | 2026-09-10 | `DATA-001` / `CALC-001` | Mellem | Data-/sats-suiterne har stærke katalog-, fingerprint- og integritetsguards, men flere facitassertions læser produktionsdata som forventning. Alle historiske endpoints, validatorpartitioner og komplet downstream-paritet er derfor ikke uafhængigt bevist. | `uafhaengigSatsFacitmatrix.test.ts` er udvidet med statiske KRL-, offentlig løn- og overenskomstperioder, og `satserDownstreamIndependentOracle.test.ts` følger en statisk reguleringssats gennem beregning og dokumenttabel. Den målrettede suite bestod med 23 filer / 477 tests. Sygedagpenge-2005-afvigelsen er fortsat uafklaret. | Delvist lukket for de udvalgte endpoints og downstream-kæden. Åbent for øvrige kilder, validatorpartitioner og komplet konsumentparitet; runtime-data/validatorer ændres ikke som del af auditten. | Ikke påkrævet for test-only styrkelse. | Delvist lukket – øvrige kilder og validatorer mangler |
