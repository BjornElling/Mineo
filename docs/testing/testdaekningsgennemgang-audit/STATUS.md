...
- Evidensrevision: `41eec477` for seneste målrettede test-only batch; seneste samlede releasegate er `27cc0751`, og seneste fulde E2E er `71746917`
- Branch: `main`
...
- Seneste test-only batch: BUILD-001-facittet TD-303; målrettet quality-suite 4/4 grønne tests; ingen produktkode, workflow, beregningslogik, UI/UX eller persistens ændret. Seneste samlede releasegate er fortsat grøn på `27cc0751`; den er endnu ikke gentaget efter TD-303.
...
| Aktuel fuld Vitest-retest | `npm run test:coverage` | Bestået | 920 filer, 9.098 tests bestået og ingen forventede `it.fails` på `27cc0751` |
...
| Aktuel samlet release-gate | `npm run verify:release:core` | Bestået | Dependency-, runtime-, type-, lint-, data-, kontrakt-, lane-, ledger-, coverage- og begge build-gates bestået på `27cc0751`; coverage 920 filer / 9.098 beståede tests / ingen forventede `it.fails` |
...
| Senest tilføjet lukket | `TD-153`, `TD-154`, `TD-155`, `TD-157`–`TD-303` – test-only facitter dokumenteret; `TD-156` er fortsat åbent |
