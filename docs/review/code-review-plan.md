...
**Opfølgning (2026-06-15) — parkerede 7.x-konsolideringer taget op (adfærds-neutralt, fuld suite grøn 5228/434):** `useSliceRowDrafts`-factory (6 row-hooks, 7.4 #3); `spliceDraftPaste`-helper (6 adaptere, 7.2 #2); `tableGridGeometry`-modul udtrukket fra `tableKeyboardNavigation` (7.3 #4); `gridUxSpec.guard.test` (7.3 #6); `ReadOnlyLockedCell`-merge + `VirtualizedDisplayTable.renderBody` (7.4 #4/#5); `TableDropdown.commitCurrent`-kommentar (7.2 #3). **Bevidst parkeret/forelagt (begrundet i de enkelte 7.x-filer):** ~~`useStyledFieldAdapter` (7.1 Tilf.1)~~, ~~dobbelt fokus-autoritet + modul-globale WeakMaps (7.3 #2/#3)~~. §5.1 inline dato-bounds (7.4 #1) forbliver krydsref. gruppe 8.

**Dedikeret arkitektur-runde (2026-06-15) — begge 7.x-parkeringer gennemført, adfærds-neutralt (fuld suite grøn 5238/437):**
- **`useStyledFieldAdapter` (7.1 Tilf.1) ✅ Gjort.** Ny delt hook (`src/hooks/useStyledFieldAdapter.ts`) ejer den identiske commit-/redigerings-lim (invalidDraft-kanal + `useDraftField` + to-trins-aktivering + draft-change/keydown/paste/blur-skelet) for de **syv** numeriske blur-commit-felter (Amount/Date/Integer/Percent/Fraction/Week/Year). Per-felt-variation udtrykkes via navngivne, defaultede seams (`transformDraftOnChange`, `keyFilter`+`gateKeyFilterOnInvalidTouched`, `onClearSideEffect`/`onDraftChangeSideEffect`, `shouldCommitOnBlur`, `escapeRevertsToFormatted`, `setPasteCaret`, `commitOnClosedPaste`, `onStartEditing` med runtime-helpers, `rejectDraft`). `StyledTextField` (fri tekst + textarea) er BEVIDST holdt udenfor (to render-mål, fri-tekst-semantik, caret-limbo-fix). Eneste adfærds-detalje rettet undervejs: `StyledIntegerField`s Backspace-clear fik den ændrings-guard de øvrige 7 fik i 7.1 (ingen overflødig undo-frame ved clear af et allerede tomt felt). `fieldUnchangedGuardInvalidDraft.test.ts` omskrevet arkitektur-bevidst (scanner hook'en + bespoke Text + kræver at de 7 delegerer til hook'en).
- **Dobbelt fokus-autoritet + WeakMaps (7.3 #2/#3) ✅ Gjort.** De døde `requestFocusPlan`-kald fjernet fra de tre rene nav-grene (Enter/vertikal/horisontal pil) — fokus-planen er nu KUN til udskudt fokus hen over editor-luk (Delete-commit + `closeEditing→executeFocusPlan`). Den fokuserede celle har én autoritet: `core.setFocusedCell` sat fra `handleTableFocusCapture` når fysisk DOM-fokus lander. `physicalFocusByTable`-WeakMap'en (det eneste reelle controller-spejl) fjernet; pointerDown læser nu `core.getFocusedCell()` (gated af den blur-nulstillede `clickEditableCellByTable`). De øvrige fire WeakMaps dokumenteret som ægte modul-lokal gestus/nav-state (ikke duplikering). Bevist adfærds-neutralt: `executeFocusPlan` var altid en no-op for nav (plan.to == allerede-fokuseret celle).

**Grid-persist konvergens (7.4 #2) ✅ Gjort (2026-06-15, brugergodkendt) — afdækkede en reel bug.** Brugeren bad om at få afklaret, om de fire grid-tabellers persist-/resync-forskelle var tilsigtede. Empirisk probning viste at fokus/delete-adfærd allerede er ens (delt grid-kerne), men afslørede to reelle uoverensstemmelser: (1) tre tabeller persisterede den syntetiske tomme række, `EetAsl` strippede den (kontrakt-korrekt); (2) **bug:** `StandardLoenTable` manglede `reconcileRowIdsByPosition` → undo mistede celle-fokus (regressionstest tilføjet). Begge rettet via ny fælles `gridCore/useGridRowPersistenceCore.ts` (strip-empties + reconcile + fingerprint-bevogtet flush ét sted); alle fire tabeller migreret. Eneste tilsigtede adfærdsændring: kun bruger-indtastede rækker persisteres (ikke synligt på skærmen). Fuld suite grøn (5239+ tests).
