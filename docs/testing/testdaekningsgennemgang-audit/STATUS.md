...
- Evidensrevision: `9655f694` for seneste samlede releasegate og målrettede test-only batch; seneste fulde E2E er `71746917`
- Branch: `main`
...
- Seneste test-only batch: PERSIST-/CALC-facitter TD-225–TD-226; målrettet kontrol 3 filer / 17 tests inklusive relaterede facitter; ingen produktkode, beregningslogik, UI/UX eller persistens ændret.
...
| `PERSIST-002` | Storage/settings, filhåndtag, PWA-filflows og reel session-reload gennemgået | 16 filer / 181 målrettede unit-/hook-tests; PWA-/filkørsel 4/4; reel input + aktiv fane + reload 1/1; supplerende 8-filers platformssuite med 89/89 beståede tests efter callable picker-detektion; `safeLocalStorage.test.ts` bestod med 11/11 direkte Node-fallback-isolationsassertions; `fileHandleVerification.test.ts` bestod med 22/22; `fileHandleKvStore.test.ts` bestod med 2/2; `fileOperationClientSession.persistence.test.ts` tilføjer 1/1 sessionStorage-klientidentitetsfacit efter simuleret reload; `fileSaveTarget.test.ts` tilføjer 1/1 orphaned-handle-facit før picker-flowet; `fileSaveTargetPermissionApiFailure.test.ts` tilføjer 1/1 permission-API-facit med handle-oprydning og picker-fallback; ugyldige `Date`-inputs har 8/8 grønne negative modcases | Fuld browser-/platformssammenhæng, herunder installeret PWA-filaflevering; `TD-022` | `I gang` |
...
Aktuel inventarrevision på `9655f694`: `PERSIST-002` omfatter nu også `fileSaveTargetPermissionApiFailure.test.ts`
med 1/1 permission-API-facit, og `CALC-006` omfatter `eoInspektionOffentligeYdelserInvalidRowsIndependent.test.ts`
med 1/1 facit for ugyldige offentlige ydelsesrækker. De tilsvarende målrettede inventartal er dermed 16 filer /
181 tests for PERSIST-002 og 168 filer / 2.182 tests for CALC-006; række-/periodefacitterne er 15/15.
...
| Senest tilføjet lukket | `TD-153`, `TD-154`, `TD-155`, `TD-157`–`TD-226` – test-only facitter dokumenteret til og med `9655f694`; `TD-156` er fortsat åbent |

## Aktuel evidensdelta – revision `9655f694`

Den seneste batch tilføjer to disjunkte test-only facitter uden produktændringer:

- `TD-225`: `fileSaveTargetPermissionApiFailure.test.ts` fastholder med 1/1 permission-API-
  fejlens præcise warning, handle-oprydning, picker-fallback og `persistHandleAfterSuccess`.
- `TD-226`: `eoInspektionOffentligeYdelserInvalidRowsIndependent.test.ts` fastholder med 1/1
  blank ydelsestype, ukendt ydelsestype og omvendt dato-interval med beløb, mens tilsvarende
  tomme rækker ignoreres.

Den målrettede kontrol bestod med 3 filer / 17 tests inklusive relaterede eksisterende facitter.
Typechecks, lint, encoding-, filnavns- og diff-kontroller bestod. `verify:release:core` bestod
derefter med 853 testfiler / 9.028 tests, coverage 90,69 / 81,87 / 94,76 / 93,49 og begge
