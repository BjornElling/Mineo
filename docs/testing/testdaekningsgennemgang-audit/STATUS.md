...
| Validatorer og dataendepunkter | Målrettet Vitest-kørsel (se hoveddokumentet) | Bestået | 7 nye filer, 43 tests; uafhængige validator-literals, 5 håndskrevne typed validator-cases, obligatorisk top-level schema-facit, statiske data-/endpointfacitter, 2 folkepensions-downstreamfacitter, 1 KRL-downstreamfacit og 1 procesrente-downstreamfacit |
...
| `INPUT-002` / `VALID-001` | Form/grid-overflader, produktions-ChoiceField og schema-/validatorhuller gennemgået | 66 filer / 1.519 tests; produktionsbundet form/grid-paritet, 12 nested row-schemas, faktiske `ZodError.issues`, 10 uafhængige validator-literaltests, 5 håndskrevne typed runtime-cases og et obligatorisk top-level schema-facit er dækket | Yderligere validator-fixture-uafhængighed og downstream-partitioner; `TD-019` | `I gang` |
