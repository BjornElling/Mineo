...
const stamdata: StamdataValues = {
  journalnr: 'TD-020-kapitalisering-2026',
  advokat: '',
  sagsbehandler: '',
...
const erhvervsevnetab: ErhvervsevnetabValues = {
  ...
  koen: 'Kvinde',
  aslAfgoerelser: [{
    id: 'td020-kapitalisering-2026-row',
...
const project = (): KapitaliseringDocumentInput => {
  ...
  if (result.status !== 'ready') {
    throw new Error('Kapitaliseringsdefinitionen blev blokeret i TD-020-facittet');
  }
  return result.input;
};

describe('TD-020 – 2026-kapitalisering gennem Word-generatoren', () => {
  it('fører 10056/2025 tabel A fra typed input til beregning og Word', async () => {
    const input = project();
    const afgoerelse = input.computation.afgoerelser[0];

    expect(afgoerelse).toEqual(expect.objectContaining({
      rowId: 'td020-kapitalisering-2026-row',
      afgoerelsesdato: '2025-12-15',
      kapitaliseringsdato: '2026-01-01',
      kapitaliseringsbekendtgoerelseLabel: 'Vejl. 10056/2025, tabel A',
      tabelLabel: 'A',
    ...
    expect(text).toContain('Vejl. 10056/2025, tabel A');
    expect(text).toContain('Alder ved kapitalisering55 år, 0 måneder');
    expect(text).toContain('Folkepensionsalder70 år');
    expect(text).toContain('Kapitaliseringsfaktor10,018');
  });
});
