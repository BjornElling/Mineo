...
// Svie/smerte-kravet er inaktivt, så den negative canonical beløbsværdi isolerer
// validatorens grænse for den aktuelle periode.
const INDEPENDENT_VALUES: ErstatningsopgoerelseValues = {
  ...
  svieSmerteAktuelPeriode: { kind: 'number', value: -1 },
...
describe('TD-019 – aktuel svie/smerte-periode uden schema-fixture', () => {
  it('afviser negativt aktuelt svie/smerte-beløb med præcis feltsti, besked og severity', () => {
    const result = erstatningsopgoerelseValidator.validateParsed(INDEPENDENT_VALUES);

    expect(result).toEqual({
      isValid: false,
      errors: [{
        path: 'svieSmerteAktuelPeriode',
