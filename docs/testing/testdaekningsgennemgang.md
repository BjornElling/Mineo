...
| INPUT-001 | Input-aggregate, feltadresser, catalog og editor/settle-engine – `src/inputCore/**` | Draft kan nå beregning/save, rejected tekst kan sameksistere med canonical, eller input kan forsvinde ved sync. | `form-contract.md`, `input-field-behavior-contract.md`, `amount-contract.md` | unit/integration | `inputCore/inputCore.test.ts`, `inputCore/editor/fieldEditor.test.ts`, `inputCore/runtime/dispatchInput.test.ts`, `inputCore/runtime/commandInvariants.test.ts`, `inputCore/stateChains.test.ts` | Acceptance-registret kobler kerneinvarianter til leaf-tests; mutation/modprøver mangler. | I gang |
...
| PERSIST-001 | `.eo`-schema, codec, save/load, migrering, preflight og atomisk apply – `src/persistence/**`, `src/utils/file*.ts` | Sagsdata kan gå tabt, gammel fil kan blive ulæselig, eller afvisning kan mutere aktiv sag. | `persistence-contract.md`, `schema-evolution.md` | unit/integration/E2E | `persistence/**`, `utils/eoFileCodec.test.ts`, `fileRoundTrip.fullState.test.ts`, `persistenceMigrations.test.ts`, `fileLoad.normalLoad.test.ts`, `e2e/file-load-validation.spec.ts` | 24 filer og 233 tests bestået. Der er ingen versionsbundne `.eo`-fixtures i repositoryet; historiske payloads er inline og kræver uafhængig fixturekontrol. | I gang |
...
| Mutation/modprøve og resultat | Ikke gennemført. Der findes ingen kvalificeret mutationsrunner, og der er endnu ikke kørt en kontrolleret svækket assertion eller uafhængig referenceprøve. Derfor kan de beståede tests endnu ikke afslutte rækken. |
| Kørte kommandoer, miljø og artefaktlink | `npx vitest run src/__tests__/inputCore/inputCore.test.ts src/__tests__/inputCore/editor/fieldEditor.test.ts src/__tests__/inputCore/runtime/dispatchInput.test.ts src/__tests__/inputCore/runtime/commandInvariants.test.ts src/__tests__/inputCore/stateChains.test.ts` – 5 filer, 176 tests bestået på 4,06 s. Fuld baseline står i [auditsporet](testdaekningsgennemgang-audit/STATUS.md). |
| Fund-ID'er, beslutninger og opfølgning | Ingen produktfund registreret. Åbne auditpunkter: mutationsrunner, uafhængig modprøve, komplet testkvalitetsrevision og browser-/adapterparitet. |
...
| --- | --- | --- | --- | --- | --- |
| `PERSIST-001` | Unit-/integrationstest af save/load og migrering | Versionsbundne `.eo`-fixtures fra tidligere udgivelser samt uafhængig struktursammenligning | Alle historiske versioner og registrerede migreringsgrene | Ingen `.eo`-fixtures fundet; inline payloads er ikke tilstrækkeligt uafhængige | I gang |
...
| --- | --- | ---: | ---: | --- | --- | --- | --- |
| | relation, aritmetik, boundary, conditional, string, array, return mv. | | | | | | |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| TD-001 | 2026-09-10 | `PERSIST-001` | Høj | Der findes ingen versionsbundne `.eo`-fixtures i repositoryet. Historisk load er derfor kun prøvet med inline payloads, og auditten kan endnu ikke bevise, at hver tidligere udgivet container indlæses uden ændret sagsdata eller ny preflight. | `rg --files -g '*.eo'` gav ingen resultater uden for ignorerede artefakter. Historiske cases i `fileLoad.normalLoad.test.ts`, `persistenceMigrations.test.ts` og E2E-filen er konstruerede payloads. | Åben auditopgave: fremskaf eller genskab repræsentative fixtures for alle tidligere udgivne formater, dokumentér proveniens og tilføj uafhængig struktursammenligning. Ingen produktændring foretaget. | Ikke påkrævet for dokumentation af fundet; en senere test-/fixtureændring skal vurderes som egen ændring. | Åben – ingen retest endnu |
