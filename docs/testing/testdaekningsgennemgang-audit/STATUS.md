...
| Aktuel fuld Vitest-retest | `npm run test:coverage` | Bestået | 695 filer, 8.748 tests bestået samt 17 forventede `it.fails`; coverage 90,15 / 81,24 / 93,97 / 92,96; coverage-fase 379,85 s på `7fb82f0a` |
...
| Aktuel samlet release-gate | `npm run verify:release:core` | Bestået | Dependency-, runtime-, type-, lint-, data-, kontrakt-, lane-, ledger-, coverage- og begge build-gates bestået på `7fb82f0a`; coverage 695 filer / 8.748 beståede tests / 17 forventede `it.fails` |
...
| Aktuel coverage-retest | `npm run test:coverage` | Bestået | 393 instrumenterede filer; 90,15 % statements, 81,24 % branches, 93,97 % functions, 92,96 % lines; 18.557 / 20.584, 13.073 / 16.090, 3.024 / 3.218 og 16.948 / 18.230 målte enheder; `coverage/index.html`, `coverage/clover.xml` og `coverage/coverage-final.json`; `7fb82f0a`, Node `v24.18.0`/Windows |
