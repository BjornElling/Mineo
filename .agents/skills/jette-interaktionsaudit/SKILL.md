...
1. **Aktivering:** Brug kun skillen, når udvikleren udtrykkeligt beder om at få udført eller genoptaget netop en Mineo-interaktionsaudit. Aktivér den aldrig af egen drift, og brug den aldrig til andre formål – heller ikke fordi en anden opgave involverer browseradfærd, test, robusthed, fejlsøgning, review eller gennemgang af Mineo. En generel anmodning om test, review eller browserkontrol er ikke tilstrækkelig; anmodningen skal tydeligt omfatte denne specifikke, systematiske interaktionsaudit.
2. **Kørsel efter aktivering:** Når udvikleren har aktiveret skillen som beskrevet ovenfor, skal auditten køre autonomt og vedvarende uden løbende bekræftelser fra udvikleren. Reglerne om at fortsætte uden nyt input fra udvikleren gælder kun for den allerede aktiverede audit og må aldrig fortolkes som tilladelse til selv at starte en ny audit i en senere eller anden opgave.
...
Gem `$keepAwakeProcess.Id` eller PID-filen `.agents/skills/jette-interaktionsaudit/state/keep-awake.pid`, og stop processen ved en entydig udviklerbesked om pause/stop. Helperen forhindrer kun normal Windows idle-sleep; den løser ikke strømudfald, genstart, tvungen sleep, netværksfejl eller en mistet Codex-forbindelse. Hvis den ikke kan starte, registreres det som et dæknings-/driftsforhold, og checkpoint-/resume-protokollen fortsætter stadig.
...
6. Kontrollér `git status --short`, aktuel commit og buildversion. Behandl eksisterende ændringer som udviklerens og rør dem ikke.
