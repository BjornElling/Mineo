...
| Aktuel fuld Vitest-retest | `npm run test` | Bestået | 822 filer, 8.973 tests bestået og ingen forventede `it.fails` på `fa52a9c9` |
...
| Aktuel samlet release-gate | `npm run verify:release:core` | Bestået | Dependency-, runtime-, type-, lint-, data-, kontrakt-, lane-, ledger-, coverage- og begge build-gates bestået på `fa52a9c9`; coverage 822 filer / 8.973 beståede tests / ingen forventede `it.fails` |
...
| Aktuel coverage-retest | `npm run test:coverage` | Bestået | 394 instrumenterede filer; 90,37 % statements, 81,50 % branches, 94,33 % functions, 93,15 % lines; coverage-artefakterne blev genereret under `coverage/`; `fa52a9c9`, Node `v24.18.0`/Windows |
...
| Senest tilføjet lukket | `TD-153`, `TD-154`, `TD-155`, `TD-157`–`TD-187` – test-only facitter dokumenteret til og med `fa52a9c9`; `TD-156` er fortsat åbent |

## Aktuel evidensdelta – revision `fa52a9c9`
...
Den seneste samlede releasegate på `fa52a9c9` er retestet grønt; den fulde E2E-suite er senest grøn
på `71746917` med 221/223. Den seneste arbejdsbatch tilføjer tre test-only facitter, og både
målrettet retest og samlet releasegate er grønne på `fa52a9c9`. Næste arbejdsenhed er en ny
