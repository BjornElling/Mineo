...
import { buildRestoreTargetAttributes } from '../../../inputCore/react/greenfieldHistoryRestore';
...
describe('Greenfield restore-target-attributter på det fokuserbare element (§3.7)', () => {
  it('form-tekstfelt (GreenfieldIntegerField)', () => {
    renderField(<GreenfieldIntegerField field={aargangField.bind()} location={{ locationId: 'loc-int' }} name="aargang" />);
    expectRestoreAttrs(screen.getByRole('textbox'), serializeFieldAddress(aargangField.bind().address), 'loc-int');
  });

  it('form-dropdown (GreenfieldChoiceField)', () => {
  ...
  it('radio (GreenfieldRadioField) — den valgte radio bærer attributterne', () => {
    dispatchInput(store, catalog, insertRow(rentekravRef(), makeRow('r1', { enhed: 'uger' })));
    renderField(
      <GreenfieldRadioField<TillaegstidEnhed>
        field={enhedField.bind('r1')}
  ...
  it('grid tekstcelle (GreenfieldGridAmountCell)', () => {
  ...
  it('grid dropdown-celle (GreenfieldGridChoiceCell)', () => {
    dispatchInput(store, catalog, insertRow(rentekravRef(), makeRow('r1', { enhed: 'dage' })));
    ...
    render(
      <InputRuntimeProvider binding={makeBinding()}>
        <GridCoreProvider value={{
          ...
          <GreenfieldGridChoiceCell<TillaegstidEnhed, unknown, TillaegstidEnhed>
            gridCell={gridCell}
